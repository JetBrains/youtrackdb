...
Generated code lives under `core/src/main/generated/java/`. **Both types are in version control**, are **not removed by `mvn clean`**, and are **regenerated only when their source (grammar or DSL) changes**.

#### SQL Parser (JavaCC)
- **Source**: `core/src/main/grammar/YouTrackDBSql.jjt`
- **Generator**: javacc-maven-plugin
- **Output**: `core/src/main/java/com/jetbrains/youtrackdb/internal/core/sql/parser/`
- **Git**: Not in version control (regenerated on every build)
- **Editing**: Never edit generated parser files directly - edit the `.jjt` grammar file

#### ANTLR Grammar
- **Source**: `core/src/main/grammar/antlr/GQL.g4`
- **Generator**: antlr4-maven-plugin
- **Output**: `core/src/main/generated/java/antlr/`
- **Git**: **Committed to version control.** Not cleaned by `mvn clean`. Regenerated only when the grammar file changes.
- **Editing**: Never edit generated files directly - edit the `.g4` grammar

#### Gremlin DSL (Annotation Processor)
- **Source**: `@GremlinDsl` annotated interfaces in `core/src/main/java/com/jetbrains/youtrackdb/api/gremlin/`
- **Generator**: `gremlin-annotations` module's `GremlinDslProcessor` annotation processor
- **Output**: `core/src/main/generated/java/gremlin/com/jetbrains/youtrackdb/api/gremlin/`
- **Git**: **Committed to version control.** Not cleaned by `mvn clean`. Regenerated only when DSL source content changes (processor stores CRC of DSL source in generated file comments and skips generation when it matches).
- **Editing**: Never edit generated files directly - edit the `@GremlinDsl` interfaces to regenerate
...
- **Qodana**: Static analysis integrated as a pipeline job (zero tolerance for critical/high/moderate issues; excludes generated code: SQL parser and `core/src/main/generated/java/`)
...
- Qodana excludes generated code: SQL parser (`core/.../sql/parser/`) and `core/src/main/generated/java/` (ANTLR + Gremlin DSL)
...
3. **Generated code** (in git, not cleaned by `mvn clean`; regenerated only when grammar/DSL changes):
   - **Never edit**: SQL parser (`core/.../sql/parser/`), ANTLR (`core/src/main/generated/java/antlr/`), Gremlin DSL (`core/src/main/generated/java/gremlin/`) - edit grammar or `@GremlinDsl` interfaces to regenerate
...
11. **Gremlin annotation processor**: The `core` module depends on generated Gremlin DSL classes from `gremlin-annotations`. After `clean`, always compile `gremlin-annotations` first: `./mvnw -pl gremlin-annotations compile -q` before running core-only builds, or use `./mvnw -pl gremlin-annotations,core clean package`
