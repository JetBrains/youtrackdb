...
import com.jetbrains.youtrackdb.ycsb.measurements.exporter.MeasurementsExporter;
import com.jetbrains.youtrackdb.ycsb.measurements.exporter.TextMeasurementsExporter;
...
public final class Client {
   ...
   * com.jetbrains.youtrackdb.ycsb.measurements.exporter.TextMeasurementsExporter.
   */
  ...
  public static void usageMessage() {
    System.out.println("Usage: java com.jetbrains.youtrackdb.ycsb.Client [options]");
    ...
    System.out.println(
        "use the \"insertcount\" and \"insertstart\" properties to divide up the records " +
            "to be inserted");
  }
  ...
  private static void exportMeasurements(Properties props, int opcount, long runtime)
    ...
    try {
      ...
      try {
        exporter =
            (MeasurementsExporter) Class.forName(exporterStr).getConstructor(OutputStream.class)
                .newInstance(out);
      } catch (Exception e) {
  ...
  public static void main(String[] args) {
    ...
    initWorkload(props, warningthread, workload);

    System.err.println("Starting test.");
    final CountDownLatch completeLatch = new CountDownLatch(threadcount);

    final List<ClientThread> clients = initDb(dbname, props, threadcount, targetperthreadperms,
        workload, completeLatch);

    if (status) {
      boolean standardstatus = false;
      if (props.getProperty(Measurements.MEASUREMENT_TYPE_PROPERTY, "").compareTo("timeseries")
          == 0) {
        standardstatus = true;
      }
      ...
      statusthread =
          new StatusThread(completeLatch, clients, label, standardstatus, statusIntervalSeconds,
              trackJVMStats);
      statusthread.start();
    }
