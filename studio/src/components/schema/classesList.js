...
let SchemaClassesController = function(
  $scope,
  $element,
  $attrs,
  $location,
  ...
  ctrl.openClass = function(clazz) {
    $location.path(
      "/database/" + ctrl.database + "/schema/editclass/" + clazz.name
    );
  };
  ...
  ctrl.queryAll = function(className) {
    ...
    $location.path(
      "/database/" + ctrl.database + "/browse/select * from " + className + ""
    );
  };
  ...
  ctrl.rename = function(cls, event) {
    ...
    var modalPromise = $modal({
      templateUrl: "views/database/changeNameModal.html",
      scope: modalScope,
      show: false
    });
    ...
    modalScope.rename = function(name) {
      if (name != cls.name) {
        ...
        SchemaService.alterClass(
          ...
          .then(() => {
            let noti = S(
              "The class {{name}} has been renamed to {{newName}}"
            ).template({
              name: cls.name,
              newName: name
            }).s;
          ...
          .catch(err => {
            Notification.push({
              content: FormatErrorPipe.transform(err.json()),
              error: true
            });
          });
      }
      ...
      success: function() {
        SchemaService.dropClass(
          ...
          .then(() => {
            ...
            ctrl.classes.splice();
            Notification.push({
              content: "Class '" + nameClass["name"] + "' dropped."
            });
          })
          .catch(err => {
            Notification.push({
  ...
  ctrl.createRecord = function(className) {
    $location.path(
      "/database/" + ctrl.database + "/browse/create/" + className
    );
  };
