...
import "../views/server/stats/cloud.html";
...
ServerModule.controller("ServerController", [
  "$scope",
  "$routeParams",
  "ServerApi",
  "Database",
  "NgTableParams",
  function($scope, $routeParams, ServerApi, Database, ngTableParams) {
...
ServerModule.controller("ServerStatusController", [
  "$scope",
  "$rootScope",
...
ServerModule.controller("MultipleServerController", [
  "$scope",
  "$rootScope",
  "$location",
  "$routeParams",
...
ServerModule.controller("SingleChartServerController", [
  "$scope",
  "ChartHelper",
  function($scope, ChartHelper) {
    $scope.transactionHeaders = angular.copy(ChartHelper.serverStatsHeader);
  }
]);
ServerModule.controller("SingleServerController", [
  "$scope",
  "$rootScope",
  "$location",
  "$routeParams",
  ...
  function(
    ...
    function initParamenters(realtime) {
      if (realtime && realtime["statistics"]["process.runtime.maxMemory"]) {
        ...
        var maxMemory =
          realtime["statistics"]["process.runtime.maxMemory"].last;
        var totalMemory =
        ...
        if (lastOps != null) {
          $scope.operations = Math.round(
            Math.abs(lastOps - ops) / (POLLING / 1000)
          );
        }
        lastOps = ops;
        ...
        if (realtime["chronos"]["server.network.requests"]) {
          ...
          if (lastReq != null) {
            $scope.requests = Math.round(
              Math.abs(req - lastReq) / (POLLING / 1000)
            );
          }
          lastReq = req;
        }
        if (realtime["chronos"]["distributed.node.latency"])
          $scope.latency =
            realtime["chronos"]["distributed.node.latency"].average;
      }
    }
...
ServerModule.controller("ServerDashboardController", [
  "$scope",
  "$routeParams",
  "Aside",
  "ServerApi",
...
ServerModule.controller("ServerConnectionController", [
  "$scope",
  "$filter",
  "NgTableParams",
  "Collection",
...
ServerModule.controller("LogsController", [
  "$scope",
  "$http",
  "$location",
  "$routeParams",
  ...
  function(
    ...
    $scope.$watch("server", function(data) {
      if (data) {
        CommandLogApi.getListFiles({ server: $scope.server.name }, function(
          data
        ) {
          if (data) {
            $scope.files = ["ALL_FILES", "LAST"];
    ...
    $scope.$watch("countPage", function(data) {
      if ($scope.resultTotal) {
        ...
        $scope.currentPage = 1;
        $scope.numberOfPage = new Array(
          Math.ceil($scope.resultTotal.logs.length / $scope.countPage)
        );
      }
    });
          ...
          function(data) {
            if (data) {
              ...
              if (data.logs) {
                ...
                $scope.currentPage = 1;
                $scope.numberOfPage = new Array(
                  Math.ceil(data.logs.length / $scope.countPage)
                );
              }
            }
            Spinner.stopSpinner();
