 ...
 *
 * The bare hop is a {@link VertexStep} with {@code returnsEdge() == false}, the same registry class
 * as the edge-returning step. The registry keys one recogniser per class, so {@link
 * VertexStepRecogniser} owns {@code VertexStep.class} and routes here on the {@code
 * returnsEdge() == false} branch (routing the edge-returning branch to {@link EdgeHopRecogniser}).
 * This recogniser is never in the walker registry. It re-asserts its full precondition — the step is
 * a {@link VertexStep} <em>and</em> is <em>not</em> edge-returning ({@code returnsEdge() == false}) —
 * so a direct mis-call (a non-{@code VertexStep}, or an edge-returning {@code outE(L)} step that
 * belongs to {@link EdgeHopRecogniser}) declines cleanly with {@code 0} rather than throwing {@code
 * ClassCastException} or mis-translating an edge step as a bare hop. This mirrors {@link
 * EdgeHopRecogniser}'s precondition, which declines a non-{@code VertexStep} or a non-edge-returning
 * step.
 *
...
final class VertexHopRecogniser implements StepRecogniser {
  ...
  public int recognize(Step<?, ?> step, WalkerContext ctx) {
    // Defence in depth: the router only routes a bare (returnsEdge() == false) VertexStep here, but
    // re-assert the full precondition — a VertexStep that is NOT edge-returning — so a direct
    // mis-call declines cleanly rather than ClassCastException-ing (non-VertexStep) or mis-
    // translating an edge-returning outE(L) step as a bare hop. Mirrors EdgeHopRecogniser, which
    // declines a non-VertexStep or a non-edge-returning step.
