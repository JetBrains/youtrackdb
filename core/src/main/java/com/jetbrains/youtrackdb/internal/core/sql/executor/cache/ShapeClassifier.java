...
public final class ShapeClassifier {
   ...
   * reconciled. Only a name that is not an aggregate at all (a scalar like {@code lower(name)})
   * returns {@code null}.
   */
  private static CacheableShape aggregateShapeForCall(@Nonnull SQLFunctionCall call) {
    ...
    return switch (lower) {
      ...
      // Aggregates with no AggregateState replay arm. Left unrecognised they fell through to RECORD,
      // which hands a scalar row with no RID to the per-record delta builder. mean divides in
      // floating point where avg divides in the input's own arithmetic, so it cannot borrow
      // AGGREGATE_AVG's replay either. median, mode and percentile need the whole multiset at
      // finalisation, so a running scalar cannot stand in for one; variance and stddev do have
      // streaming formulations and could be replayed if an arm were added, but none exists. K0_NONE
      // re-executes on any mutation, which is the answer the engine itself produces.
