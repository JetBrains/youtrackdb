...
/**
 * Recogniser for {@link PropertyMapStep} ({@code valueMap(keys…)} / {@code elementMap(…)}): one RETURN
 * column per map entry and {@link
 * com.jetbrains.youtrackdb.internal.core.gremlin.translator.step.BoundaryOutputType#MAP}. Bare {@code
 * valueMap()} (all properties) and {@code by(...)} modulators decline.
 */
final class PropertyMapStepRecogniser implements StepRecogniser {
  ...
  public Outcome recognize(StepCursor cursor, RecognitionContext ctx) {
    ...
    return GremlinProjectionAssembler.configurePropertyMap(
        ctx, mapStep.getPropertyKeys(), mapStep.getIncludedTokens());
  }
}
