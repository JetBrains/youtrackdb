...
public abstract class YTDBGraphImplAbstract implements YTDBGraphInternal, Consumer<Status> {
  ...
  private YTDBVertex createVertexWithClass(DatabaseSessionEmbedded sessionEmbedded, String label) {
    // Read the transaction's schema snapshot, including its private copy.
    // If the snapshot is null, read the transaction's schema proxy instead.
    // Create missing labels in a private copy when one exists.
    // Otherwise, keep the separate-session creation path.
    var snapshot = sessionEmbedded.getMetadata().getImmutableSchemaSnapshot();
    ...
    if (snapshot != null) {
      ...
      var vertexClass = sessionEmbedded.getSchema().getClass(label);
      exists = vertexClass != null;
      isVertexType = exists && vertexClass.isVertexType();
    }

    if (!exists) {
      if (sessionEmbedded.getTxSchemaState() != null) {
        var schema = sessionEmbedded.getSchema();
        schema.getOrCreateClass(label, schema.getClass(SchemaClass.VERTEX_CLASS_NAME));
      } else {
        executeSchemaCode(session -> {
          var schema = session.getSharedContext().getSchema();
          schema.getOrCreateClass(session, label, schema.getClass(SchemaClass.VERTEX_CLASS_NAME));
        });
      }
