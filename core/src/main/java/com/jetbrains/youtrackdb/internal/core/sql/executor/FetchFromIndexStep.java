...
public class FetchFromIndexStep extends AbstractExecutionStep {
  ...
  private static List<Stream<RawPair<Object, RID>>> multipleRange(
    ...
    for (var i = 0; i < secondValueCombinations.size(); i++) {
      ...
      if (index.supportsOrderedIterations()) {
        ...
        if (from == null && to == null) {
          // Both bounds are null -- fetch entries stored under a null key explicitly,
          // because the index between-scan API does not handle null bounds correctly.
          stream = getStreamForNullKey(session, index);
          if (acquiredStreams.add(stream)) {
            streams.add(stream);
          }
          ...
          if (from instanceof Collection<?> fromColl) {
            // When the key conversion produces collections (e.g. from multi-value index
            // definitions), each pair of (from[j], to[j]) defines a separate range scan.
            ...
            // Standard single-range scan between the from and to keys.
  ...
  /**
   * Returns {@code true} if the index is a built-in FULLTEXT index (not the Lucene variant).
   * Lucene indexes use a different query API and are handled elsewhere.
   */
  ...
  /**
   * Returns {@code true} if every sub-block in the AND condition is an equality
   * ({@code =}), CONTAINSKEY, CONTAINSVALUE, or IN expression -- i.e. no range
   * operators that would require an ordered index scan.
   */
