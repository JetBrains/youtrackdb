...
public abstract class YouTrackDBAbstract<R extends BasicResult, S extends BasicDatabaseSession<R, ?>>
  ...
  public void restore(@Nonnull String databaseName, @Nonnull String userName,
      @Nonnull String userPassword,
      @Nonnull String path,
      @javax.annotation.Nullable Configuration config) {
    if (config == null) {
      restore(databaseName, userName, userPassword, path, YouTrackDBConfig.defaultConfig());
    } else {
      var builder = YouTrackDBConfig.builder().fromApacheConfiguration(config);
      restore(databaseName, userName, userPassword, path, builder.build());
    }
  }
