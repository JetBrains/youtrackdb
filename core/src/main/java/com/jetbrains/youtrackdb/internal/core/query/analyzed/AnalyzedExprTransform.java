...
///
/// Each of the five `visitX` methods carries a recurse-into-children default: leaf variants
/// ([Var], [Const]) return themselves, and compound variants ([BinaryOp], [UnaryOp],
...
/// not reach. Because every `visitX` here has a pass-through default, adding a sixth IR variant
/// does not break a transform pass at compile time — a pass that needs special handling for
/// the new variant would default-recurse silently instead. Adding a variant therefore obliges
/// a manual audit of every transform pass to confirm the pass-through default is correct for
/// it. (This slice ships no transform pass, so the mechanical reflective backstop is a
/// later-slice obligation.)
