...
public class BTreeGetVisibleTest {
  ...
  // ---- Multiple versions, latest visible → returns correct RID ----

  @Test
  public void getVisible_multipleVersions_returnsLatestVisible() throws Exception {
    ...
    // (lower version) and returns RID_1 since it's visible (version < visibleVersion).
    atomicOperationsManager.executeInsideAtomicOperation(atomicOperation -> {
      tree.put(atomicOperation, new CompositeKey("Foo", 0L), RID_1);
    ...
    atomicOperationsManager.executeInsideAtomicOperation(atomicOperation -> {
      var result = tree.getVisible(prefixKey, snapshot, atomicOperation);
      assertThat(result)
          .as("Should return the first visible entry (lowest version)")
          .isNotNull();
      // Both versions are visible since both are committed; the scan finds the
      // first one in sorted order (v100 < v200), so should return RID_1.
      assertThat(result).isEqualTo(RID_1);
    });
  }
  ...
  @Test
  public void getVisible_afterCacheWarm_usesOptimisticPath() throws Exception {
    // After warming cache, the next getVisible() should use the optimistic path.
    // This test verifies correctness through the optimistic code path.
    atomicOperationsManager.executeInsideAtomicOperation(atomicOperation -> {
      tree.put(atomicOperation, new CompositeKey("Foo", 0L), RID_1);
    });
    ...
    atomicOperationsManager.executeInsideAtomicOperation(atomicOperation -> {
      var result = tree.getVisible(prefixKey, snapshot, atomicOperation);
      assertThat(result)
          .as("Optimistic path should return correct result")
          .isEqualTo(RID_1);
    });
  }
  ...
  public void getVisible_tombstoneThenLiveEntry_returnsLiveRid() throws Exception {
    // Version 0 is a tombstone, version 1 is a live entry.
    // The forward scan should skip the tombstone and return the live entry.
    atomicOperationsManager.executeInsideAtomicOperation(atomicOperation -> {
      tree.put(atomicOperation, new CompositeKey("Foo", 0L), TOMBSTONE_1);
      tree.put(atomicOperation, new CompositeKey("Foo", 1L), RID_2);
    });
