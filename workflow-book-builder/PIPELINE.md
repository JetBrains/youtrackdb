...
> 4. **Removed features.** If a chapter documents a workflow procedure that no longer exists, do not delete the chapter silently. Flag it in the drift file and ask the operator whether to remove it or replace it with the successor procedure. The book stays current-state: a removed procedure is cut or replaced, never narrated as history.
