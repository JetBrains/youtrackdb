...
> 3. Open the chapter with one paragraph that reminds the reader what earlier chapters gave them that this one builds on. Close with one paragraph that names what comes next and the question this chapter set up. When you refer to another chapter by number, write it as a relative Markdown link to that chapter's file (`[Chapter N](NN-slug.md)`), per the chapter-cross-reference convention in `BOOK_BRIEF.md`.
> 4. Draw diagrams per `DIAGRAMS.md`: every diagram is an inline Mermaid fenced block, placed where the prose leans on it and captioned below the closing fence. Each diagram carries one idea the prose leans on; if it restates the prose, drop one of them. Do not put a Markdown link inside a Mermaid node label.
