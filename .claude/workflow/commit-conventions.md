...
| **Workflow update** | Imperative summary of the workflow-file change (no special prefix; commit only touches paths under `_workflow/`, plus the one exception below for `workflow-issues/`) | `Add initial implementation plan and design`, `Phase A review and decomposition for histogram track`, `Record Phase B base commit for histogram track`, `Record episode for histogram leaf write step`, `Apply plan corrections from histogram-leaf review`, `Mark histogram-leaf track complete`, `Inline replan after Track 2`, `Record Phase C iteration 1 for histogram track`, `Record Phase C iteration failure for histogram track`, `Self-improvement reflection from <phase> of <adr-dir>` |
| **Self-improvement reflection** | A specialization of Workflow update — `Self-improvement reflection from <phase> of <adr-dir>` — produced by the end-of-session reflection step (see `self-improvement-reflection.md` §Commit format). It is the only Workflow update commit that touches paths outside `_workflow/` (it touches `workflow-issues/` at repo root, a branch-local pending-triage buffer; the implementer empties the directory before merging the PR — files are not intended to reach `develop`). For resume / orphan-detection purposes (`step-implementation-recovery.md`), treat any commit that touches only `_workflow/` **or** `workflow-issues/` as a Workflow update. | `Self-improvement reflection from phase-b of histogram` |
