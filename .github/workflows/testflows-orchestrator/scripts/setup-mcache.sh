set -x
{
  if [ -d "/mnt/cache" ]; then
          ...
          sudo mkdir -p "$MAVEN_ARTIFACTS" "${UBUNTU_HOME}/.m2/repository"

          # Bind mount volume-backed directories
          sudo mount --bind "$MAVEN_ARTIFACTS" "${UBUNTU_HOME}/.m2/repository"

          # Ensure ubuntu user owns the directory
          sudo chown -R ubuntu:ubuntu "${UBUNTU_HOME}/.m2"
