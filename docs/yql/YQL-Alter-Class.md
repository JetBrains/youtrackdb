...
- Remove all superclass assignments:

```sql
ALTER CLASS Employee SUPERCLASSES NULL
```
...
| `SUPERCLASSES` | Identifier \[, Identifier\]* | Replaces the superclasses of the class. Supports multiple inheritance via a comma-separated list. Use `NULL` to remove all superclass assignments. |
| `STRICT_MODE` | Boolean | Enables or disables strict mode. When in strict mode, you work in schema-full mode and cannot add new properties to a record if they are not part of the class' schema definition. |
| `CUSTOM` | | Defines custom properties. Property names and values must follow the syntax `<property-name>=<value>`. The attribute name is an identifier. The value is a string, so it has to be quoted with single or double quotes. |
