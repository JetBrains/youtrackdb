...
You arrive here with the implement-test-commit loop from [Chapter 10](10-phase-b-implement-test-commit.md) in your head. You saw that after the implementer commits a step, a `risk: high` step triggers a review before the episode is written; `medium` and `low` steps skip it and lean on the always-on track-level pass instead. [Chapter 10](10-phase-b-implement-test-commit.md) named that review and moved on. This chapter opens it up. Everything below applies identically whether the diff under review is one step or one track — [Chapter 12](12-phase-c-track-review-completion.md) runs it at track scope and uses it to close a track, but the mechanism is the same, so we teach it once, in the neutral.
...
[Chapter 10](10-phase-b-implement-test-commit.md) showed you where the step-level run sits inside the implement-test-commit loop. The track-level run is the one that still owes you a chapter. [Chapter 12](12-phase-c-track-review-completion.md) takes this machine to Phase C: it runs the dimensional review against the whole track's diff, wraps it in a conversational approval loop where you can steer which findings get fixed, delegates the fixes to a fresh implementer, and once the review passes writes the completion episode and closes the track. The question this chapter set up, and [Chapter 12](12-phase-c-track-review-completion.md) answers, is how a reviewed track gets signed off and becomes done.
