...
What the plan does carry is deliberately thin. It has a `## Checklist` that lists the tracks in order, each with a one-paragraph intro and a scope line, and a thin cross-track *Component Map* that shows the slice of the system the whole change touches. That is the cross-track view: enough for an execution session to see the shape of the work and assess how a change in one track ripples into another, without duplicating the per-track detail. The goals, the constraints, and the full architecture notes live in the track files, the research log, or the pull-request description, not in the plan.
...
Everything above is the `full`-tier path: a frozen design, a separate derivation session, a multi-track plan, track files seeded from the design. [Chapter 3](03-tiers-and-the-tier-gate.md) taught that the tier decides how much of this you run, and the two lighter tiers shed parts of it.
