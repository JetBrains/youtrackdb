...
those Java objects; the *planner* is the code that walks them. Three sentences is all the theory
you need. The rest of this chapter is concrete.

## A grammar rule and the class it produces

The grammar lives in
`core/src/main/grammar/YouTrackDBSql.jjt`. At line 1245 you find the rule that handles the
`MATCH` keyword:
...
`jjtThis.addMatchExpression(...)`, accumulating chain after chain into a list. Every optional
clause — `GROUP BY`, `ORDER BY`, and the rest — assigns directly to a field of the same name on
`jjtThis`.
...
`core/src/main/grammar/YouTrackDBSql.jjt:1173`. When it sees the `MATCH` keyword it delegates to
`MatchStatement()` at line 1184.
...
`SQLExpression`, and appends it to `returnItems`.
...
- `SQLMatchExpression` — one pattern chain. Owns an `origin` filter and a list of path items.
...
- **`pattern` is null at this point.** `SQLMatchStatement` has a field called `pattern`
  ...
  the planner calls its own private `MatchExecutionPlanner.buildPatterns(CommandContext)` at line
  4378, which populates a `Pattern` field on the planner itself (not on the AST). At the end of
  parsing, only syntax is present.
...
The parser has done its job. It has turned the SQL text into an `SQLMatchStatement` — a tree of
