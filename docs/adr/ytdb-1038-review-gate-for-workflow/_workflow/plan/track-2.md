...
- Track 2 cumulative diff carries three whole-file-add staged copies (`conventions.md`, `step-implementation.md`, `track-code-review.md`), each a §1.7(e) first-touch copy. Phase C's track-level review must hand-inject the Track 4 / D5 delta-scoping (diff each staged copy against its live counterpart and scope findings to the delta), because Track 4 is not yet implemented and the §1.7(h) carve-out leaves this branch's own reviews on the unfixed live machinery. See Episodes §Step 2.
- Track 2's cumulative diff (base 81bc15de39..HEAD) carries ten whole-file-add staged copies after Steps 1-4 (the §1.7(d) `conventions.md` plus the nine caveat sites), refining the three-copy count noted above. S3 is orchestrator-verified: the caveat sentence is byte-identical (md5 a0dda603) across all nine sites. Phase C still needs the Track 4 / D5 delta-scoping hand-injection per §1.7(h). See Episodes §Step 4.
...
### Step 2 — commit 704d847, 2026-06-02T04:29Z [ctx=info]
**What was done:** Copied the live `step-implementation.md` and `track-code-review.md` into the staged subtree verbatim (first touch, §1.7(e)), then added a parallel "## Staged-read precedence (workflow-modifying plans)" caveat block to each block's fenced prompt body. Placed it at the same logical anchor in both: immediately after the shared `## Tooling` section, before the next section (`## Changed Files` in step-implementation, `## Diff` in track-code-review). The caveat self-gates on the §1.7(b) marker and routes reads through §1.7(d) precedence (staged copy when present, else live), with a one-sentence rationale that reading live would compare against develop's version of an already-rewritten rule and report a phantom mismatch. Orchestrator-verified the caveat block is byte-identical across the two staged files after de-indent (S2 parity; S3 uniformity for the caveat sentence). Docs-only: no tests, no Spotless.
**What was discovered:** The two context blocks share a `## Tooling` section but differ in section order (step-implementation runs Tooling, Changed Files, Diff; track-code-review runs Changed Files, Skip, Tooling, Diff), so "after ## Tooling" is the unambiguous shared logical anchor the plan prescribes, not an absolute line. Both staged files are whole-file adds (first touch), so Track 2's cumulative diff now carries three whole-file-add staged copies (this pair plus Step 1's `conventions.md`). That is the Track 4 / D5 scenario: the cumulative diff hides the real per-file delta.
**Key files:** `docs/adr/ytdb-1038-review-gate-for-workflow/_workflow/staged-workflow/.claude/workflow/step-implementation.md` and `.../track-code-review.md` (new staged copies; parallel caveat block added).
**Critical context:** Steps 3-4 mirror the one-line caveat sentence into the seven other prompts and verify S3 byte-uniformity across all nine sites. The canonical sentence is the one now in these two blocks: "When the plan's `### Constraints` carries the `§1.7(b)` marker, resolve every read of a `.claude/workflow/**` or `.claude/skills/**` file through `§1.7(d)`, taking the staged copy under `_workflow/staged-workflow/` when present and the live file otherwise." The two context blocks additionally carry the section heading and the phantom-mismatch rationale sentence, which are block-only and not part of the S3 byte-uniform caveat sentence.

### Step 3 — commit 0be6c2e, 2026-06-02T04:34Z [ctx=info]
**What was done:** Copied the four live Phase A track-review prompts (`technical-review.md`, `risk-review.md`, `adversarial-review.md`, `review-gate-verification.md` under `.claude/workflow/prompts/`) into the staged subtree verbatim (first touch, §1.7(e)), then added the marker-gated one-line caveat to each as a bolded `**Staged-read precedence (workflow-modifying plans):**` line immediately after the prompt's `Inputs:` block. Orchestrator-verified the four prompt caveat lines share one md5 (byte-identical to each other and to the canonical Step 2 sentence). Docs-only: no tests, no Spotless.
**What was discovered:** All four prompts share an identical Inputs-block shape (a bullet list ending in `{previous_findings}`, then the first instruction paragraph; `review-gate-verification.md`'s list is longer), so "directly after the Inputs block" is a uniform anchor across all four. That makes Step 4's placement in the gate-check and Phase 2 prompts predictable, and is a candidate anchor for Track 3's addendum if it rides next to the Inputs block too.
**Key files:** the four staged `.claude/workflow/prompts/technical-review.md`, `risk-review.md`, `adversarial-review.md`, `review-gate-verification.md` (new staged copies; one-line caveat added).
**Critical context:** Step 4 mirrors the same byte-identical sentence into `dimensional-review-gate-check.md`, `consistency-review.md`, and `structural-review.md`, then verifies S3 byte-uniformity across all nine sites (the two context blocks need de-indent + de-wrap before comparison; the seven one-line mirrors compare directly). Track 3's dependency (caveat present in technical/risk/adversarial) is now satisfied.

### Step 4 — commit 11f2367, 2026-06-02T04:41Z [ctx=info]
**What was done:** Copied the gate-check and two Phase 2 plan-review prompts (`dimensional-review-gate-check.md`, `consistency-review.md`, `structural-review.md` under `.claude/workflow/prompts/`) into the staged subtree verbatim (first touch, §1.7(e)), then mirrored the one-line caveat into each after its `Inputs:` block. Targeted `prompts/structural-review.md` (the sub-agent prompt), not `.claude/workflow/structural-review.md` (the orchestration driver of the same name). Orchestrator-verified S3: the canonical caveat sentence is byte-identical (one md5) across all nine staged sites. Docs-only: no tests, no Spotless.
**What was discovered:** S3 holds across all nine sites. The caveat sentence ("When the plan's `### Constraints` carries the `§1.7(b)` marker, resolve every read … taking the staged copy under `_workflow/staged-workflow/` when present and the live file otherwise.") is byte-uniform; the per-site rendering differs as the plan expects: the seven prompts carry it on one line behind a bold `**Staged-read precedence …:**` label, the two context blocks under a `##` heading with two extra block-only sentences (the inert-without-the-marker note and the phantom-mismatch rationale). Those extras are not part of the S3-uniform sentence.
**Key files:** the three staged `.claude/workflow/prompts/dimensional-review-gate-check.md`, `consistency-review.md`, `structural-review.md` (new staged copies; one-line caveat added).
**Critical context:** The nine-site caveat rollout is complete and S3-uniform. Track 2's cumulative diff (base 81bc15de39..HEAD) carries ten whole-file-add staged copies (the §1.7(d) `conventions.md` plus the nine caveat sites), so Phase C's track-level review must hand-inject the Track 4 / D5 delta-scoping (diff each staged copy against its live counterpart, scope findings to the delta), because Track 4 is not yet implemented and the §1.7(h) carve-out leaves this branch's own reviews on the unfixed live machinery. Phase C's `review-workflow-consistency` re-checks S2 (the two context blocks) and S3 (all nine).
