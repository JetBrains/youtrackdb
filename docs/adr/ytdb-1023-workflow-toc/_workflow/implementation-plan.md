...
## Plan Review
- [x] Plan review (consistency + structural) — passed at iteration 1 (D15 re-validation, 2026-05-29)

**Auto-fixed (mechanical)**: CR1 — corrected D15's affected-ref enumeration (the three refs inside the workflow-root `structural-review.md` target the `prompts/` prompt, not the workflow-root file; the real converted workflow-root refs are `planning.md` ×2, `review-iteration.md`, `conventions.md`, `implementation-review.md`), at 5 plan sites including the Step 4 acceptance criterion. S1 — trimmed invariant I7 to its enforceable rule (it had accreted D13/D14/D15 clauses past the ~5-line invariant budget; provenance tags retained, per-clause derivation lives in the DRs).
