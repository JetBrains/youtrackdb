...
- **Reinforced by D17**: D6's "agent files get the cross-file suffix and bootstrap, not the TOC / per-section annotations" split is exactly what D17 teaches the script — D17 scopes live agent files into rules 6 and 7 only, never rules 2/3/4. D6's refs-only-plus-bootstrap scope is unchanged.
...
- **Note (D18)**: Track 5's Phase A reviews found the two-branch verification is unrunnable before this plan reaches develop. D18 reframes the real replay as a post-merge acceptance step and limits Track 5's in-branch work to a static premise check plus local machinery / `--check` / telemetry smokes. D7's no-op-replay core is unchanged.
...
#### D17: `workflow-reindex.py` scopes live `.claude/agents/**/*.md` into rules 6 and 7 (sixth reindex reopen)

- **Alternatives considered**: stage the agent files so the existing staged-agents glob validates them (contradicts §1.7's "agents not stageable" and D6 / design §"Migration replay semantics", which route `.claude/agents/**` as a live refs sweep — would force reopening the staging convention); accept no `--check` backstop for agent files and rely on hand-discipline (drops Validation's "every agent ref carries the suffix" and "`--check` exits 0 including agent files" and the inter-track dependency on Track 2's rule 6/7, leaving the enforcement claim hollow).
- **Rationale**: Track 5's Phase A reviews (technical, risk, adversarial — all three) found that `workflow-reindex.py`'s `IN_SCOPE_GLOBS` carries only a *staged* agents glob (`docs/adr/*/_workflow/staged-workflow/.claude/agents/**/*.md`) and no live `.claude/agents/**/*.md` glob, while agents are modified live per §1.7. The `validate` and `--write` loops iterate only the discovered in-scope set, so rules 6 (cross-file ref suffix subset) and 7 (bootstrap presence) never fire on the 20 live agent files — verified empirically: `--check --files <live-agent>` exits 0 with zero findings, and an unscoped `--check` reports zero findings under `.claude/agents/`. The staged glob is dead (no staged agents exist; §1.7 forbids staging them). The fix adds the live `.claude/agents/**/*.md` path to the citing-file scope for rules 6 and 7 **only** — NOT rules 1/2/3/4/5/8, because D6 keeps agent files refs-only (no TOC, no per-section annotations) and they carry no `_workflow/**` workflow-sha stamp (§1.6(f)). Same class as D14/D15/D16: a script-resolution / discovery-scope gap one layer below the text work, where the plan and convention commit to the behavior but the script lacks the scope. The dead staged-agents glob is removed or left inert per the implementer's judgment; a per-rule applicability gate keeps rules 2/3/4 from demanding a TOC on agent files.
- **Risks/Caveats**: sixth reopen of `workflow-reindex.py` (the Track 4 fence-exclusion, staged-aware lookup, bare-basename collision, and SKILL-key steps, plus Track 2's original author, were the prior five). Forward-safe: on `develop` and non-workflow-modifying branches the live agent files carry their suffixes plus bootstrap post-promotion, so the new scope finds them already compliant; the rules-6/7-only gate means no agent file is ever flagged for a missing TOC. Landed as a HIGH-risk script-fix step ahead of the agent bootstrap and refs-sweep steps, mirroring the Track 4 reopens.
- **Implemented in**: Track 5 (new HIGH-risk script-fix step ahead of the agent sweep)
- **Full design**: design.md §"Reindex script" → §"Discovery mechanism" reconciliation deferred to Phase 4 `design-final.md` (joins the deferred-drift basket alongside the D13 cross-ref-convention narrative, the D14/D15/D16 staged-resolution drift, and the Track 3 discovery-glob drift); conventions.md §1.8 (the schema rules 6/7 enforce), §1.7(e) (the agents-live write-routing this scope aligns with), D6 (agent files refs-only).

#### D18: Migration replay verification is a post-merge acceptance step; Track 5's in-branch work is a local dry-run plus a static premise check

- **Alternatives considered**: keep the original Steps 5-6 ("rebase candidate branches onto develop, run `/migrate-workflow`") as in-track work (rejected by all three Phase A reviews — this plan reaches develop only at the Phase-4 squash-merge, so a candidate rebased onto current develop gains zero workflow-format commits from this plan and the replay exercises nothing this plan introduced; it also destructively rewrites other people's in-flight branches and cannot run inline, since `/migrate-workflow` requires a fresh session); simulate by rebasing the candidates onto this branch's own HEAD (approximates post-merge develop but still rebases real branches in fresh sessions and conflates the simulation with the real gate).
- **What changed**: Track 5's Phase A reviews found the two-branch acceptance verification is structurally unrunnable before this plan lands on develop. The drift gate ranges `git log $BASE_SHA..HEAD` over the candidate branch's own HEAD and never fetches develop; this plan's §1.8 plus workflow-sha bump reaches develop only after Phase 4 (`develop` was 89 commits behind with no §1.8 at Track-5 time). D7's `**Implemented in**: Track 5` over-claimed: Track 5 cannot close the two-branch acceptance gate.
- **Revised decision**: the real two-branch `/migrate-workflow` replay is a **post-merge acceptance procedure** — run by the user in a fresh session after this plan squash-merges to develop, on candidate branches rebased onto post-plan develop, recorded in the Track 5 completion episode and the Phase 4 `adr.md`. Track 5's in-branch verification step is limited to runnable substitutes: a static check that D7's "no `_workflow/**` artifact-shape change" premise holds (the plan's commits touch `.claude/...` only, never the branch's own `_workflow/**`), an optional local `/migrate-workflow` machinery smoke, the staged-scoped plus live-agent `workflow-reindex.py --check` green (rules 2/3/4/5/6/7/8), and the `measure-read-share.py` smoke.
- **Rationale**: D7's core (migration replay is a content no-op for this plan; the workflow-sha bump is what other branches' drift gates pick up) is unchanged. design.md §"Migration replay semantics" already frames verification as "rebased onto post-plan develop", so the design needs no mutation; only the track file's Step 5 "after this plan's previous tracks land" wording was wrong. Splitting the runnable-now substitute from the post-merge real gate keeps the acceptance honest.
...
- I5 — Every in-scope system-prompt file (7 SKILL.md, 11 `.claude/workflow/prompts/*.md`, 20 `.claude/agents/*.md`) carries the bootstrap block (literal heading `## Reading workflow files (TOC protocol)`) at the top, between frontmatter and main body. The reindex script's rule 7 enforces presence; rule 7 fires on the 20 live agent files only after the D17 agent-scope script fix (the discovery scope omitted live `.claude/agents/**` before D17).
...
- [x] Track 4: Universal annotation rollout + cross-ref link conversion (49 files)
  ...
  >
  > **Strategy refresh:** CONTINUE — Track 4's four `workflow-reindex.py` reopens (D14/D15/D16 staged-aware lookup) unblock Track 5's cross-file ref resolution to staged workflow-doc and `<dir>/SKILL.md` targets rather than contradicting any Track 5 assumption. No plan rewrites needed. Three Track-4 cross-track signals fold into Track 5 decomposition: M12 (bootstrap placement covers all three anchor shapes, not the two-shape glossary/I5 wording), the H4-union exclusion (a citer claiming an H4-only token fails rule_6 subset), and the cross-file-with-subsection form (whole-file ref plus a separate backticked `§X.Y`).

- [ ] Track 5: Bootstrap block + agent files refs sweep + migration verification
  > Land a HIGH-risk `workflow-reindex.py` agent-scope fix (D17) so rules 6/7 validate the 20 live agent files, then insert the bootstrap protocol block (~30 lines, body sourced from `design.md §"Bootstrap protocol for agent system prompts"`) at the TOC anchor of 38 system prompts (7 SKILL.md, 11 `.claude/workflow/prompts/*.md`, 20 `.claude/agents/*.md`), and apply the `name:roles:phases` cross-file suffix to outgoing workflow-doc refs in the 20 agent files (the 7 SKILL.md read-lists were largely converted by Track 4's D13 — Track 5 verifies and gap-fills). The real two-branch `/migrate-workflow` replay is a post-merge acceptance procedure (D18); Track 5's in-branch verification is a static D7-premise check plus local `--check` and telemetry smokes.
  > **Scope:** ~6 steps covering the D17 reindex agent-scope fix (HIGH-risk), bootstrap insertion (38 files, batched SKILL / prompts / agents), the agent refs sweep (HIGH-risk; depends on D17), SKILL.md read-list suffix verify and gap-fill, and the in-branch local migration dry-run plus final validation. The real two-branch migration verification is a documented post-merge acceptance step (D18), not an in-track commit.
