...
- Steps 2-3 split the 31 workflow-root files by heading count, not file count (`conventions.md` alone carries ~54 headings), so each batch is a comparable hand-authoring and review unit. Step 2's annotations stand; its link-conversion is deferred to Step 6 (Step 3 onward folds conversion in).
- Step 4 covers the 11 prompts, including the already-staged `create-final-design.md`, with link conversion folded in.
- Step 5 covers the 7 skill files (after-frontmatter TOC for the 5 H1-less ones; no TOC for the 2 with no real headings), with link conversion folded in.
- Step 6 runs the deferred link-conversion over Step 2's 9 batch-1 files, then the full-set `--check` green run plus the house-style sweep on every `summary="..."` and the prose around converted refs.
...
3. Annotate workflow-root batch 2 (the remaining 22 root files) AND convert this batch's Markdown links to in-scope `.md` targets into bare `name.md:roles:phases` cross-file refs, backtick-wrapping references to non-annotatable targets (D13). Same per-file procedure (hand-place TOC delimiters → per-section annotations → `--write` → `--check --files`), with the link-conversion folded in before `--write`. — risk: medium (annotation semantic accuracy + cross-file suffix/backtick-wrap judgment; Phase C focal point)  [ ]
4. Annotate the 11 prompts under `.claude/workflow/prompts/` (including staged `create-final-design.md`, whose fenced `adr.md`-template headings Step 1's fence-exclusion skips) AND convert their links per D13. Same procedure. — risk: medium (annotation semantic accuracy + cross-file conversion; Phase C focal point)  [ ]
