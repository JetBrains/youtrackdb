...
- Agent files out of `IN_SCOPE_GLOBS` (live and staged-agent paths): live `.claude/agents/*.md` paths are explicitly NOT in the schema validator's discovery set. Only staged agent paths under `docs/adr/*/_workflow/staged-workflow/.claude/agents/**/*.md` participate in the walk. The pre-commit hook routes agent edits through `--check --files` but the script silently drops live agent paths per A6. This is consistent with D6 (agents get the bootstrap block and the `:roles:phases` refs sweep, not per-section TOC annotations). Phase C reviewers reading this track's diff should not expect WB findings against the six agent files even though Step 5 modified them. See Episodes §Step 5.
- Rule 5e (malformed annotation comment) is currently a defensive branch unreachable through the public parsing API: the annotation regex rejects multi-line and unclosed shapes before they reach the `well_formed=False` path. Documented in the test file's commentary; design intent preserved. Phase C reviewers may flag the absence of a focused rule_5e unit test — this is by-construction unreachable code, not missing coverage. See Episodes §Step 5.
- Per-agent prefix family scope: the new `WB / HS / PD / IC / WS / CN` per-finding prefixes (from Step 5) live at the raw-agent-output layer. The orchestrator-side synthesis layer (review-iteration.md table) still uses `M<n>` for the synthesised `findings:` input the implementer consumes. Phase C of Track 2 (next session) will be the first dimensional review fan-out exercising the new prefix families against a real diff. See Episodes §Step 5.
...
### Step 5 — commit 1865bcf7cc, 2026-05-28T15:08Z [ctx=info]
**What was done:** Six `.claude/agents/review-workflow-*.md` agents each gained a per-finding numeric prefix under their existing `#### Critical` / `#### Recommended` / `#### Minor` headings: `WB` (context-budget), `HS` (hook-safety), `PD` (prompt-design), `IC` (instruction-completeness), `WS` (writing-style), `CN` (consistency). Placement and shape pin to a single bullet of the form `**<PFX><N>** — File: ..., Axis: ..., Cost: ..., Issue: ..., Suggestion: ...` with numeric IDs sequencing across severities rather than restarting per H4. The change is template-bound; each agent's axes and evaluation criteria are unchanged. The context-budget agent additionally invokes `python3 .claude/scripts/workflow-reindex.py --check` during workflow-machinery review (new "Workflow-reindex script integration" section) and folds script findings into one consecutive `WB<N>` sequence, with rules 1/2/3/4/7 mapping to Critical and rule 5 sub-checks / rule 6 / rule 8 mapping to Recommended. The test matrix grew from 75 to 80 cases: three new rule_1 stamp-enforcement tests against staged `docs/adr/.../staged-workflow/.claude/...` paths, one rule_6 both-any wildcard positive test, and one `--write` halts-on-mixed-stale-and-unresolved test that asserts the stale ref stays byte-for-byte on disk after the halt.

**What was discovered:** Live `.claude/agents/*.md` paths are NOT in `IN_SCOPE_GLOBS`; only staged `docs/adr/*/_workflow/staged-workflow/.claude/agents/**/*.md` paths are. The pre-commit hook still routes agent edits through `workflow-reindex.py --check --files`, but the script silently drops them per the A6 out-of-scope `--files` rule (consistent with D6: agent files get the bootstrap block and the refs sweep, not per-section annotations). Rule 5e is currently a defensive branch: the annotation regex `^<!--\s*(.+?)\s*-->\s*$` rejects multi-line and unclosed shapes before they reach the `well_formed=False` path, so a focused unit test for rule_5e is not constructable through the public parsing API. Documented in the test file's commentary; no behavior change needed. See Episodes §Step 5.
...
**Critical context:** The per-agent prefix families (`WB`, `HS`, `PD`, `IC`, `WS`, `CN`) live at the raw-agent-output layer. The orchestrator-side synthesis layer (review-iteration.md table) still uses `M<n>` for the synthesised `findings:` input passed to the implementer per the rulebook. Phase C of Track 2 (the next session) will be the first round of dimensional reviews emitting the new prefix families against this track's cumulative diff.
