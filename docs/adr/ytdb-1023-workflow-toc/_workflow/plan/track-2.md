...
2. **Validation rules and `--check` mode.** Implement every check from `design.md §"Reindex script" > Validation rules`: stamp present, TOC present, TOC matches annotations, annotation present after `##` headings, field well-formedness, enum-token validation (script reads enums from `conventions.md §1.8`), cross-reference suffix on `CLAUDE.md` / agent file refs. Exit codes per design.
3. **`--write` mode.** TOC region rebuild from current annotations. Idempotent. Does not modify annotations. Tested for idempotence by running `--write` twice and asserting no diff after first run.
