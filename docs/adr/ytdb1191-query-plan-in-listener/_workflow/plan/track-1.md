...
- D2's `limit(0)` example was wrong: a downstream `limit(0)` does *not* leave the plan `null`. The `YTDBGraphStep` source supplier runs its query on iteration before `RangeGlobalStep` short-circuits, so a non-null scan plan is captured. Found during Phase C review; D2's Risks/Caveats corrected and pinned by test `downstreamLimitZeroStillCapturesSourcePlan`.
...
- **Risks/Caveats**: The plan is `null` in two cases: when the source step never runs (the by-id branch, or a non-`V()`/`E()`-rooted root traversal per D3), and on a cache-hit replay where the source step *does* run but re-serves cached rows whose plan was already nulled at cache-entry close (see D5). **Correction (found during Phase C review, test `downstreamLimitZeroStillCapturesSourcePlan`):** a downstream `limit(0)` does *not* leave the plan `null` — the `YTDBGraphStep` source supplier executes its query as soon as the traversal is iterated, before `RangeGlobalStep` can short-circuit, so a non-null scan plan is still captured. The earlier claim that `limit(0)` produces a `null` plan was wrong for this fork's eager-source execution model; the regression test pins the actual behavior.
