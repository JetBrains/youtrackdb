...
- **Risks/Caveats**: A cache-hit replay surfaces `null` — see D5. The plan is `null` only when the source step genuinely never runs (the by-id branch, or a non-`V()`/`E()`-rooted root traversal per D3). **Correction (found during Phase C review, test `downstreamLimitZeroStillCapturesSourcePlan`):** a downstream `limit(0)` does *not* leave the plan `null` — the `YTDBGraphStep` source supplier executes its query as soon as the traversal is iterated, before `RangeGlobalStep` can short-circuit, so a non-null scan plan is still captured. The earlier claim that `limit(0)` produces a `null` plan was wrong for this fork's eager-source execution model; the regression test pins the actual behavior.
