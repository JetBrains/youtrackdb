...
- [x] Track-level code review
  ...
  - [x] 2026-05-24T16:31Z [ctx=info] Track-level code review iteration 2 complete (2/3 iterations)
- [x] 2026-05-24T16:34Z [ctx=info] Track complete
- [ ] Track completion
...
- The pre-`endTxCommit` `IOException` failure path leaves the storage in error mode today: `rollback(error, op)` routes through `endAtomicOperation(op, error)` and `moveToErrorStateIfNeeded(error)` lands `setInError` for any non-`HighLevelException` `RuntimeException`. Tests wired around this path must assert counter lockstep but cannot pin a `!isInError()` invariant on the failure leg, and cannot exercise a follow-up "recovery commit after failure" because the storage refuses new commits once in-error. The initial draft of `MainCommitCounterSyncTest.preEndTxCommitIOException_countersStayAtPreTxValues` had to drop its `!isInError()` assertion for this reason. Material for follow-up tracks' regression-test design. See Episodes §Step 4.
- Ephemeral-identifier residue at `AbstractStorage:6421`, `:6429`, and `:6681` carries pre-existing workflow-marker citations from older Phase B episodes; outside Step 4's diff scope. Phase C track-level review or the Phase 4 design-final sweep can address it. See Episodes §Step 4.
- `MainCommitCounterSyncTest` uses the in-memory storage engine (`DatabaseType.MEMORY`); persisted-side EP-page reads are not directly asserted because `BTreeMultiValueIndexEngine` does not expose accessors for the underlying `svTree`/`nullTree` persisted counters. The public engine surface is `getTotalCount` / `getNullCount`; the restart-cycle pattern from `BTreeEnginePersistedCountIT` is the technique for direct persisted-EP-page assertions if a follow-up reviewer wants them. See Episodes §Step 4.
...
### Step 4 — commit 21fe1a5c45, 2026-05-24T13:49Z [ctx=safe]
**What was done:** Deleted the three inline counter-sync calls (`persistIndexCountDeltas`, `applyIndexCountDeltas`, `applyHistogramDeltas`) and their two post-`endTxCommit` log-and-swallow catches from `AbstractStorage.commit`. The pre-`endTxCommit` catch (Track 1's broadened `IOException | RuntimeException | AssertionError` shape) stays as the sole catch around the inner try. Optional A8 picked up: broadened the `cleanupSnapshotIndex` catch from `RuntimeException` to `RuntimeException | AssertionError` for symmetry with the pre-`endTxCommit` catch. Swept doc-comment prose-residue that named the deleted call sites in five files: the "legacy inline call" anchor in Hook A's source comment, the "inline persist call (until it is deleted)" Javadoc on `EndAtomicOperationPersistHookTest.emptyHolderHasNoDeltasAndIsNotPersisted`, the "endTxCommit followed by applyIndexCountDeltas" routing comment in `CommitNonRuntimeExceptionFallbackTest`, and the "inline call inside commit()" idempotency-latch comments on both `applyIndexCountDeltas` and `applyHistogramDeltas`. Added two regression tests under `core/.../storage/impl/local/`. `MainCommitCounterSyncTest` exercises the consolidated path end-to-end: a nominal multi-value commit with one null key, plus a wrapper-bypassing failure transaction asserting counter lockstep on both paths and no escaping `AssertionError`. `ClearIndexApiRollbackTest` is `@Ignore`-annotated scaffolding for the follow-up `clear()` pure-delta encoding work; the body compiles against the current API surface so the follow-up's regression-test step only needs to flip the annotation.

**What was discovered:** The pre-`endTxCommit` `IOException` failure path leaves the storage in error mode today via `rollback(error, op)` → `endAtomicOperation(op, error)` → `moveToErrorStateIfNeeded(error)` → `setInError(error)` for any non-`HighLevelException` `RuntimeException`. The initial draft of `MainCommitCounterSyncTest.preEndTxCommitIOException_countersStayAtPreTxValues` asserted `!isInError()` and failed; the rewrite drops that assertion and documents the rationale inline. The contract under test is counter lockstep, not the in-error transition; a sanity-pin "recovery commit after failure" assertion is impossible on this path because the storage refuses new commits once in-error.

Cross-track signals captured from `CROSS_TRACK_HINTS`: (a) the follow-up tracks consuming the consolidated lifecycle gate get a clean `endAtomicOperation` with no inline counter-sync call surviving anywhere in `AbstractStorage.commit`; `ClearIndexApiRollbackTest`'s `@Ignore` flip is the only edit the follow-up's regression-test step needs to make. (b) Ephemeral-identifier residue at `AbstractStorage:6421`, `:6429`, and `:6681` carries pre-existing workflow-marker citations from older Phase B episodes; outside Step 4's diff scope, deferrable to Phase C track-level review or the Phase 4 design-final sweep. (c) `MainCommitCounterSyncTest` uses `DatabaseType.MEMORY`; persisted-side EP-page reads are not directly asserted because `BTreeMultiValueIndexEngine` does not expose accessors for the underlying `svTree`/`nullTree` counters. `getTotalCount` / `getNullCount` are the public engine surface; the `BTreeEnginePersistedCountIT` restart-cycle pattern is the technique for direct EP-page assertions if a follow-up reviewer wants them.

**What changed from the plan:** Optional A8 (broaden `cleanupSnapshotIndex` catch) was applied for symmetry with the pre-`endTxCommit` broadened shape. Optional A5(ii) (interim reflective `IndexCountDelta` injection in the `clearIndex` API path) was deliberately skipped: the work surface stayed narrow and the `@Ignore`-d `ClearIndexApiRollbackTest` plus the follow-up plan-of-work already cover the same gap. Sweep scope matched the A11 inventory plus four additional prose-residue sites surfaced by reading each comment block end-to-end rather than relying on grep alone.

**Key files:**
- `core/src/main/java/com/jetbrains/youtrackdb/internal/core/storage/impl/local/AbstractStorage.java` (modified)
- `core/src/main/java/com/jetbrains/youtrackdb/internal/core/storage/impl/local/paginated/atomicoperations/AtomicOperationsManager.java` (modified)
- `core/src/test/java/com/jetbrains/youtrackdb/internal/core/storage/impl/local/CommitNonRuntimeExceptionFallbackTest.java` (modified)
- `core/src/test/java/com/jetbrains/youtrackdb/internal/core/storage/impl/local/paginated/atomicoperations/EndAtomicOperationPersistHookTest.java` (modified)
- `core/src/test/java/com/jetbrains/youtrackdb/internal/core/storage/impl/local/ClearIndexApiRollbackTest.java` (new)
- `core/src/test/java/com/jetbrains/youtrackdb/internal/core/storage/impl/local/MainCommitCounterSyncTest.java` (new)

**Critical context:** `AbstractStorage.applyIndexCountDeltas` and `applyHistogramDeltas` still carry the `holder.setApplied()` latch hoist and the matching latch read. Both Javadocs were rewritten to describe the latch as a defensive belt against future re-entry (a nested or mistakenly-replayed lifecycle pass) rather than as a window-closer for the legacy inline call. The latch is no longer load-bearing for the legacy-inline-call race but remains useful regression protection; follow-up reviewers entering the apply layer can treat the latch as optional defense-in-depth rather than a correctness requirement.
