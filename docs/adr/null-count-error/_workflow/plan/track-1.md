...
### Step 6 — commit 7468b6e76d, 2026-05-22T20:54Z [ctx=info]
**What was done:** Added `StorageCascadeContainmentOnNullCountUnderflowTest` in package `com.jetbrains.youtrackdb.internal.core.storage.impl.local`. The test opens a fresh in-memory storage, creates a UNIQUE single-value index on a string property, seeds one row so the SV engine is fully instantiated and `load()`-ed, then runs a transaction that puts a second row and reflectively overwrites `IndexCountDelta.nullDelta` with `Long.MIN_VALUE + 1` before `db.commit()`. The commit's `applyIndexCountDeltas` branch on the success path calls `BTreeSingleValueIndexEngine.addToApproximateNullCount(Long.MIN_VALUE + 1)`; the engine's `reportAndClampUnderflow` helper logs a SEVERE record with the engine name and id, clamps the counter to 0 via CAS, and `db.commit()` returns normally. The test asserts: commit does not throw, `storage.isInError()` stays `false` (read directly via the same-package `protected` accessor), a follow-up begin/put/commit cycle on the same storage succeeds, and exactly one SEVERE record from the engine's logger names the engine and the observed delta. The capture goes through `BTreeEngineTestFixtures.captureSevereOn` (the Step 5 Review fix helper).

**What was discovered:** `IndexCountDelta.accumulate` add-assigns into the holder; the reflective overwrite of `nullDelta` has to land between vertex creation and `db.commit()`, because the commit-time put would otherwise add its own delta on top and shift the magnitude. The same `0x7FFFFFF` external-id mask documented in Surprises §"engineId bit-encoding" applies for looking up the per-engine delta in the holder.

**What changed from the plan:** The plan said the test should reuse Step 5's shared JUL-capture helpers. `BTreeEngineTestFixtures` was package-private after Step 5's Review fix, so two one-token visibility relaxations were needed (`class BTreeEngineTestFixtures` to `public`, `static List<LogRecord> captureSevereOn(...)` to `public static`). No other helpers in that file widened. Step 7 is unaffected (different test directory, no engine log capture currently).

**Key files:**
- `core/src/test/java/com/jetbrains/youtrackdb/internal/core/storage/impl/local/StorageCascadeContainmentOnNullCountUnderflowTest.java` (new)
