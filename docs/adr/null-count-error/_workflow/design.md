...
*Third, the per-index lock is held during apply.* `lockIndexes` at `AbstractStorage:2233` acquires each index's exclusive lock via `acquireExclusiveLockTillOperationComplete`, which adds the component to `operation.lockedComponents()` (`AtomicOperationsManager.java:313`). The inner-finally `releaseLocks` call at line 263 iterates `lockedComponents()` and unlocks each. Placing Hook B before that finally block means Hook B runs with all per-index locks still held — including the ones the next TX is blocked on. The race window the old design left open (manual apply at `AbstractStorage:2333` running after the lock release inside `endAtomicOperation`) is closed.
