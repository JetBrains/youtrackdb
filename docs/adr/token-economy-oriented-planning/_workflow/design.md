...
YouTrackDB's workflow already plans for token cost in two places. Step fill in `track-review.md` merges small steps to remove cold-read re-pays; track sizing in `planning.md` maximizes each track to cut the number of track cycles, each of which pays a fixed review-and-implementation tax. Both levers minimize a count, fewer steps and fewer tracks, and neither looks at which source files the work touches.

This design adds the missing dimension: source-file overlap. A file edited across K separate sub-agent sessions is cold-read K times, so decomposition should pull changes that touch the same files together.

The load-bearing observation is that co-location and adjacency are not equal levers. Merging same-file changes into one step or one track removes a real cold read, while ordering two unmergeable units next to each other saves almost nothing. The directive therefore reads co-locate first, and fall back to adjacency only when a hard rule forbids sharing.
...
**TL;DR.** Co-location removes a cold read; adjacency mostly does not. Within a step, merging two same-file changes gives one implementer one read instead of two implementers reading twice; within a track, one decomposition and one review cover the shared file once. Adjacency between unmergeable units leaves each sub-agent reading independently, so it recovers only rebase distance and orchestrator freshness. The principle holds at both granularities and sits below the hard constraints, never above them.

A file edited by K separate sub-agent sessions costs K cold reads. The whole directive is one move: lower K by pulling same-file work together. The architecture decides where that move pays off. Per-step implementers are fresh sub-agents, so merging two same-file changes into one step turns two reads into one. Phase A decomposition and Phase C review run once per track, so co-locating two same-file changes in one track has each of those passes read the file once instead of twice. A boundary that spawns a fresh context is a re-pay point, and both the step boundary and the track boundary are such points, which is why the same principle earns its place at each.

The footprint budget is what makes overlap the preferred fill choice rather than a mild nicety. The ~12 fill target and the ~20-25 track ceiling count distinct files. An overlapping unit adds only its new files to that count, so it spends less of the budget than a disjoint unit of the same size and frees room for more change under the same cap. Choosing the overlapping unit first therefore packs more work per cold read and per cap.

Adjacency is the honest fallback, not a co-equal lever. A naive reading of the directive, "make overlapping changes adjacent", is directionally right but misstates the mechanism, because two adjacent-but-separate steps still get two fresh implementers and Phase C still reads the whole track diff regardless of step order. What adjacency does buy is real but smaller: a shorter rebase distance in the stacked-diff series and an orchestrator whose just-built knowledge of the shared file is fresh when it briefs the next unit. The rule text leads with merge-and-pack so a future author does not promote adjacency into a token claim it cannot support.

### Edge cases / Gotchas

- Step adjacency that is not a merge buys almost nothing at the step level: the track-level review reads the cumulative diff order-independently and each step's implementer is fresh. The step rule states this plainly rather than implying a saving.
- Overlap can pull toward a smaller track if taken too far; it must not override "maximize footprint". Overlap chooses which units to pack and where to cut, not how many to pack.
- A `high`-isolation step never absorbs a neighboring overlapping change; isolation wins over the tie-breaker.

### References

**D1. Overlap-awareness as a co-locate-first tie-breaker.** Alternatives: do nothing, which leaves both levers overlap-blind; the naive "make overlapping changes adjacent", which is directionally right but misstates the mechanism since adjacency without merging saves little; make overlap a sizing or relatedness criterion, which contradicts the standing rule that thematic coherence is not a sizing criterion. Rationale: co-location removes a real cold read and spends less footprint budget, so it is the strong lever and adjacency is the fallback. Risk: authors may still read "adjacent" as the goal, so the rule text leads with merge-and-pack to counter it.

**D2. Apply at both track and step granularity.** Alternatives: track-only (leaves the step fill rule's which-unit choice overlap-blind); step-only (leaves cross-track re-reads unaddressed). Rationale: both the step boundary and the track boundary spawn fresh contexts and re-pay cold reads, so the same principle pays off at each, and the step-level gain is distinct from the existing maximize-fill because it orders which mergeable unit to pull in first. Risk: two authoritative edit sites instead of one, accepted because the principle is identical and is stated once in this section.
...
Packing order is the first refinement. Among candidate units that fit under the ceiling, the one overlapping the track's current file set wins the tie, because it spends less footprint and avoids a cross-track re-read of the shared file later. When no candidate overlaps, the planner packs anyway and maximizes the footprint, related or not, exactly as the standing rule says.
...
**TL;DR.** The fill rule already merges `low`/`medium` work toward ~12 edited files to remove cold-read re-pays. Overlap sharpens the choice of what to merge: prefer pulling in a unit that touches files already in the step, since the shared file is then read once and the unit spends less of the ~12 budget. Pure adjacency between two steps that cannot merge buys almost nothing at the step level, and the rule says so plainly.
...
The rule keeps the adjacency caveat explicit. Two distinct steps each get a fresh implementer, and Phase C reads the whole track diff regardless of step order, so ordering two unmergeable steps next to each other does not remove a step-level read. The one step-level adjacency gain is the orchestrator briefing the next implementer from its just-built knowledge of the shared file, which is modest. Stating this stops the rule from drifting into a claim that step adjacency is a token win on par with merging.
...
**D4. Step overlap-aware fill, with step adjacency named as not a merge.** Alternatives: extend the fill rule to prefer overlap silently, which loses the caveat and lets a future author reintroduce "make steps adjacent" as a false token claim; leave fill overlap-blind, the status quo. Rationale: the merge preference is a real token win via the footprint-budget math, and stating the adjacency caveat keeps the rule from drifting to a claim it cannot support. Risk: none material; the change is one ordering clause plus one caveat on an existing rule.
