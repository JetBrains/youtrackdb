...
**Escalated to inline replanning.** Completion review surfaced a design-level gap: a class, property type, or constraint rule created in an open transaction is not enforced on that transaction's own entities, because `EntityImpl.validate()` reads the committed-only immutable snapshot. The user escalated it to inline replanning, which added **D21** (a tx-aware immutable snapshot) to **Track 5**, alongside D15's index overlay. D21 touches Track 4's commit path: `computeCommitWorkingSet` (`AbstractStorage:2528`) reads `getImmutableSchemaClass` then `getCollectionForNewInstance` to resolve a new entity's collection id, after `reconcileCollections` and before `forceSnapshot`. Track 5 must confirm the tx-local class carries its reconciled real id there, or guard the read so a tx-aware snapshot never hands `doGetAndCheckCollection` a provisional id (`<= -2`). Two index-side findings from the same review were also deferred to Track 5 (the create-side provisional-collection index gap and the drop-side tx-local `dropIndex` no-op).
