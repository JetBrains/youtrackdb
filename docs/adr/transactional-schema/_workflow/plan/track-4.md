...
- **Rationale**: index-engine creation must land before `lockIndexes` (which resolves engines by id and throws on a missing one), and collection creation before the record-position-allocation loop (a record can only get a position once its collection exists). Reconciliation and population call the lock-free inner primitives (`doAddCollection` / `dropCollectionInternal`, and new `doAddIndexEngine` / `doDeleteIndexEngine(atomicOperation, …)` extracted from the public methods), never the public structural methods, which re-acquire the non-reentrant `stateLock` the commit already holds and self-deadlock.
- **Risks/Caveats**: index population is a lock-free internal scan feeding `doPut`, all on the commit's atomic operation, emitting zero additional WAL units; a nested batch transaction would re-enter `stateLock` and make the build durable independently of the commit.
- **Implemented in**: this track
- **Full design**: design.md §"Commit-time reconciliation"
...
- **Risks/Caveats**: the structural create/drop set is NOT derived from the changed-record set — a dropped class's record is deleted, so it carries no per-property signal, and a diff over the changed-record set would drop nothing. The structural set uses D9's set difference instead.
- **Implemented in**: this track
- **Full design**: design.md §"Commit-time reconciliation"
...
3. Extract `doAddIndexEngine` / `doDeleteIndexEngine` from the public methods so
   reconciliation runs lock-free under the held write lock.
...
5. Resolve provisional ids through the five-item patch list, re-pointing the
   changed-class records' property values before `commitEntry`.
6. Defer shared-registry publication to the post-`commitChanges` success path.
...
- One transaction creates two classes (16 collections, 2+ engines) plus records,
  commits, and restarts; every collection and engine resolves (I-A3).
...
- Force a commit to fail at apply; no collection or engine is registered afterward
  and the next commit reuses the ids (I-A4).
...
- **In scope**: `AbstractStorage.commit` (the schema-carry branch, the four-lock
  ...
  extraction; the provisional-id sites (the `collectionsToClasses` reverse-map re-key
  and the `collectionId < 0` vs `<= -2` predicate split); the two snapshot-first read
...
- **Signatures**: `doAddIndexEngine(atomicOperation, …)` / `doDeleteIndexEngine(atomicOperation, …)`
  extracted from the public methods; a commit-local structural-id allocator seeded inside
  the write lock; the schema-carry branch reading the unified schema-or-index signal at
  commit entry.
