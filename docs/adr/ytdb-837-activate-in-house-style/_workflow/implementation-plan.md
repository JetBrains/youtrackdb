...
- **`.claude/hooks/house-style-write-reminder.sh`** — New PreToolUse hook added by Track 2. Bash script following the pattern of `mcp-steroid-grep-reminder.sh` (jq-or-printf JSON emission, portable `stat` mtime read, fail-silent on every error mode) but keyed by `session_id` extracted from the hook input JSON per D2, with per-tier state in `${TMPDIR:-/tmp}/house-style-reminder-${session_id}.txt`. Emits `hookSpecificOutput.additionalContext`.
