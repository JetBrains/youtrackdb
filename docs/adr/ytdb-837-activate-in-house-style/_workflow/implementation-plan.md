...
- [x] Track 3: Pointers in workflow prompts and review agents
  ...
  >
  > **Strategy refresh:** ADJUST — Track 4 Phase A decomposition will absorb the two Track 3 operational findings as decomposition inputs: per-step plan calls native `Edit` fallback (since `steroid_apply_patch` is not exposed in implementer spawns) and pre-scans each candidate file for the colon-terminated-lead-in shape before choosing the insertion anchor. No disk amendments to Track 4's four Phase 1 sections at the gate.

- [x] Track 4: Pointers in implementer + commit-convention files (Tier A + Tier B split)
  ...
  >
  > **Track episode:**
  > Landed the canonical house-style pointer paragraphs in `implementer-rules.md` (Tier-A + Tier-B bullet appended to § Tooling discipline), `episode-format-reference.md` (Tier-A intro paragraph), `step-implementation.md` (Tier-A paragraph after the five-field bullet list), and `commit-conventions.md` (Tier-A paragraph inside § Commit type prefixes). Each pointer names `.claude/output-styles/house-style.md` as the rule source, cites `conventions.md §1.5` via the Markdown-link form, and after iteration 1 carries the four banned-section heading slugs verbatim (`## Banned vocabulary`, `## Banned sentence patterns`, `## Banned analysis patterns`, `### Em-dash discipline`) for the §1.5 rename-gate grep enumeration. Phase C iteration 1 closed 6 in-scope findings cleanly: WS1 fixed the double em dash in commit-conventions.md that violated `### Em-dash discipline` inside the very paragraph citing that rule; WC1 added the four-slug verbatim citation to episode-format-reference.md (Step 1 had silently omitted it despite the episode prose claiming otherwise); WC4 aligned Tier-A→Tier A so all four pointers use the space form; WS2 swapped an AI-cadence em dash for a colon; WI4 named the four `EPISODE_DRAFT`-sourced fields explicitly in step-implementation.md (the bullet list above shows five); WI2 bound the four slugs to the Tier-B AI-tell subset in implementer-rules.md.
  >
  > **Cross-track impact for Track 5:** Track 5's orchestrator-file pointers should follow Track 4's link-form citation precedent and keep the literal `conventions.md §1.5 Writing style for Markdown and prose artifacts` substring un-wrapped on one line (Step 2 surprise). Every new pointer must carry the four banned-section heading slugs verbatim — the §1.5 rename-gate grep enumerates by literal slug strings, so an omission breaks the chain silently (WC1 caught this happening in Track 4 itself). Audit shapes that bound by `grep -B<N> -A<M>` windows over rule-doc paragraphs are brittle and silently return 0 when the cited string sits outside the window (iteration 1 surprise): prefer per-paragraph (blank-line-split) audits when asserting "feature X is cited in paragraph Y".
  >
  > **Cross-track impact for Phase 4:** Two deferred findings need anchoring before `_workflow/` cleanup wipes the Track 4 surprise log — the single-line-layout constraint for link-form citations (WI5) and the commit-subject-line carve-out asymmetry between commit-conventions.md and conventions.md §1.5 (WI6). Both are one-sentence additions to conventions.md §1.5 (or to `design-final.md`'s style section) at Phase 4. Three other findings (WB1-3 collapse-pointers, WI3 load-house-style-once invocation language, WC3 anchor-fragment in link targets) re-open D3 design choices or apply at broader scope than this branch; left as out-of-branch follow-ups, not promoted into the plan.
  >
  > **Track file:** `plan/track-4.md` (2 steps, 0 failed)
  >
  > **Strategy refresh:** ADJUST — Track 5 Phase A decomposition absorbs four Track 4 operational inputs as per-step plan ingredients: (1) link-form citation precedent (Markdown-link form to `house-style.md` plus `conventions.md §1.5 Writing style for Markdown and prose artifacts` kept un-wrapped on one line per WI5); (2) four banned-section heading slugs (`## Banned vocabulary`, `## Banned sentence patterns`, `## Banned analysis patterns`, `### Em-dash discipline`) cited verbatim per WC1's rename-gate grep contract; (3) colon-terminated lead-in pre-scan before choosing the insertion anchor in each candidate file (carried from Track 3 surprise via Track 4); (4) native `Edit` fallback since `steroid_apply_patch` is not exposed in implementer sub-agent spawns. No plan/track-file disk content changes at the gate.

- [x] Track 5: Pointers in orchestrator files (Tier-B subset for chat-scale prose)
  ...
  >
  > **Track episode:**
  > Landed the canonical chat-scale-prose pointer paragraph in 9 orchestrator-surface files: the top-level `workflow.md`, the three user-invocable skill bodies (`create-plan`, `execute-tracks`, `review-plan`), and the five mid-loop protocols (`mid-phase-handoff`, `inline-replanning`, `review-mode`, `review-iteration`, `design-decision-escalation`). Every pointer cites the four banned-section heading slugs verbatim (`## Banned vocabulary`, `## Banned sentence patterns`, `## Banned analysis patterns`, `### Em-dash discipline`), exempts the three structural-rule sections by name, and links `conventions.md §1.5` in Markdown-link form. Phase C iteration 1 surfaced a form-drift between Step 1's blockquote landings (workflow.md + 3 SKILL.md) and Step 2's plain-bold landings (5 mid-loop protocols); three dimensional reviewers (WP1, WI1, WC1) converged. The `Review fix:` commit `198b6996a2` converted the five mid-loop protocols to blockquote form and tightened the Validation grep from substring match to `^> \*\*House style for chat-scale prose\.` anchor so future drift fails the check rather than silently passing. The iteration 1 implementer exited mid-flight without a RESULT block after applying all edits but before its own commit step; the orchestrator finalised via the `RESULT_MISSING` commit-as-is recovery handler (the only Phase C case where the orchestrator commits source-file changes directly). All 4 gate-check dimensions returned PASS at iteration 1: WC1/WP1/WI1/WI2 VERIFIED, WP2/WP3/WI3/WI4/WI5/WB1 REJECTED.
  >
  > **Cross-track impact for Phase 4:** F-DEF1 / WI2 — the `.githooks/prepare-commit-msg` hook treats any case-insensitive `YTDB-NNN` match in the commit body as a 'prefix already present' signal and skips prepending the subject prefix. Track 5 Step 1 surfaced the trap when the implementer's commit body cited `YTDB-837` literally; Step 2 confirmed in both directions. Candidate landing site: `CLAUDE.md § Git Conventions § Commit Messages`. Phase 4's `create-final-design.md` should fold this in alongside the Track 4 WI5/WI6 carryovers already queued (single-line layout constraint for link-form citations, commit-subject-line carve-out asymmetry between commit-conventions.md and conventions.md §1.5).
  >
