...
The MCP Steroid `apply_patch` tool input shape needs research — the canonical place to look is `mcp-steroid://skill/apply-patch-tool-description` via `steroid_fetch_resource`. From the project rule in `.claude/workflow/conventions.md §1.4 *Other mcp-steroid routes*`, the tool accepts a patch string with embedded `+++ b/<path>` lines (standard unified-diff shape). The hook parses these lines via `grep -oE '^\+\+\+ b/(.+)$'` or equivalent to enumerate target paths. The tool ID at the dispatch site is `mcp__<server>__steroid_apply_patch` where `<server>` is the user-global `mcpServers` registry key from `~/.claude.json`; the hook matches it via the regex `mcp__.+__steroid_apply_patch` so it fires under any registry-key choice.

The existing `.claude/scripts/tests/test_dsc_ai_tell.py` proves the `pytest` pattern works under `.claude/scripts/tests/`. The new `test_house_style_hook.py` follows the same shape: subprocess-invoke the bash script with fixture JSON on stdin, assert the parsed JSON output matches expectations.
...
Step 3 — Write the Python test suite at `.claude/scripts/tests/test_house_style_hook.py`. Each test invokes the hook as a subprocess, pipes fixture JSON on stdin (carrying `session_id`, `tool_name`, `tool_input`), parses the JSON output, and asserts on the `additionalContext` content. Tests use `tmp_path` fixtures pointed at via `TMPDIR` so the state file is isolated per test. Test cases must include: (a) state-file isolation between two distinct `session_id` values (the post-`/clear` simulation — same Markdown path fires the Tier-A reminder again under a fresh `session_id`); (b) state-file persistence within one `session_id` across multiple invocations (the rate-limit case).
...
- Hook fires on `Write`, `Edit`, and `mcp__localhost-6315__steroid_apply_patch` for `*.md` paths and surfaces the full-house-style reminder. Acceptance bullet from YTDB-837: "A test branch where a non-compliant draft is written into `docs/adr/<branch>/` surfaces house-style rules via the hook within the same Write/Edit turn."
...
- Python test suite at `.claude/scripts/tests/test_house_style_hook.py` runs under `pytest` from the repository root and passes.

<!-- Phase A placeholder for per-step EARS/Gherkin lines. -->

<!-- Reserved for Move 3. -->

## Idempetence and Recovery
<!-- Phase A placeholder. -->
...
- `.claude/settings.json` (modify — add `PreToolUse[1]` entry with matcher `Write|Edit|mcp__.+__steroid_apply_patch`)
...
- `tool_input.<field>` — for `mcp__<server>__steroid_apply_patch`, field name to be confirmed in Phase A by reading the apply-patch tool description; likely `patch` carrying unified-diff text with `+++ b/<path>` lines.
