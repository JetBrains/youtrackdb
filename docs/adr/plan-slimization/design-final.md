...
adversarial-reviewer agent file exists under `.claude/agents/` (the reviewers are
...
`.claude/scripts/tests/` proves the unchanged precheck parses it across the State
...
the live `.claude/**` files stayed at the `develop` state while staged edits
accumulated under `_workflow/staged-workflow/.claude/` until the Phase 4 promotion.
