...
- **Memory bounded.** Three knobs (`maxEntries`, `maxRecordsPerEntry`, `maxRecordsPerEntryForBlockingSort`) cap per-tx footprint to predictable limits. The blocking-sort cap is smaller because over-fetch's eager-materialize cost in `OrderByStep` plans is not amortized by consumer-paced iteration (D17).
...
- **`CacheableShape` (new enum)** — discriminator computed by `ShapeClassifier.classify(stmt)`: `RECORD | AGGREGATE_COUNT | AGGREGATE_SUM | AGGREGATE_AVG | AGGREGATE_MIN | AGGREGATE_MAX | MATCH_TUPLE_MULTI | K0_NONE | HARD_NONE`. Drives `DeltaBuilder` dispatch and the K0 invalidation gate. MATCH_TUPLE_MULTI introduced for multi-alias MATCH (Etap B partial in v1 — see D8-lazy); supports DELETED + UPDATED via reverseIndex, tombstones on CREATED. K0_NONE introduced for delta-unreconcilable shapes (LET, GROUP BY, $matched, subqueries) — D18 caches them under `tx.mutationVersion` gate; entry serves cache hits while mutationVersion is unchanged, invalidates when it diverges. HARD_NONE for shapes that cannot be cached regardless of mutation state (`LIMIT > maxRecordsPerEntry`, blocking-sort over D17's cap, etc.).
...
- **`GlobalConfiguration`** (modified) — five new knobs: `QUERY_TX_RESULT_CACHE_ENABLED`, `QUERY_TX_RESULT_CACHE_MAX_ENTRIES`, `QUERY_TX_RESULT_CACHE_MAX_RECORDS_PER_ENTRY`, `QUERY_TX_RESULT_CACHE_MAX_RECORDS_PER_ENTRY_FOR_BLOCKING_SORT` (D17 — smaller cap that applies when the plan contains an `OrderByStep`), `QUERY_TX_RESULT_CACHE_K0_NONE_INVALIDATION_THRESHOLD` (D18 — N invalidations of a K0_NONE entry before its key joins `nonCacheableKeys`, default 3).
...
- **Risks/Caveats**:
  ...
  - **Over-fetch cost for one-off queries** — a query with `LIMIT 5` that never repeats now pulls up to `maxRecordsPerEntry` records from storage instead of 5. The cost is paid even when cache is never hit. Mitigated by: (a) cache is opt-in (default disabled), (b) cache only over-fetches when ShapeClassifier returns RECORD (which includes single-alias MATCH Etap A folded into RECORD with `returnProjector`) and `cache.enabled == true`, (c) one-off-query workloads typically have non-determinism markers that already bypass the cache. For Hub-typical workloads where queries repeat, the amortization is favorable.
...
- **Implemented in**: Track 4 (plan rewrite at cache-miss for LIMIT-bounded queries, view-level SKIP/LIMIT application, `HARD_NONE` classification above cap).
...
- **Rationale**: Under lazy + D18, the relevant coverage metric shifts from "what fraction of queries reach K1 sharp-merge" (eager) to "what fraction of queries classify as cacheable (anything except `HARD_NONE` — K0_NONE entries are cacheable under D18's version gate)" (lazy). Before Hub deployment, Track 8 will record an anonymized DNQ-emission sample from a Hub staging environment (single tx, ~1000 queries) and replay against the cache; pass criteria: ≥70% of repeat-shape queries classify as cacheable (`ShapeClassifier.classify(stmt) != HARD_NONE`) AND view output matches fresh execution across the recorded mutation sites. Also measures: per-query delta-build cost (informs per-class-index v2 decision), WHERE re-evaluation hot RIDs (informs per-RID memoization v2 decision), MIN/MAX extremum-churn frequency (informs D14 sorted-value index v1.1/v2 decision), MATCH Etap B coverage (informs whether the separate Etap-B ADR is high-priority), paginated-workload share (informs canonical-CacheKey v2 decision).
...
#### D17: Per-plan-shape over-fetch cap — smaller cap for blocking-sort plans

- **Alternatives considered**:
  - **Single cap `maxRecordsPerEntry` for all plans (prior baseline)** — D10-lazy used one cap regardless of plan shape. For streaming plans (no `OrderByStep`, or `OrderByStep` backed by an index) over-fetch is lazy — the executor pulls from storage on demand, so capacity = `maxRecordsPerEntry` is a ceiling rather than an eager allocation. For blocking-sort plans (`OrderByStep` with no backing index — `OrderByStep.java:18` "sorts all upstream records") the executor drains the full matching set into its sort buffer on first `next()`; the LimitStep value the cache rewrites to does NOT bound that upstream drain (LimitStep is downstream of OrderByStep and only truncates emission). The cache holds whatever the executor sorted into `entry.results` up to the cap. For a one-shot `SELECT ... ORDER BY priority LIMIT 20` with no priority index and a low-selectivity WHERE matching 5000 rows, the entry retains 5000 (or `maxRecordsPerEntry = 10000`, whichever is smaller) sorted Results when the consumer only ever needed 20. Memory waste per such entry: up to 9980 Result refs.
  - **Selectivity-aware classify (deferred to v2)** — at classify time, query `SchemaClass` statistics to estimate WHERE selectivity; bypass cache when blocking-sort + low selectivity. Adds DB call to classify hot path, depends on stat accuracy, fuzzy "low selectivity" cutoff. Not v1 material.
  - **Top-K heap inside `OrderByStep` (out of scope)** — replace the full-sort `OrderByStep` with a bounded heap of size `LIMIT + margin` so blocking sort retains only top-K rows. Reduces both sort work and memory. Executor-level change, separable from the cache, valuable independently. Tracked as a separate ticket; the cache cannot assume it lands.
  - **Per-plan-shape cap (chosen)** — keep one cap for streaming, add a smaller cap for blocking-sort. Detect the plan shape once at the existing plan-rewrite walk (we already iterate `plan.steps` to find `LimitStep` / `SkipStep`; the same walk records whether any `OrderByStep` exists). Set `effectiveCap = planHasOrderByStep ? maxRecordsPerEntryForBlockingSort : maxRecordsPerEntry` before mutating `LimitStep.limit`.
- **Rationale**: streaming over-fetch is essentially free for read-mostly workloads (consumer iterates only what's needed; backfill paid on-demand by stream pull). Blocking-sort over-fetch is fundamentally different — `OrderByStep` materializes the full matching set into a sort buffer regardless of downstream `LimitStep`, and the cache then retains the prefix (truncated at cap) of that buffer. The asymmetry isn't user-visible (both shapes produce correct results) but the memory cost is. The two-cap approach is the minimal mechanism that bounds the blocking-sort case without affecting streaming. Default `500` for the blocking-sort cap is a guess shaped by Hub's typical LIMIT (20-50) plus generous backfill margin; D13 measurement informs the v1.1 default.
- **Risks/Caveats**:
  - **AST classify can't predict plan shape.** `ShapeClassifier.classify` runs on the AST before `createExecutionPlan` so it cannot know whether an `OrderByStep` will appear in the plan. Classify uses the upper bound `maxRecordsPerEntry` for its LIMIT admission gate; a query with `LIMIT 2000` passes classify as RECORD. Plan-rewrite then discovers the plan IS blocking-sort and `2000 > maxRecordsPerEntryForBlockingSort = 500`. The cache cannot fit the user's `LIMIT 2000` within the blocking-sort cap (truncating to 500 would silently short-list the user-visible result). The query falls back to the splice-failure path: close the partial plan, increment `QueryCacheMetrics.blockingSortOverCap` (sibling counter to `spliceFailures`), call `statement.execute(...)`, return `LocalResultSet` directly, add the key to `nonCacheableKeys` so subsequent identical queries skip the plan-build round-trip. The plan-build cost is paid once per such key per tx — acceptable for queries that fall outside the cache's footprint.
  - **`OrderByStep` detection is a simple class-instance walk** over `SelectExecutionPlan.steps`. Index-backed ORDER BY does NOT produce an `OrderByStep` in current YTDB planning (`SelectExecutionPlanner` consumes the ORDER BY at index-step level); confirmed by reading `SelectExecutionPlanner.java` ORDER BY handling. If a future planner change emits a different blocking-sort step class, the detector misses it and the streaming cap applies — wasted memory but no correctness loss. The detector lives in the same plan-walk method that finds `LimitStep` / `SkipStep`, so adding new sort step classes is one-line.
  - **Default 500 is a guess** — D13 Hub-replay measures: (a) fraction of cached queries that are blocking-sort, (b) `LIMIT` distribution for blocking-sort branches, (c) actual backfill demand under realistic mutation patterns. If `blockingSortOverCap` fires frequently the default rises; if blocking-sort entries dominate memory the default falls. Hot-tunable via the GlobalConfiguration knob; no recompile / restart needed for tuning rounds.
  - **No symmetric blocking-sort gate at AST classify** is intentional. Treating "AST has ORDER BY" as the gate would over-classify (index-backed ORDER BY produces streaming plans and should get the larger cap). Predicting plan shape from AST alone requires schema + index lookup, which AST-level classify avoids. The cost of late detection is at most one wasted `createExecutionPlan` call per blocking-sort-over-cap query — bounded and amortized by `nonCacheableKeys`.
- **Implemented in**: Track 4 (extends the existing plan-rewrite step with `OrderByStep` detection and dual-cap selection; adds `blockingSortOverCap` counter to `QueryCacheMetrics`; adds tests for streaming-cap vs blocking-cap behavior and the over-cap fallback path).
- **Full design**: design.md § Over-fetch for backfill → Per-plan-shape cap (streaming vs blocking-sort)
...
- [ ] Track 4: Lazy delta core + RECORD shape
  > Implement `ShapeClassifier` RECORD / `K0_NONE` / `HARD_NONE` classify, `DeltaBuilder.buildForRecord` (class-filtered single-pass over `recordOperations` producing skip-set + sorted inject-list), `CachedResultSetView` sorted-merge for RECORD plus the K0_NONE branch (lazy stream-pull, no delta), and the D18 version-gate at `QueryResultCache.lookup`. SKIP/LIMIT, polymorphism (D11), and the dispatch table for `(op.type, cached, match_after)` all fold into the RECORD path. Detail in `plan/track-4.md`.
  > **Scope:** ~7 steps covering `ShapeClassifier` RECORD / `K0_NONE` / `HARD_NONE`, `DeltaBuilder.buildForRecord`, `TxDeltaCursor` sorted-merge + K0_NONE branch in `CachedResultSetView.next()`, polymorphism + SKIP/LIMIT integration, D18 version-gate + `k0InvalidationCount` + `nonCacheableKeys` threshold, full test matrix (CREATED/UPDATED/DELETED × cached/not × match_after × ORDER BY × SKIP, plus T4k1–T4k5 K0_NONE coverage).
