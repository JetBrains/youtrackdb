...
- **Risks/Caveats**:
  - `returnProjector` correctness depends on the projection closure built at entry construction matching the original execution's projection semantics exactly. Track 6a step-g test validates equivalence vs fresh re-execution.
...
## Plan Review
- [x] Plan review (consistency + structural) — manual re-run iteration 4 via `/review-plan`, passed
