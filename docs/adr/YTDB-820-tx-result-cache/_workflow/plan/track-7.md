...
8. **Re-entrancy guard (L9 fix + SO5 scope clarification)**: `QueryResultCache` tracks a re-entrancy counter `cacheCodeDepth: int` (per-tx, single-threaded — counter rather than boolean to handle arbitrarily-nested cache-internal calls). The counter is incremented at the START of every cache-mutating code path (`lookup`, `put`, `invalidateAll`, the stream-pull-append loop inside `view.next()`, `DeltaBuilder.buildForRecord`/`buildForAggregate`) inside a try/finally that decrements on exit. While `cacheCodeDepth > 0`, any re-entrant call to `cache.lookup` short-circuits to "skip cache" mode (return null/bypass — the caller sees a cache miss and executes uncached via `statement.execute(...)`). This catches the L9 hazard (UDF in WHERE → `session.query()` → nested cache code) regardless of whether the UDF fires during `lookup` itself, `view.next()`, or stream-pull-append.
