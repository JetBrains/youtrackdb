...
DML invalidation: `DatabaseSessionEmbedded.executeInternal()` (line 702) routes the one legitimately mid-tx-runnable bulk operation through `invalidateAll()`. The trigger is an **explicit type list** (helper `isBulkBypass(SQLStatement)`), not `!isIdempotent()`:
...
Schema DDL (`SQLCreate/Drop/AlterClassStatement`, `SQLCreate/Drop/AlterPropertyStatement`, `SQLCreate/Drop/RebuildIndexStatement`) is **excluded** from the bulk-bypass list because invariant **I8** makes those statements unreachable mid-tx — `SchemaShared.saveInternal` (`SchemaShared.java:820-823`) throws on every schema mutation while a tx is active, and `IndexManagerEmbedded` (lines 307, 459) throws on index DDL. The cache hook fires before `statement.execute()`, but the schema-DDL statement throws before any cache effect would matter, so wiping the cache there is wasted work.

To defend against silent regression if the upstream guard is ever relaxed, this track wires a `Java assert` in the cache-lookup helper of `executeInternal`: `assert !isSchemaDDL(statement) || !session.getTransactionInternal().isActive() : "Schema DDL unreachable mid-tx per I8 — upstream SchemaShared.saveInternal should have thrown.";`. Asserts are stripped in production by default but fire in tests and `-ea` runtime, so any future regression surfaces immediately as a test failure with a clear pointer to I8.

Regular `SQLInsertStatement`/`SQLUpdateStatement`/`SQLDeleteStatement` are also **not** in the list. They flow through `addRecordOperation` per affected record, so per-entry sharp-merge from Track 4 handles them — adding `invalidateAll()` on top would destroy K1-merged state for zero benefit. Scripts (`computeScript(...)`) are outside `executeInternal()` entirely and are a Non-Goal of this plan. The hook fires before `statement.execute()` so subsequent queries within the same statement see a clean state. `invalidateAll()` iterates entries, closes each entry's stream via `entry.close()`, clears the map.
...
3. Wire DML invalidation. In `DatabaseSessionEmbedded.executeInternal`, before `statement.execute()`: `if (queryResultCache != null && isBulkBypass(statement)) queryResultCache.invalidateAll();`. The `isBulkBypass(SQLStatement)` helper returns true only for `SQLTruncateClassStatement` (the only mid-tx-runnable bulk op — see Context and Orientation; schema DDL is excluded because I8 makes it unreachable mid-tx). Wire an accompanying `Java assert` to defend against silent upstream-guard regression: `assert !isSchemaDDL(statement) || !session.getTransactionInternal().isActive() : "Schema DDL unreachable mid-tx per I8 — upstream SchemaShared.saveInternal should have thrown.";` (helper `isSchemaDDL` matches against the schema-DDL type set: `SQLCreate/Drop/AlterClassStatement`, `SQLCreate/Drop/AlterPropertyStatement`, `SQLCreate/Drop/RebuildIndexStatement`). Regular `INSERT`/`UPDATE`/`DELETE` do **not** trigger the wipe; they rely on `addRecordOperation` from Track 4. Tests verify: `TRUNCATE CLASS` wipes; plain `INSERT`/`UPDATE`/`DELETE` do not (and per-entry sharp-merge from Track 4 is preserved across them); attempting `CREATE CLASS` / `DROP CLASS` / `CREATE INDEX` mid-tx throws `SchemaException` / `IllegalStateException` per I8, leaving cache state unchanged (the assert under `-ea` validates the cache-hook precondition).
...
- `boolean DatabaseSessionEmbedded.isBulkBypass(SQLStatement)` (static helper) — returns true only for `SQLTruncateClassStatement` (schema DDL excluded per I8; see Context and Orientation).
- `boolean DatabaseSessionEmbedded.isSchemaDDL(SQLStatement)` (static helper, assertion-only) — returns true for the schema-DDL statement types; used in the I8 canary assert.
