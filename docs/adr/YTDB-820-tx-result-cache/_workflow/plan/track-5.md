...
- Context-variable reference (`SQLBaseExpression` with `dollar=true`) with name in: `now`, `current`, `currentdate`, `currenttime`, `thread`, `parent`, `depth`.
...
5. R-B: relax K1 RECORD gate for deterministic ORDER BY expressions. In `SharpMergePredicate.classify(SQLSelectStatement)`, the current ORDER BY gate accepts only items whose expression is a plain property reference. Extend: for each `SQLOrderBy.items[i]`, accept the item if either (a) it's a plain property reference (current behavior), or (b) `NonDeterministicQueryDetector.contains(item.recordsExpression)` returns false. The `OrderByComparator` from Track 4 already calls `SQLExpression.evaluate(record, ctx)` for each item, so no comparator changes are needed; this is purely a classify-gate relaxation. Tests: (i) `SELECT FROM User WHERE active=true ORDER BY lower(name) LIMIT 10` → entry created with RECORD discriminator; mutations re-splice via `lower(name)` comparator. (ii) `SELECT FROM User ORDER BY sysdate() LIMIT 10` → non-determinism detector flags `sysdate()`, classify returns NONE (or cache is bypassed at lookup time, same effect). (iii) `SELECT FROM Issue ORDER BY priority * 10 + age` → deterministic expression, K1 RECORD; mutation that changes `priority` or `age` re-splices correctly.
...
- R-B correctness: `ORDER BY` expressions that the detector flags as deterministic (e.g., `lower(name)`, `priority * 10`) classify as K1 RECORD and re-splice correctly on mutation. `ORDER BY sysdate()` / `ORDER BY random()` / `ORDER BY $now` bypass the cache entirely.
