...
4. Wire stream-close into entry lifecycle. `CachedEntry.close()` (existing skeleton from Track 1): if `stream != null && !exhausted`, call `stream.close(ctx)`, then null out plan/ctx/stream, AND **set `exhausted = true`**. The flag flip is load-bearing: `CachedResultSetView.hasNext()` reads `!entry.exhausted && entry.stream.hasNext(entry.ctx)`; after close, `stream` is null, so without `exhausted=true` the next `hasNext()` call would NPE on the null-stream branch. With the flag, the `!exhausted` short-circuits and only the `position < results.size()` branch decides termination — view drains the frozen list cleanly and raises `NoSuchElementException` at the end. Applies to all close-callers: LRU eviction, K0 wipe (Track 4), `invalidateAll` (Track 5), tx-end `cache.clear()`. `QueryResultCache.clear()` already iterates entries and calls `entry.close()` (per Track 1 skeleton). Test (g) added in step 6 below verifies post-close `hasNext()` behavior.
...
6. Tests: (a) partial iteration → re-query: pull 5 of 100, drop view, query again same key, assert second view returns rows 1-100 correctly (rows 1-5 from cache, rows 6-100 from stream); (b) two interleaved consumers: view A pulls row 1, view B pulls row 1 (same row, cached), view A pulls row 2 (cached?-no, pulls from stream and appends), view B pulls row 2 (now cached); (c) exhaustion: pull all 100, assert `entry.exhausted == true` and `entry.stream == null` after the 100th pull; (d) eviction-while-stream-open: set `maxEntries=1`, do query A (live stream), do query B (different key — evicts A), assert A's stream was closed; (e) tx-end with held stream: query, pull 5 of 100, commit, assert stream closed; (f) **`ExecutionStream.close()` idempotency regression (I6)** — populate an entry, call `entry.close()` twice in a row, assert no exception thrown and stream's close is invoked at most once (or invoked twice but is itself idempotent — verify which guarantee the codebase provides; if `ExecutionStream` is not currently idempotent, add a guard in `CachedEntry.close()` via the null-out-on-first-call pattern and document the choice); **(g) post-close exhausted flag flip** — populate entry with non-null stream (`exhausted=false`), open a view at position 0, pull 3 of 100 records (position now 3, stream still has 97), invoke `entry.close()` directly (simulates LRU eviction / K0 wipe / cache.clear), assert: `entry.exhausted == true`, `entry.stream == null`, `view.hasNext()` returns true (position 3 < results.size 3 — wait, results was extended to 3 by the pulls, so position equals size now), `view.hasNext()` returns false, `view.next()` raises `NoSuchElementException`. **NO NullPointerException** on the closed stream. Variant: if view pulled only 1 row before close, position=1, results.size=1, behavior the same. Variant: if view hadn't pulled anything (position=0, results.size=0), `hasNext()` returns false immediately.
