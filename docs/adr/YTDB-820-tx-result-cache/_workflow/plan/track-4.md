...
- **`RECORD` and `AGGREGATE_*` (simple SELECT)**: union of class names in `SQLFromClause.items` (top-level).
...
Polymorphism check on a dirty record: if `entry.fromClasses == null`, wipe unconditionally; otherwise for each name in `entry.fromClasses` evaluate `record.getSchemaClass().isSubClassOf(name)`. The `isSubClassOf(String)` method lives on the `SchemaClass` interface (`SchemaClass.java:150`) and is delegated through `SchemaClassProxy`; the concrete implementation is `SchemaClassImpl`.
...
1. Capture metadata. In `DatabaseSessionEmbedded.query()` miss path (where Track 2 creates the `CachedEntry`), classify the statement via `SharpMergePredicate.classify(stmt) → MergeKind` and:
   - For `RECORD`: extract `fromClasses` (resolve `SQLSelectStatement.target.items` to class names), `whereClause`, `orderBy`, `limit` from the AST. Pass into the `CachedEntry` constructor.
...
- `core/src/main/java/com/jetbrains/youtrackdb/internal/core/tx/MergeKind.java` (NEW) — enum with seven values.
