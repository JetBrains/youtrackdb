...
- **Plan-rewrite for over-fetch** (D10-lazy + D17): at cache-miss for RECORD shape, **iff the query has `LIMIT m` with `m <= maxRecordsPerEntry`**, walk `SelectExecutionPlan.steps` once and record two facts in the same pass: (a) the positions of every `LimitStep` and `SkipStep`, (b) whether any `OrderByStep` instance appears in the chain (D17 — blocking-sort detection; `OrderByStep` is YTDB's blocking-materializer step inserted only when ORDER BY has no backing index in the chosen plan). Then choose `effectiveCap = planHasOrderByStep ? maxRecordsPerEntryForBlockingSort : maxRecordsPerEntry`. If the AST's `LIMIT m` exceeds the chosen `effectiveCap` (the blocking-sort case where AST classify approved m up to `maxRecordsPerEntry` but the actual plan needs the smaller cap), take the splice-failure fallback path: close the partial plan, increment `QueryCacheMetrics.blockingSortOverCap` (new counter, sibling to `spliceFailures`), call `statement.execute(...)`, return `LocalResultSet` without caching, add the cache key to `nonCacheableKeys` so subsequent identical queries skip the plan-build. Otherwise mutate `LimitStep.limit` to `effectiveCap` and `SkipStep.skip` to 0 (or remove the steps if the field is final — implementation track decides). For queries with no LIMIT, the rewrite is skipped — the executor's natural stream feeds the cache up to `effectiveCap`. Original `skip` and `limit` from the parsed AST are stored on `CachedEntry`. Same splice-failure fallback (close, increment `QueryCacheMetrics.spliceFailures`, statement.execute, nonCacheableKeys) when the plan walk doesn't find the expected `SkipStep` / `LimitStep` shape.
...
7. Test matrix (T4 set):
   ...
   - T4h5 (D17 — streaming plan uses `maxRecordsPerEntry`): `SELECT FROM Foo ORDER BY @rid LIMIT 20` where `@rid` is the cluster's natural sort order (no `OrderByStep` in the plan — streaming). Classify=RECORD; plan-rewrite detects `planHasOrderByStep == false`; chooses `effectiveCap = maxRecordsPerEntry = 10000`; rewrites `LimitStep.limit = 10000`. Verify the entry can grow up to 10000 rows under no-LIMIT delete-then-refresh flows.
   - T4h6 (D17 — blocking-sort plan uses `maxRecordsPerEntryForBlockingSort`): `SELECT FROM Foo ORDER BY arbitraryExpr LIMIT 20` against a class with no index on `arbitraryExpr` (plan contains `OrderByStep`). Classify=RECORD; plan-rewrite detects `planHasOrderByStep == true`; chooses `effectiveCap = maxRecordsPerEntryForBlockingSort = 500`; rewrites `LimitStep.limit = 500`. Verify entry size at populate equals min(matching rows, 500), not 10000.
   - T4h7 (D17 — blocking-sort LIMIT over blocking-cap → fallback): `SELECT FROM Foo ORDER BY arbitraryExpr LIMIT 2000` with `maxRecordsPerEntry=10000`, `maxRecordsPerEntryForBlockingSort=500`. Classify=RECORD (uses upper bound `maxRecordsPerEntry`); plan-rewrite discovers `OrderByStep` AND `LIMIT 2000 > 500`. Verify: plan closed, `QueryCacheMetrics.blockingSortOverCap == 1`, query falls back to `statement.execute(...)`, consumer receives 2000 rows from non-cached `LocalResultSet`, `cache.size() == 0`, `nonCacheableKeys` contains the key. Re-issue the same query: `nonCacheableKeys` short-circuits, no plan build, same fallback behavior.
   - T4h8 (D17 — blocking-sort below blocking-cap still caches): `SELECT FROM Foo ORDER BY arbitraryExpr LIMIT 100` (≤ 500). Verify caching proceeds with `effectiveCap = 500`, entry populates up to 500 rows, view emits 100 to consumer, subsequent identical query is cache hit.
   - T4k1 (D18 — K0_NONE pure-read replay): `SELECT FROM Person GROUP BY age` in tx with no mutations. First call: classify=K0_NONE, populate, `populateMutationVersion=0`. Second call (same query, no mutations between): lookup sees `tx.mutationVersion (0) == entry.populateMutationVersion (0)` → cache hit, view returns same rows as first call's result. Verify exactly one storage execution; second call returns from cache.
   ...
   - T4k3 (D18 — K0_NONE invalidation threshold): same setup, but do mutation+repeat 4 times. After third invalidation: cache key in `nonCacheableKeys`. Fourth call: lookup short-circuits via `nonCacheableKeys.contains(key)` → miss, no plan-rewrite, falls to `statement.execute(...)`, no entry created. Verify `QueryCacheMetrics.k0NoneShortCircuits == 1` (counter for the short-circuit path).
   - T4k4 (D18 — K0_NONE coexistence with RECORD): in one tx, run BOTH a RECORD-classify query (e.g., `SELECT FROM Person WHERE active=true`) AND a K0_NONE query (e.g., `SELECT FROM Person GROUP BY age`). After mutation that affects Person: RECORD entry's view reflects the mutation via delta-build (existing behavior); K0_NONE entry is invalidated on next lookup. Verify both behaviors fire correctly in the same tx; cache entries don't cross-interfere.
   - T4k5 (D18 — HARD_NONE never cached): `SELECT FROM Foo LIMIT 50000` with `maxRecordsPerEntry=10000`. Classify=HARD_NONE (cap violation). Verify: no entry created, no K0 path attempted; query falls through to `statement.execute(...)` like the prior NONE behavior. Existing T4h2 already covers part of this; T4k5 specifically asserts that HARD_NONE classify path does NOT take the K0_NONE populate branch.
