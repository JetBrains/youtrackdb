...
- **AGGREGATE_*** — single-aggregate SELECT shape (`SELECT <COUNT(*)|SUM(prop)|AVG(prop)|MIN(prop)|MAX(prop)|COUNT(DISTINCT prop)> FROM Class [WHERE simple-predicate]`), no GROUP BY, no HAVING, no expression in aggregate argument, no SKIP / LIMIT. `COUNT(DISTINCT prop)` classifies as `K0_NONE`: the engine composes `count` over the inner `distinct` filter as a plain ROW count (`count(distinct(v))` over `{10,20,20}` returns 3, verified empirically), so a true distinct-count replay would diverge from fresh execution, and the K0 version gate reproduces the engine's row count exactly. The distinct **value set** classifies separately as `DISTINCT_VALUES` (next bullet); other DISTINCT shapes (`COUNT(DISTINCT a+b)`) also classify as `K0_NONE`.
- **DISTINCT_VALUES** — the distinct value set of a single bare property, `SELECT distinct(prop)` or `SELECT DISTINCT prop`, carrying a deterministic ORDER BY on that column (no GROUP BY / LET / UNWIND / SKIP / LIMIT). Reuses the per-value RID buckets of the aggregate `AggregateState` (kind `AGGREGATE_COUNT_DISTINCT`), but the view emits the bucket keys as rows, sorted by the ORDER BY, instead of their count. A path/expression argument, multiple columns, or a missing/foreign ORDER BY routes to `K0_NONE`. This shape closes a latent bug: before it, a distinct projection fell through to RECORD and mis-reconciled (injected raw records without deduping) under an in-tx mutation. The ORDER BY is required because an unordered distinct's row order is storage-scan-dependent and a bucket replay cannot reproduce it.
...
**Before-state from membership, not `op.type` (D21 collapse safety).** Every aggregate `applyMutation` computes its transition from two facts rather than from `op.type`: `was_contributing = contributingValues.containsKey(rid)` (for `COUNT(*)`, `contributingRids.contains(rid)`), and `now_contributing = (status == DELETED) ? false : match_after`. Dispatch keys on the `(was_contributing → now_contributing)` transition; `status` is consulted only to fold `DELETED` into `now_contributing = false`, never as a stand-in for the before-state. This mirrors the RECORD path's `cached_at_build` and is required for the same reason: `addRecordOperation` collapse keeps a pre-populate CREATE labelled `CREATED` while bumping its version past `populateMutationVersion` (D21), so a `CREATED` op can carry a record already observed by the populate-time tap and already in `contributingValues`. Keying on `op.type` would misread it as brand-new and no-op on `match_after=false`, leaving a stale contributor and a wrong scalar / `extremumRid`. The membership test needs no stream-pull unification here: the blocking `AggregateCacheTapStep` observes every contributor at populate, so `contributingValues` is complete from populate, unlike the RECORD path's `cached_at_build`, which the lazy stream-pull leaves partial at view construction.
...
**DISTINCT_VALUES (D20, retargeted).** The original D20 targeted a scalar `COUNT(DISTINCT prop)`, but the engine computes `count(distinct(prop))` as a plain ROW count (verified empirically), so that scalar is unachievable without breaking `cache == fresh`; `COUNT(DISTINCT prop)` therefore stays `K0_NONE`. The per-value bucket machinery is retargeted to the distinct **value set** `SELECT distinct(prop)` / `SELECT DISTINCT prop` (single bare property, with a deterministic ORDER BY on the projected column), which the engine computes correctly. The cached entry carries `distinctBuckets: Map<Object, Set<RID>>` (a `LinkedHashMap`). Each distinct value maps to the set of RIDs currently contributing it; the `DISTINCT_VALUES` view emits one `{prop: value}` row per bucket key, sorted by the ORDER BY (a future `count(distinct)` lift could read `distinctBuckets.size()` instead, once the engine grows a native distinct count). The reverse lookup uses the existing `contributingValues: Map<RID, Object>` (storing the raw value per RID, needed for transition dispatch on UPDATED). Bucket keys use raw `Object.equals`/`hashCode`, mirroring `SQLFunctionDistinct.getResult`'s `LinkedHashSet<Object>` semantics, so `Long(5)` and `Integer(5)` ARE distinct buckets (matching storage); non-numeric values (strings, RIDs) follow the same rule trivially. The earlier "BigDecimal coercion collapses cross-subtype duplicates" design was abandoned together with D19's BigDecimal accumulator because it diverged from storage's `LinkedHashSet`-Object semantics.

`applyMutation` transitions for AGGREGATE_COUNT_DISTINCT:
- **F→F** — no-op.
- **F→T** (`was_contributing=false → now_contributing=true`, per the membership principle above) — compute `newKey` from the post-state, add `rid` to `distinctBuckets.computeIfAbsent(newKey, k -> new HashSet<>())`, store `contributingValues.put(rid, newKey)`. The scalar grows by 1 if the bucket was empty before, else stays.
- **T→F** (`was_contributing=true → now_contributing=false`) — look up `oldKey = contributingValues.get(rid)`, remove `rid` from `distinctBuckets.get(oldKey)`, if the resulting set is empty remove the bucket key from the map, then `contributingValues.remove(rid)`. Scalar shrinks by 1 if the bucket emptied.
...
Coverage delta: `COUNT(DISTINCT prop)` itself stays `K0_NONE` (the engine-row-count premise above). What D20 makes delta-reconcilable is the distinct **value set** `SELECT distinct(prop)` / `SELECT DISTINCT prop` carrying an ORDER BY, via `DISTINCT_VALUES`; a no-ORDER-BY distinct, a path/expression argument, or a multi-column distinct stays K0_NONE. Memory profile matches existing AGGREGATE_* entries: the bucket map is the same shape and order-of-magnitude as `contributingValues`.

**Why MIN/MAX recompute is amortized O(1) without the D14 sorted-value index.** Per the membership principle above, `applyMutation` for MIN/MAX derives `was_contributing` and `now_contributing`, then dispatches on the `(was_contributing → now_contributing)` transition; within a transition it uses `was_extremum = rid.equals(entry.extremumRid)` (RID identity, not `Number.equals`, to sidestep the cross-`Number`-subtype hazard). A collapsed CREATE carrying an already-contributing record gets `was_contributing=true`, so it flows through the `T→F` / `T→T` rows, never the `F→` rows; that is what closes the D21 hole. Of the cases below only two trigger the O(n) scan over `contributingValues`; the rest are O(1). For workloads where the extremum is stable (which dominates the Hub aggregate profile, `max(salary)` across 1000 employees with a rarely-changing holder), per-mutation cost amortizes to O(1).

| Case | Action | Cost |
|---|---|---|
| F→T + new value beats current scalar | `currentScalar = newValue; extremumRid = rid; contributingValues.put(rid, newValue)` | O(1) |
| F→T + new value does not beat | `contributingValues.put(rid, newValue)` (scalar unchanged) | O(1) |
| F→F | no-op | O(1) |
| T→F + `was_extremum=false` | `contributingValues.remove(rid)` (scalar unchanged) | O(1) |
| T→F + `was_extremum=true` | `contributingValues.remove(rid)` + scan remaining values for new extremum | **O(n)** |
| T→T + `was_extremum=false` + new value beats scalar | `currentScalar = newValue; extremumRid = rid; contributingValues.put(rid, newValue)` | O(1) |
| T→T + `was_extremum=false` + new value does not beat | `contributingValues.put(rid, newValue)` (scalar unchanged) | O(1) |
| T→T + `was_extremum=true` + new value stays in extremum direction (MAX `new ≥ old`; MIN `new ≤ old`) | `currentScalar = newValue; contributingValues.put(rid, newValue)` (`extremumRid` unchanged) | O(1) |
| T→T + `was_extremum=true` + new value lost extremum direction | `contributingValues.put(rid, newValue)` + scan for new extremum | **O(n)** |

Both O(n) cases share one structural property: the previous extremum holder either left (`T→F`) or dropped out of extremum direction (`T→T`), so the new extremum is unknown without scanning the remaining values. Every other transition has enough information from `was_extremum` plus a single comparison against `currentScalar` to update in O(1).
...
- **Aggregate result type** — `COUNT(*)` returns `Long`; `COUNT(DISTINCT prop)` is not cached as an aggregate (it is `K0_NONE`, served by replay / re-execution), and the `DISTINCT_VALUES` shape returns rows of distinct property values rather than a scalar. `SUM/AVG/MIN/MAX` return whatever the storage aggregate function returns; type evolves through `PropertyTypeInternal.increment` per D19, so cache replay matches fresh execution by construction. No pinned-return-type field; the accumulator type IS the result type, and evolves naturally with input promotion (Long+Integer→Long, Long+Double→Double, BigDecimal+anything→BigDecimal). The cached `Result` wrapping uses the same shape as fresh execution because both paths run the same arithmetic primitive.
...
- **D20 (retargeted) — the distinct value set `SELECT distinct(prop)` / `SELECT DISTINCT prop` (with ORDER BY) is cacheable via per-value RID buckets; scalar `COUNT(DISTINCT prop)` stays K0_NONE.** *Why retargeted:* the engine computes `count(distinct(prop))` as a row count (verified empirically), so a scalar distinct-count replay would break `cache == fresh`; the same `distinctBuckets: Map<Object, Set<RID>>` instead backs the `DISTINCT_VALUES` value-set shape, whose view emits the bucket keys as rows sorted by ORDER BY, reusing `contributingValues` for transition dispatch. This also closes a latent bug where a distinct projection classified as RECORD and mis-reconciled (injected raw records without deduping) under an in-tx mutation. *Risk:* high-cardinality DISTINCT grows linearly, bounded by `maxRecordsPerEntry`; bucket keys use raw `Object.equals` mirroring `SQLFunctionDistinct`'s `LinkedHashSet<Object>`, so `Long(5)` and `Integer(5)` are distinct buckets (matching storage); `COUNT(DISTINCT expr)` routes to K0_NONE.
...
- **Function names from the denylist** (matched by name): `sysdate`, `date` (zero-arg form), `uuid`, `eval`, `math_random`. One list spans both build-time function sources: builtins from `DefaultSQLFunctionFactory`, and reflectively-registered functions from `CustomSQLFunctionFactory` (`register(prefix, clazz)` exposes a class's public static methods as `<prefix>methodName`, so `Math.random()` is `math_random`, while `math_abs` / `math_log10` / the rest of `Math.*` are deterministic and stay cacheable). The denylist is fail-open (anything unlisted defaults to deterministic), so its completeness is enforced by the I5 enumeration test, which walks every registered function across all three factories and fails the build on an unclassified one. `eval` is listed because its argument is an opaque runtime-parsed string the walk cannot inspect.
...
The denylist is **fail-open**: a function the walk does not recognize defaults to deterministic, so a missed non-deterministic function caches its populate-time value and replays it within the transaction, producing a silent wrong result rather than a safe bypass. Completeness, not auditability alone, is therefore the property that has to hold. One name list covers both build-time function sources (builtins and reflectively-registered `math_*`), and the I5 enumeration test keeps it exhaustive by failing the build whenever a registered function in any of the three factories is neither denylisted nor marked deterministic, so the only way to silently miscache is to add a non-deterministic function and skip both the denylist entry and the test.

The alternative is an `isDeterministic()` predicate on `SQLFunction` (today the interface carries only `aggregateResults()` / `filterResult()`). An SPI has two genuine advantages a name list cannot match. First, co-location: the determinism decision sits on the interface a function author implements, beside `execute()`, instead of in a separate file they may never open. Second, UDF self-declaration: a stored `CREATE FUNCTION` could carry its own determinism flag, the one surface a name list structurally cannot reach. The catch is the default direction. An SPI is strictly safer only if `isDeterministic()` defaults to `false`; a `default true` shares the denylist's exact fail-open risk, and a `default false` over-bypasses every un-migrated function, a day-one caching regression across dozens of deterministic builtins until each is audited and flipped.

v1 keeps the denylist: the function set known at build time is small and stable, one name list plus the I5 enumeration test keeps it complete, and the residual gap an SPI would close (stored UDFs declaring their own determinism) is covered conservatively by the trust-deterministic + `NOCACHE` contract. Promoting that gap to an `isDeterministic()` SPI is the v2 lever, gated on the D13 Hub-replay measurement of whether real UDF determinism needs justify the API churn.
...
The remaining MATCH non-determinism surface is **fully covered** by `NonDeterministicQueryDetector`'s name denylist (`sysdate`, `uuid`, `eval`, zero-arg `date()`, `math_random`) and its context-variable list (`$now`, `$current`, `$currentMatch`, `$matched`, `$thread`, `$parent`, `$depth`). User-defined Java functions in MATCH pattern WHEREs are trusted as deterministic by default — same trust contract as SELECT, but with materially lower exposure given typical MATCH usage patterns are graph traversal over storage-resident state.

SELECT retains `NOCACHE` for its broader use cases the denylist does not cover: free-form projection queries calling a custom function the detector trusts as deterministic (`SELECT myUdf(name) FROM ...` style), `LET`-based opt-out where the LET expression embeds a custom function, and user-defined-function escape valves where the user knows their UDF is non-deterministic but the detector cannot see it.
...
### References
- **D6 — Non-determinism via a denylist AST walk plus the reused `noCache` hint.** *Alternatives:* a `SQLFunction.isDeterministic()` SPI (co-location at the function class plus UDF self-declaration, but added API surface and a fail-open `default true` unless it defaults to non-deterministic; full weighing in § Why a denylist); denylist plus opt-out (chosen); ignore the problem. *Rationale:* the non-deterministic function set known at build time is small and stable, so one `NonDeterministicQueryDetector.contains(stmt)` name walker handles both builtins and reflective `math_*`, and `SQLSelectStatement.noCache` already parses, so its semantics extend to "skip result cache". *Risk:* the name denylist is fail-open, so a new non-deterministic function (builtin or reflective `math_*`) needs an entry, enforced by I5's enumeration test across all three factories; user-defined Java functions cannot be inspected, so the documented escape valve is the `NOCACHE` hint; per-row context variables (`$current`, `$matched`, and the rest) are bypassed conservatively rather than reconciled, because `DeltaBuilder` does not replicate the executor's per-row `ctx.setSystemVariable` chain.
...
- **I4 — View output equals fresh-execution result composed with tx-delta-applied snapshot.** For every cacheable shape (RECORD, AGGREGATE_COUNT, AGGREGATE_SUM, AGGREGATE_AVG, AGGREGATE_MIN, AGGREGATE_MAX, DISTINCT_VALUES, MATCH Etap A, and MATCH_TUPLE_MULTI), a view constructed at moment T over the recordOperations snapshot returns the same sequence of `Result`s a fresh uncached execution at moment T would return against the same storage + tx state, honoring WHERE, ORDER BY, LIMIT, projection, and RETURN. For K0_NONE shapes, equivalence holds via D18's lookup-time version-mismatch invalidation: cache hits serve pure replay of an entry whose populate-time `tx.mutationVersion` matches the current value (so storage + tx state has not changed since populate); cache misses re-execute from scratch through the standard executor and produce identical output to an uncached call. Composition with D21's populate-version filter eliminates the populate-time double-application that would otherwise break this equivalence for cacheable shapes when populate happens after pre-existing tx mutations on the queried classes. Test (per shape): cache a query with all four mid-tx mutation patterns (no mutation, pre-populate mutation only, post-populate mutation only, both pre- and post-populate mutations on the same RID through addRecordOperation collapse), assert view output matches a parallel uncached `db.query(...)` issued at the same moment, across CREATED/UPDATED/DELETED scenarios. For MATCH_TUPLE_MULTI the scenario matrix must additionally include edge CREATE, edge DELETE, and edge-property UPDATE on a `.out/.in/.both(label)` traversal, an UPDATE that flips a non-participating record into a node WHERE (update-into-match), and a cross-class-dereference WHERE (`where:(assignee.name = ?)`) mutated on the dereferenced record. These are the cases a vertex-only matrix misses, and the gap the original edge-mutation bug slipped through. For AGGREGATE_* the collapse case must additionally include a pre-populate CREATE of a contributing record (for MIN/MAX, the extremum holder) followed by a post-populate UPDATE that breaks WHERE, plus one that drops the extremum holder's value below a non-holder; assert the cached scalar matches fresh per aggregate kind (COUNT, SUM, AVG, MIN, MAX), since a status-keyed dispatch would leave the stale contributor. (`COUNT(DISTINCT prop)` is K0_NONE, not a cached scalar; the distinct value set is covered by `DISTINCT_VALUES`, whose equivalence test crosses CREATE-new-value, CREATE-duplicate, delete-one-of-two-sharing-a-value, delete-last-contributor, WHERE-break, and value UPDATE.)
- **I5 — Cache only stores results of idempotent, deterministic statements.** Test: query with `sysdate()`, `math_random()`, and `noCache` hint; assert no entry is created. **Completeness test:** enumerate the registered functions across all three service-loaded `SQLFunctionFactory` implementations and assert each is classified. Every build-time-known function (builtins from `DefaultSQLFunctionFactory` and reflective `math_*` from `CustomSQLFunctionFactory`) must be on the denylist or in an explicit test-side known-deterministic allowlist, so a new non-deterministic function in either factory without a denylist entry fails the build. Stored functions (`DatabaseFunctionFactory`) fall under the trust-deterministic + `NOCACHE` contract, so the test asserts that policy rather than enumerating user names. This closes the gap that let `math_random` slip past a single-function bypass check.
