...
This design is for YouTrackDB query-engine maintainers and reviewers familiar with `FrontendTransactionImpl`, the SQL executor pipeline, and the existing `STATEMENT_CACHE` AST cache. A fresh reader who has not internalised the prior eager iteration of this design should read § Core Concepts before diving into the Parts.
...
Disabled by default behind `youtrackdb.query.txResultCache.enabled`. Two more knobs bound memory (`maxEntries`, `maxRecordsPerEntry`). Non-deterministic queries (sysdate, random, uuid, $now, $current, $currentMatch, $matched, $thread, $parent, $depth) are detected via a denylist AST walk and bypass the cache; `SQLSelectStatement.noCache` hint extends to opt-out per-query.
...
Companion: `design-mechanics.md` holds long-form pseudocode and exhaustive state-machine tables for § Lazy merge-on-read. Cross-references go one direction, `design.md → design-mechanics.md`.

The rest of the document is structured as: Core Concepts → Class Design → Workflow → Cache key composition → Pause/resume mechanics → Lazy merge-on-read → Cache invalidation → Non-determinism handling → Memory bounds → Concurrency and lifecycle → Invariants → Open questions deferred to execution.

## Core Concepts

Eight load-bearing terms appear throughout the design. Each entry pairs the term with what it replaces or how it relates to the baseline, so the delta from today's executor is visible at a glance, and points at the section that elaborates it.
...
**`K1` sharp-merge (prior iteration).** The eager design this proposal replaces. K1 mutated `entry.results` in place on every `addRecordOperation` and required live views to fail-fast with `IllegalStateException` when a mutation invalidated the position counter. Named here so the Overview phrase "no K1 dispatch" parses for readers who land cold; no K1 mechanism appears in v1. → § Overview §"Why this approach was chosen over eager".

**Etap A (single-alias MATCH CREATED).** Polish for "stage A". The track-6a delivery: `MATCH {as:u, class:X WHERE p} RETURN <projection of u>` folds to RECORD shape via a stored `returnProjector` that wraps a record into the same single-binding tuple the executor produces. Cacheable in v1. → § Lazy merge-on-read §"MATCH Etap A — RECORD-shape composition".

**Etap B (multi-alias MATCH CREATED).** Polish for "stage B". The track-6b partial delivery in v1: `MATCH_TUPLE_MULTI` covers DELETED + UPDATED via per-RID reverse index, while CREATED on a pattern class tombstones the entry and forces re-execution. Full constrained-pattern-walk discovery on CREATED is deferred to a separate ADR. → § Lazy merge-on-read §"MATCH multi-alias (partial Etap B in v1)".
...
**`effectiveFromClasses`.** Per-entry `Set<String>` carrying the full subclass closure of every class the query reads from (FROM target plus any MATCH pattern node `class:` annotations). Computed once at entry construction per D11, stable for the entry's lifetime per I8 (schema immutable per tx). `DeltaBuilder` uses it as the O(1) class filter on every `RecordOperation`. → § Class Design.
