...
Delta-build for MATCH Etap A is the RECORD path with the `returnProjector` applied to each inject-list entry. Equivalence vs fresh re-execution validated by Track 2's T6h test that runs the same MATCH twice (cache miss then hit + delta) and asserts result-set equality across CREATED/UPDATED/DELETED scenarios.
...
**TL;DR.** Three correctness-bounded scope cuts. Pagination falls back to a coarser caching mode that invalidates on any tx-write. Multi-alias MATCH CREATED discovery is deferred to a separate ADR. MIN/MAX recompute is O(n) at delta-build time when the cached extremum element leaves.
...
**Multi-alias MATCH CREATED deferred to separate ADR.** Single-alias MATCH CREATED folds to RECORD shape with a RETURN projector (Etap A). Multi-alias / cross-join / pattern-with-edges classifies as `MATCH_TUPLE_MULTI` and is cacheable for DELETED + UPDATED via per-RID reverse index; CREATED on a pattern class tombstones the entry and forces re-execution (partial Etap B in v1). Full constrained-pattern-walk discovery of new tuples on CREATED is a separate-ADR candidate using `MatchPrefetchStep` + `PREFETCHED_MATCH_ALIAS_PREFIX`. The work belongs in a dedicated ADR because constrained pattern walk + edge-CREATED dispatch is a substantial infrastructure piece, not a localized hardening fix.
