...
This design adds an opt-in **transaction-scoped result cache** keyed by parsed query AST + normalized parameters. The cache lives on `FrontendTransactionImpl` and is wiped on every transaction-end path (commit, rollback, close). Cache entries grow append-only after populate; observable state for any live view is frozen at view construction (invariant I7). Each `query()` call constructs a `CachedResultSetView` over the entry plus a **snapshot tx-delta-cursor** built from `FrontendTransactionImpl.recordOperations` at view-construction time. `view.next()` performs a sorted-merge between the cached list and the delta-cursor's skip-set + sorted inject-list. Mutations only ever grow `recordOperations`; the cache itself reacts only at the next `query()` call via fresh delta build.

**Populate-version stamping (D21)** anchors the model. Each `RecordOperation` carries a `version: long` from `tx.mutationVersion` at `addRecordOperation` time. Each entry carries `populateMutationVersion: long` from `tx.mutationVersion` at the moment cache miss begins driving the executor. `DeltaBuilder` filters `tx.recordOperations` by `op.version > entry.populateMutationVersion` — only post-populate mutations enter the delta. This eliminates the double-application that would otherwise occur because the YTDB executor is tx-aware: `RecordIteratorCollection` emits tx-CREATED records in its `nextTxId` phase and `FrontendTransactionImpl.loadRecord` returns tx-pending state, so populate already reflects every mutation up to `populateMutationVersion`. The cache content depends on when populate happened in the tx lifecycle, but `view.output = state-at-populate + delta-after-populate` always equals what a fresh uncached execution would return at the moment of `query()` (invariant I4).
...
Queries with `SKIP` or `LIMIT` in the AST classify as `K0_NONE` and cache under D18's `tx.mutationVersion` gate. The entry serves cache hits while no mutation has happened in the transaction; any mutation invalidates the entry and forces re-populate on the next lookup. Reconciling a `LIMIT`-bounded window with mid-tx mutations requires either over-fetch or cross-page entry sharing. The over-fetch approach broke for blocking-sort plans because `OrderByStep`'s bounded heap is sized at planner-construction time and ignores downstream `LimitExecutionStep` mutations. Cross-page sharing broke on SKIP boundaries when the entry didn't hold the required window. K0_NONE sidesteps both by serving cache hits only while no mutation has occurred and re-populating on any tx-write.
...
Three correctness-bounded scope cuts accepted for v1.

- **`SKIP`/`LIMIT` queries route through K0_NONE.** Paginated queries cache under D18's mutation-version gate. They hit cache on pure-read repetition and invalidate on the first tx-write. Coverage for the typical Hub "scroll a list with no intervening writes" pattern is preserved; coverage for "save then re-read the list" is lost (first read after save misses, repopulates, subsequent reads hit until next save). The earlier over-fetch + canonical-SKIP-key design was eliminated because `OrderByStep`'s planner-bound heap defeats over-fetch for blocking-sort plans and the cross-page sharing scheme assumes the entry holds rows beyond the original LIMIT, which it does not.
- **MATCH multi-alias CREATED (Etap B) deferred to separate ADR.** Track 6a Etap A handles single-alias MATCH CREATED by folding to RECORD shape with a RETURN projector. Multi-alias / cross-join / pattern-with-edges classifies as `MATCH_TUPLE_MULTI` (cacheable for DELETED + UPDATED via reverseIndex; CREATED on a pattern class tombstones the entry and forces re-execution). Full constrained-pattern-walk discovery on CREATED is a separate-ADR candidate using `MatchPrefetchStep` + `PREFETCHED_MATCH_ALIAS_PREFIX` for constrained pattern execution. The work belongs in a dedicated ADR because constrained pattern walk + edge-CREATED dispatch is a substantial infrastructure piece, not a localized hardening fix. See § Open questions deferred to execution.
...
classDiagram
    ...
    class AggregateState {
        ...
        -sumAccumulator: BigDecimal
        -scalarReturnType: Class
        -distinctBuckets: Map
        +observe(result) void
...
**TL;DR.** Key = `(SQLStatement, normalizedParams)` delegating equality to `SQLStatement.equals()`, which is already structural over target/projection/whereClause/groupBy/orderBy/unwind/skip/limit/fetchPlan/letClause/timeout/parallel/noCache (see SQLSelectStatement:380). Same-text queries from `STATEMENT_CACHE` return the same `SQLStatement` instance, so `CacheKey.equals` short-circuits on identity (D12) before any deep walk.
...
1. **Copy** — `deltaState = entry.aggregateState.copy()`. The copy is shallow-deep — new mutable containers (`contributingRids`, `contributingValues`, and for COUNT_DISTINCT the `distinctBuckets` map plus per-bucket Sets) but reuse of underlying RID, Number, and bucket-key references.
...
**Cross-subtype safety for SUM/AVG (D19).** AggregateState for SUM/AVG carries `sumAccumulator: BigDecimal` as the internal running total, separate from the publicly-replayed `currentScalar`. `observe()` and `applyMutation()` coerce every observed value via `new BigDecimal(value.toString())` (the only round-trip that preserves cross-`Number`-subtype mathematical equality) before adding to `sumAccumulator`. This eliminates the silent drift hazard from Java implicit widening when a property's stored type drifts across records (a column that holds `Long` for some records and `Integer` for others, or a `Long` value past `2^53 + 1` upcast to `Double`). `toResult()` coerces `sumAccumulator` back to the pinned `scalarReturnType` (captured at first `observe()`) so the consumer-visible type matches a fresh execution, per § Edge cases / Gotchas → Aggregate result type. AVG returns `sumAccumulator.divide(BigDecimal.valueOf(count), MathContext.DECIMAL64)` then coerces.

**AGGREGATE_COUNT_DISTINCT (D20).** For `COUNT(DISTINCT prop)`, the cached entry carries `distinctBuckets: Map<Object, Set<RID>>`. Each distinct value maps to the set of RIDs currently contributing it; the scalar is `distinctBuckets.size()` (recomputed at `toResult()`, not maintained as a separate counter). The reverse lookup uses the existing `contributingValues: Map<RID, Object>` (storing the bucket key per RID, needed for transition dispatch on UPDATED). Numeric bucket keys are normalized via the same BigDecimal coercion D19 uses so `Long(5)` and `Integer(5)` collapse to one bucket; non-numeric values (strings, RIDs) are stored raw and rely on Java's `equals` semantics.
...
- Edge-CREATED dispatch (creating a new edge between existing vertices can introduce new tuples in a multi-alias pattern). Edge records also flow through `addRecordOperation`. The tombstone trigger covers an edge-CREATED only when the edge class is named via `class:` on a pattern path item (e.g., `.outE() { as: e, class: E } .inV()`). The common `.out('label')` syntax binds the edge label, not the edge class, so default-class edges (`E` or label-typed subclasses not appearing in any `class:` annotation) slip through without triggering tombstone. This gap is part of the separate-ADR scope for full Etap B CREATED-discovery, which adds an edge-CREATED dispatch hook on `addRecordOperation` to close it.
...
- **Aggregate result type** — `COUNT(*)` and `COUNT(DISTINCT prop)` return `Long`; `SUM/AVG/MIN/MAX` return the property's pinned `Number` subtype, captured at first `observe()` as `scalarReturnType`. The cached `Result` wrapping needs the same shape on replay as a fresh execution: SUM/AVG accumulate internally in `BigDecimal` per D19, and `toResult()` coerces back to `scalarReturnType` so the consumer-visible type matches a fresh execution. Don't coerce everything to `double`; the property's pinned type is what flows through.
...
### References
- D-records: D5-lazy, D8-lazy, D9 (deterministic ORDER BY admission), D11 (effectiveFromClasses closure), D14 (MIN/MAX sorted-value index — v2-deferred), D15 (snapshot-at-construction), D19 (BigDecimal accumulator for SUM/AVG), D20 (AGGREGATE_COUNT_DISTINCT shape)
...
**TL;DR.** Ten load-bearing properties the v1 implementation must hold: I1 (clear on every tx-end), I2 (mutation-path thread-affinity), I3 (paused-stream lifetime ≤ entry lifetime), I4 (view output equals fresh-execution composed with tx-delta-applied snapshot for every cacheable shape + K0_NONE via invalidate-and-re-execute), I5 (no caching of non-deterministic or `NOCACHE`-hinted queries), I6 (idempotent tx-end clear under cross-thread invocation), I7 (view's deltaCursor is immutable post-construction; recordOperations growth doesn't affect live views), I8 (schema immutable per tx — enforced upstream), I9 (view output cardinality matches uncached path under LRU pressure via live-view pinning), I10 (cache is transparent to the user — enable flag changes performance, not semantics). Each invariant carries an explicit test assertion in the track that introduces the relevant primitive.
...
### References
- D-records: D8-lazy (Etap A in scope; Etap B deferred to separate ADR), D13 (Hub-replay), D14 (MIN/MAX sorted-value index — v2-deferred, measurement-gated), D18 (K0-version-fallback for NONE shapes — covers SKIP/LIMIT in v1)
- Tracks: T8 (JMH + metrics), T6a (Etap A delivery), T6b (partial Etap B delivery)
