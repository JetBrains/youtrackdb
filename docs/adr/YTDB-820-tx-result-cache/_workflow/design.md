...
- **Aggregate result type** — `COUNT(*)` and `COUNT(DISTINCT prop)` return `Long`. `SUM/AVG/MIN/MAX` return whatever the storage aggregate function returns — type evolves through `PropertyTypeInternal.increment` per D19, so cache replay matches fresh execution by construction. No pinned-return-type field; the accumulator type IS the result type, evolves naturally with input promotion (Long+Integer→Long, Long+Double→Double, BigDecimal+anything→BigDecimal). The cached `Result` wrapping uses the same shape as fresh execution because both paths run the same arithmetic primitive.
