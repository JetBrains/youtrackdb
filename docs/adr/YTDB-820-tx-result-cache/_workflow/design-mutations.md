...
## Mutation 35 — 2026-06-08 — content-edit (design.md)

**Diff summary**: The non-determinism denylist named functions that do not exist and missed the one that does. `random`, `currentTimeMillis`, and `nanoTime` are not registered SQL function names: `Math.random()` is exposed by the service-loaded `CustomSQLFunctionFactory` (reflective `register("math_", Math.class)`) under the lowercased name `math_random`, and `System.currentTimeMillis`/`nanoTime` are unreachable because only `Math.class` is registered reflectively. So `SELECT math_random() FROM Foo` classified as RECORD shape and cached its populate-time value, replaying a stale result on repeat within the same transaction — the fail-open failure mode the PR-review finding flagged, made concrete (verified against `CustomSQLFunctionFactory.java:25/35/55`, `CustomSQLFunctionsTest:14`, and the `META-INF/services` registration). Seven coordinated edits across design.md: corrected both denylist enumerations (§ Non-determinism handling line 655 and § MATCH NOCACHE asymmetry line 683) to `sysdate` / zero-arg `date()` / `uuid` / `eval` / `math_random`; added a caveat that the walk matches by exact name and is therefore blind to reflectively-registered functions, with `eval` kept because its argument is an opaque runtime-parsed string the walk cannot inspect; updated four illustrative prose enumerations (§ Overview, § classify note, the SELECT-NOCACHE sentence, I10) to `math_random`; and extended invariant I5 with a completeness test that enumerates registered names across all three service-loaded `SQLFunctionFactory` implementations (`DefaultSQLFunctionFactory`, `CustomSQLFunctionFactory`, `DatabaseFunctionFactory`) so a future non-deterministic function added without a denylist entry fails the build instead of silently miscaching. The mechanism stays a denylist (the original D6 choice); this edit makes its names correct and adds the missing completeness backstop.

**Mechanical checks** (target=design, scope=whole-doc): PASS — 0 blockers, 0 should-fix after iteration, 1 suggestion (`per-section-length` on § Lazy merge-on-read, pre-existing, under the 300 cap).

**Cold-read** (scope: whole-doc): PASS. Whole-doc escalation fired (35th design-touching mutation; periodic N=5 counter). The reviewer confirmed both denylist enumerations name the identical set, all four prose enumerations use `math_random`, no stale bare `random` survives in a denylist context (the only residual `random` token is the intentional "reachable as `math_random` (not `random`)" correction at line 655), and the new caveat plus the I5 completeness test reinforce rather than contradict the D6 risk note (line 696) and the "Why a denylist" rationale (lines 661-664); no design.md ↔ design-mechanics.md drift.

**Findings**:
- should-fix (mechanical, iter 1): `dsc-ai-tell` em-dash density on I5 (line 801) — 3 em dashes (the `I5 —` invariant-naming dash plus a balanced aside pair around the factory list) → RESOLVED by replacing the aside's em-dash pair with parentheses, leaving only the single convention dash.
- suggestion (mechanical): `per-section-length` on § Lazy merge-on-read → INFORMATIONAL, pre-existing, under the 300 cap, out of scope for this edit.

**Iterations**: 1 of 3 (PASS — seven-hunk fix applied, the em-dash should-fix cleared on the first iteration, mechanical re-run and whole-doc cold-read both clean).

## Mutation 36 — 2026-06-08 — content-edit (design.md)

**Diff summary**: Answered the PR-review comment in full and hardened the denylist as far as a name/type check can go, on two threads. (1) Mechanism: the reflective function category (`SQLStaticReflectiveFunction`, the `CustomSQLFunctionFactory.register(prefix, clazz)` wrapper, `math_*` today) moved from name-listing `math_random` to a type-based rule — any reflective function is bypassed unless its name is on a deterministic-reflective allowlist — which flips the open-ended reflective surface from fail-open to fail-safe (a future `register("foo_", Foo.class)` defaults `foo_*` to bypass until allowlisted). The builtin name denylist narrows to the closed, stable set `sysdate` / zero-arg `date()` / `uuid` / `eval`. (2) Rationale: § Why a denylist was rewritten from a two-line API-cost argument into an honest three-paragraph weighing that states the denylist is fail-open and completeness (not auditability alone) is the property that must hold, weighs the `isDeterministic()` SPI's co-location and UDF-self-declaration advantages against its default-direction problem (strictly safer only if it defaults to `false`; `default true` shares the fail-open risk, `default false` over-bypasses every un-migrated function), and keeps the denylist for v1 because the reflective conservatism plus the I5 enumeration test recover most of the SPI's safety, deferring an SPI to v2 gated on D13. D6's Alternatives/Rationale/Risk, the § MATCH NOCACHE asymmetry coverage sentence, and I5's completeness test were updated to the same split-mechanism story (builtin = name/fail-open/I5-backstopped; reflective = type/fail-safe; stored UDF = trust + `NOCACHE`/v2-SPI). Five hunks, plus a sixth iterate-fix for a contradiction the rewrite surfaced.

**Mechanical checks** (target=design, scope=whole-doc): PASS — 0 blockers, 0 should-fix, 1 suggestion (`per-section-length` on § Lazy merge-on-read, pre-existing, under the 300 cap). No em-dash findings — paragraph-level discipline held across the rewritten prose.

**Cold-read** (scope: whole-doc): NEEDS REVISION → PASS after 1 iteration. The reviewer confirmed the split-mechanism story is consistent across the TL;DR, § Why a denylist, D6, the MATCH section, and I5, and that every illustrative `math_random` mention outside the contradiction spot correctly frames it as bypassed-via-reflective-rule rather than a named builtin entry. The one should-fix was a coverage contradiction the rewrite sharpened; a focused re-check confirmed the fix resolved it without introducing a new inconsistency.

**Findings**:
- should-fix (cold-read, iter 1): § MATCH NOCACHE asymmetry listed `SELECT sysdate(), math_random() FROM ...` as a NOCACHE use case "the denylist does not cover", contradicting the just-rewritten sentence two paragraphs up that says the reflective bypass covers `math_random` (and `sysdate` was always name-covered) → RESOLVED by replacing the example with a custom function the detector trusts as deterministic (`SELECT myUdf(name) FROM ...`), so the NOCACHE use-case list now names only genuinely-uncoverable UDF cases.
- suggestion (mechanical): `per-section-length` on § Lazy merge-on-read → INFORMATIONAL, pre-existing, out of scope for this edit.

**Iterations**: 1 of 3 (PASS — five-hunk rationale + mechanism edit applied, the cold-read's coverage-contradiction should-fix cleared on the first iteration, mechanical re-run and focused cold-read re-check both clean).

## Mutation 37 — 2026-06-08 — content-edit (design.md)

**Diff summary**: Consolidated the non-determinism handling from the Mutation 36 hybrid (three mechanisms with opposite fail directions: builtins by name / fail-open, reflective by type / fail-safe, UDF by trust) into one uniform name denylist, on a design-taste call that one consistent mechanism beats the split. The reflective type-based fail-safe rule was reverted: `math_random` is again a named denylist entry alongside `sysdate` / zero-arg `date()` / `uuid` / `eval`, and the open-ended reflective surface is kept complete by the I5 enumeration test (which already converts fail-open into a build break for every factory) rather than by a second type-based mechanism. The rationale: the type rule bought only marginal safety over what the enumeration test already guarantees, at the cost of a second opposite-direction mechanism and an allowlist to maintain. Six hunks reframed the TL;DR (two bullets back to one), § Why a denylist, the § MATCH NOCACHE coverage sentence, I5's completeness test, and D6's Rationale/Risk to the single-mechanism story. The honest rationale from Mutation 36 was preserved, only reframed off the hybrid: § Why a denylist still states fail-open as the axis, weighs the `isDeterministic()` SPI's co-location + UDF-self-declaration advantages against its default-direction problem, and defers the SPI to v2 gated on D13. Stored UDFs remain the one documented exception (trust-deterministic + `NOCACHE`) that only a runtime SPI could fold into the same mechanism.

**Mechanical checks** (target=design, scope=whole-doc): PASS — 0 blockers, 0 should-fix, 1 suggestion (`per-section-length` on § Lazy merge-on-read, pre-existing, under the 300 cap). No em-dash findings.

**Cold-read** (scope: focused on the non-determinism story): CONSISTENT / PASS, 0 iterations. The reviewer confirmed one mechanism is told consistently across the TL;DR, § Why a denylist, the MATCH coverage sentence, D6, and I5; `math_random` is framed as a named denylist entry everywhere; the honest rationale (fail-open axis, SPI weighing, v2 deferral) is intact; and no dangling reference to the removed reflective type-rule survives — the sole remaining "allowlist" token is the legitimate test-side known-deterministic allowlist inside I5's enumeration backstop.

**Findings**:
- suggestion (mechanical): `per-section-length` on § Lazy merge-on-read → INFORMATIONAL, pre-existing, out of scope for this edit.

**Iterations**: 0 of 3 (PASS — six-hunk consolidation applied, mechanical whole-doc and focused cold-read both clean on the first pass).
