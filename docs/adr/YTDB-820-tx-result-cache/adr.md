...
- **`QueryResultCache`** (new) — the per-transaction access-ordered `LinkedHashMap` LRU with `nonCacheableKeys`, the `k0Strikes` map, the lookup-level `inFlightLookup` guard, view-pin-aware eviction, and idempotent `clear()`.
...
**Re-entrancy guard model (two guards as-built).** The cache code path is bracketed by two distinct guards, not one, and neither pre-existed in the codebase. A transaction-level `cacheCodeDepth` counter on `FrontendTransactionImpl` brackets the whole lookup-and-build scope (and, per row during iteration, the lazy stream pull and delta merge through `CachedResultSetView.hasNext`); a lookup-level `inFlightLookup` boolean on `QueryResultCache` guards the bare `lookup` call. A `query()` issued from a UDF in a WHERE clause sees `cacheCodeDepth > 0` and bypasses the cache. The session releases the depth guard unconditionally in a finally after the synchronous scope; the view re-enters and exits it per row, so the guard is held only over the row-production windows and a `query()` issued by user code between two `next()` calls still uses the cache. An earlier framing of this guard as pre-existing infrastructure was wrong; both guards are new in this work.
...
- **Invariant 9 (I9) — View cardinality matches the uncached path under LRU pressure.** `liveViewCount` pins a mid-iteration entry; eviction skips a pinned eldest.
...
- `GlobalConfiguration` → four `youtrackdb.query.txResultCache.*` knobs; `CoreMetrics` → five global `QUERY_CACHE_*_RATE` counters bridged from the per-transaction `QueryCacheMetrics`.
