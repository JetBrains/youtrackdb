...
- **The structural findings** — edge-cases sub-section, References footer, sibling consolidation, TL;DR, length budgets, `Mechanics:` link resolution.
...
Emit exactly the comprehension-assessment, mental-model verdict, structural-findings, and verdict Markdown defined in `prompts/design-review.md § Output format`, minus the prose-AI-tell finding rows (you produce none). Apply the house-style AI-tell subset to your own finding prose.
