# Two-role authoring loop for readable design docs — Final Design

## Overview

Before this change one agent authored `design.md` with the whole planning
conversation loaded in its context, and one cold-read reviewer then checked the
draft for comprehension, structure, decision absorption, and prose readability
all in a single pass. Two things let dense prose survive. The reviewer read the
research log to run its absorption check, so it was no longer a cold reader when
it judged "could a fresh reader follow this." And the prose-readability axis was
one of five jobs in that pass, so it got a sliver of attention.

The two-role authoring loop replaces single-agent authoring plus the diluted
pass with a generate-then-verify loop. A code-grounded **author** sub-agent
drafts the doc for a reader who has only the finished document. A dedicated
**cold readability auditor** then reports every passage that reader cannot
reconstruct. The loop repeats author-then-audit until the auditor is clean.
...
Several existing pieces changed to fit. The comprehension reviewer
(`design-review.md`) lost the prose axis and the log read, so it became a
...
Phase 4 final-design step gained a doc-against-episodes fidelity check. New agent
definitions under `.claude/agents/` carry the roles with minimal tool
allow-lists. The research-log read-scope invariants (S2/S3) got the wording
updates these moves implied.
...
change strips its prose axis and its log read. It keeps the comprehension
questions, the structural findings, and whole-doc navigability, and it judges
only the doc. Replaces the old warm multi-axis reviewer. → Part 2
...
covers the residual. Replaces a PSI-only comparison against code. → Part 2
...
classDiagram
    ...
    CreateFinalDesign --> EditDesign : routes phase4-creation
```

The author is the only writer; the auditor, absorption check, comprehension
review, and fidelity check are read-only. The auditor and the round's second
check both run every round of the inner loop; the comprehension review runs once
as the outer gate after the loop converges. `edit-design` is the direct spawner
on both the Phase 1 and the Phase 4 design paths: it spawns the author, the
auditor, the per-round second check, and the comprehension gate, and it selects
the second check by mutation kind — the warm absorption check at
`phase1-creation`, the fidelity check at `phase4-creation`. `create-final-design`
does not spawn the sub-agents itself; it routes `phase4-creation` through
`edit-design`, which is why the fidelity check is an `edit-design` spawn and not
a `create-final-design` spawn. The fidelity check replaces absorption on the
Phase 4 path because at Phase 4 the source of truth is the as-built code and its
episodes, not the research log.

The five roles are realized as named agent definitions: `design-author`,
`readability-auditor`, `absorption-check`, `comprehension-review`, and
`fidelity-check`. The absorption check and the fidelity check run on Sonnet
(cheap two-way text matching); the author, the auditor, and the comprehension
gate run on the workflow default model. The comprehension review's allow-list is
`Read, Grep` rather than `Read`-only, so it can resolve the cited links its own
reading rules name (D20); the auditor owns prose readability, so the
comprehension reviewer judges the doc alone.

### Decisions & invariants
- D-records: D3, D4, D5, D7, D10, D13, D14, D20.
- Invariants: S1.
...
check is absorption against the track decision records. On the Phase 4 path
`edit-design` runs the same loop with the author seeding from the episodes and
as-built code, and the second check is the fidelity check rather than absorption.
...
(one Part of that run went 19 to 6 to 3). The reason: much residual density was a
...
access for every round, not only the first. As realized, the author's allow-list
is `Read`, `Write`, `Edit`, `Bash`, and the mcp-steroid PSI tool surface (the
`mcp__localhost-6315__*` wildcard, the one form that covers PSI execution,
project preflight, and resource fetch).
...
The auditor is realized as the `readability-auditor` agent definition so its tool
allow-list can be cut to `Read` and `Grep`. It needs no Write, no Edit, and no
PSI, because it judges text it is handed.
...
This Part covers how the existing comprehension reviewer changed once the auditor
...
structural findings, and whole-doc navigability, and it judges only the doc, so its
...
Before this change the same agent ran the comprehension questions, the structural
checks, the absorption cross-check, and the prose-readability additions. The prose
additions got a sliver of a multi-axis pass, the diluted-pass failure mode this
change exists to fix. And the absorption cross-check made the agent read the
research log, so the comprehension verdict came from a reader who was no longer
cold.
...
comprehension reviewer runs on the design creation kinds, the Step 4b track
cold-read, and (as a prose-owner safeguard) on `design-sync`, so the auditor takes
the prose axis on all of them, and the comprehension reviewer re-runs it nowhere.
The invariant: no surface runs the prose axis on both the auditor and the
comprehension reviewer. For the lighter `design-sync` mutation kind, which does not
spawn the full inner loop, the as-built wiring runs one cold `readability-auditor`
prose pass alongside the comprehension gate, so the auditor stays the single prose
owner there too; the one-owner-per-surface rule holds either way.
...
The comprehension reviewer is realized as the `comprehension-review` agent
definition. Its allow-list is `Read, Grep`, not `Read`-only (D20). The gate's own
reading rules direct it to resolve `**Full design**` links and read cited
`house-style.md § <heading>` sections, the same grep-a-cited-section pattern the
sibling cold roles (`readability-auditor`, `absorption-check`) already carry. A
`Read`-only gate cannot run those resolutions or its own structural checks.
Adding `Grep` is a read-only expansion that touches no invariant: S1 governs the
auditor not reading the research log, not the comprehension gate's tool surface.
The reviewer's coldness is a property of what it judges, the doc and never the
research log, which `Read, Grep` preserves.
...
### Decisions & invariants
- D-records: D5 (one cold comprehension-and-structure review at creation; the warm reviewer is retired), D8 (the five human-reader checks split by context need between the sliced auditor and the whole-doc reviewer), D9 (the auditor owns the prose AI-tell axis on every prose-checked surface; the comprehension reviewer runs it nowhere), D20 (the comprehension-review allow-list is `Read, Grep`, not `Read`-only: Grep resolves the cited links and sections its reading rules name, a read-only expansion that touches no invariant).
...
cheap shape as absorption, so the per-round cost stays low. The fidelity check is
realized as the `fidelity-check` agent definition with allow-list `Read` plus the
mcp-steroid PSI surface, on Sonnet — the same cheap-text-matching tier as the
absorption check it parallels.
...
The kind-keyed swap of fidelity-over-absorption lives in `edit-design`, not in
`create-final-design`. `create-final-design` routes `phase4-creation` through
`edit-design` and threads the fidelity check's caller-supplied inputs — the
episodes path, the frozen `design.md` for the PSI residual, and the comprehension
gate's `output_path`. Those paths derive from the design-directory offset rather
than the plan directory, which keeps the fidelity contract inside the narrow
`edit-design` boundary.
...
(review) spawns the auditor and the per-round second check each round and, after
the loop converges, the cold comprehension gate; Step 6 is the bounded inner loop.
...
auditor plus the second check) followed by the cold comprehension gate. The
iterate step is the dual-clean loop. So `edit-design` becomes the multi-agent
...
`create-final-design.md` routes `phase4-creation`. On the Phase 4 path
`edit-design` is the direct spawner of the author, the auditor, the fidelity check
(the kind-keyed second check), and the comprehension gate.
...
derivation became an author spawn plus the same inner loop, with the track files
...
auditor, one absorption check. The author's seed source is the frozen `design.md`
in `full` and the research log directly in `lite` and `minimal` — the heavy lift,
turning dense log prose into cold-readable track prose. Track absorption matches
log or design decisions against the track decision records, so the track cold-read
de-warms the same way the design one does. The auditor's per-round fan-out runs one
spawn per `plan/track-N.md` in track-number order, and the de-warmed comprehension
reviewer runs once after convergence with no prose axis.
...
The boundary existed to force a `/clear` so Step 4b derived the plan from the
...
committed design regardless of session. So the boundary stopped earning its keep,
and Step 4a now flows into Step 4b in one invocation.
...
path was re-specified to fire only on a dirty or absent plan after a
committed-clean design; the happy path no longer crosses the boundary, and Step
1c's auto-resume becomes crash-recovery-only. Both session-end commits now land
in one session (the `Add initial design` freeze-and-commit and the `Add initial
implementation plan` commit), with the design commit kept as the logical gate and
crash checkpoint.
...
boundary existed to keep apart, and the collapse regresses context isolation. The
author return contract returns a thin summary and the Step-4b wiring passes
`output_path`, so the orchestrator never receives the drafted doc; that
static confirmation is what licensed the collapse. A live-harness re-confirmation
against the running `Agent` tool is the one carried-forward item, settled on the
first live run after promotion. If by-reference cannot hold there, the boundary is
retained.
...
constraint that kept the new routine non-live on this change's branch, and the
...
sites; this change extends it to name the warm absorption agent as one of those
readers, landing the wording edit in `research.md` and its `design-document-rules.md`
restatement. S3 is the freeze-order gate: the cold-read may not run while a
log-adversarial entry is open.

S2 reads as "the log is read for decision content in exactly two places: at
...
site fairly covers, but that site historically meant the author or the cold-read
reviewer, not a separate absorption spawn. So this change extends S2 to name the
absorption agent as a sanctioned reader under the existing authoring site, which
keeps the site count at two. The wording edit landed in `research.md` (S2's
canonical home) and its `design-document-rules.md` restatement; `conventions.md`
needed no edit, because its cross-references name the two read *sites*, not the
readers, and stay accurate after the absorption-reader addition. Without the
`research.md` update a later reader who took S2 literally would see a third
log-reading agent as a violation.
...
### Decisions & invariants
- D-records: D5 (the de-warm that frees the comprehension review of its log read, the change these invariants rest on), D18 (extend S2 to name the warm absorption agent as a sanctioned log reader; the wording edit lands in `research.md` and its `design-document-rules.md` restatement, and `conventions.md` needs no change because its cross-refs name read sites, not readers).
...
**TL;DR.** This change modifies workflow machinery under `.claude/`, so its edits
accumulated in the staged subtree and the live workflow stayed at develop state
until the Phase 4 promotion. The branch therefore authored its own design with the
old single-agent routine. Dogfooding happens on real targets instead, with the
first design after promotion the full dogfood.

The literal dogfood, having the new routine author this change's own design, is
blocked two ways. Staging keeps the new routine non-live until promotion, so this
branch's Phase 1 ran the old single-agent flow. And the new routine's code is this
change's own output, built during implementation, so it did not exist at Phase 1
when the design was authored. The instinct is honored on real targets instead.

The same staging fact reaches this artifact: `design-final.md` itself is authored
by the old single-agent flow, because the new routine stays non-live until this
Phase 4 promotion commits the staged subtree onto the live tree. The first design
authored after promotion is the routine's first natural use, the full dogfood.

The matched risk category for this change is workflow machinery, and the densest
decisions are the read-scope invariants and gate semantics this very design states.
Validating only on a non-workflow target would leave a workflow-prose gap to
surface after promotion, costing a follow-up branch — so a workflow-prose target,
not a storage design alone, is the load-bearing dogfood, because workflow prose is
this change's actual domain.

### Edge cases / Gotchas
- The diagram format stays Mermaid; no ASCII or SVG toolchain enters this change.
...
- The YTDB-1128 / YTDB-1129 house-style rules are deferred to a separate PR. The
  auditor reads the live `house-style.md`, so it absorbs those rules whenever they
  land, with no rule list hard-coded here.
- PR-description readability is a non-goal of this change, tracked by a separate
  issue; the authoring loop's scope here is the design docs and the track files.

### Decisions & invariants
- D-records: D12 (keep Mermaid; no diagram-format change), D17 (dogfood on real targets including a workflow-prose target; self-wiring is blocked by staging and the temporal bootstrap), D19 (scope is the full two-role loop; the YTDB-1128 / YTDB-1129 house-style rules move to a separate PR).
...
roles became agent definitions, not general-purpose spawns. The auditor and
absorption check get `Read` plus `Grep`; the comprehension reviewer gets `Read`
plus `Grep` (D20); the author gets `Read`, `Write`, `Edit`, `Bash`, and the
mcp-steroid PSI surface; the fidelity check gets `Read` plus PSI.
...
The warm-up is a tunable cost lever, not a correctness dependency. This harness has
no foreground-delay primitive (`Bash` cannot hold a timed wait), so the concrete
wait mechanism is deferred to live wiring with a "disable the warm-up and pay N cold
prefixes" fallback. The warm-up-disabled path is the correctness baseline: the loop
produces the same result either way; the warm-up only lowers cost. The
byte-identical-prompt assumption that makes the shared body cache is confirmed
against the live `Agent` tool on the first live run.
...
- A heavy code-grounded author whose first turn runs long can push its cold write
  ...
  The exact plumbing is the most involved orchestration in the change and is
