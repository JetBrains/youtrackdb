...
### Step 1 — commit 92d7a9d8c9, 2026-06-04T03:16Z [ctx=safe]
**What was done:** Trimmed the inline startup-protocol recital in the staged `execute-tracks/SKILL.md` to a pointer. First touch copied the live SKILL verbatim into `staged-workflow/.claude/skills/execute-tracks/SKILL.md` (byte-identical), then replaced the recital (the divergence-check step, the handoff `ls -t` scan, and the State 0/A/C/D/Done routing sub-bullets) with a short block that defers all detection-and-routing to the staged `workflow.md § Startup Protocol` dispatch rule. The State C routing sub-bullets were removed in full, so no `state.substate`-free or `state.track`-assuming routing survives to contradict the dispatch rule. Preserved untouched: the phase-doc-loading map, the one-phase-per-session boundary, the user-interaction model, and the self-improvement-reflection step. File shrank from 147 to 115 lines.

**What was discovered:** The SKILL has no generated TOC region; its only top heading is the `## Reading workflow files (TOC protocol)` bootstrap block, so it is a `§1.8(d)` bootstrap-only file and the trim needs no reindex. For Phase C: the new pointer names the state set (State 0 / A / C / D / Done) and cites `workflow.md § Startup Protocol` plus `workflow-startup-precheck.sh --mode full`, so the track-level review should confirm that phrasing still matches the staged dispatch rule as it stands at review time (this medium step is a focal point on the cumulative diff).
