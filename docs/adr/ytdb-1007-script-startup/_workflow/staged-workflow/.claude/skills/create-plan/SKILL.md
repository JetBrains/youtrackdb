...
the gate's § Detection — which now runs the two-phase drift walk inside
`.claude/scripts/workflow-startup-precheck.sh` under `--mode full` and
reads the resulting `drift` JSON object — against the resolved
`<dir-name>` from the previous block (the script resolves the plan dir
from the active branch, so no inline `PLAN_DIR=` bash line runs here),
and follow its § Skip conditions, § No-drift normalization, and
