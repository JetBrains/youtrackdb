...
1. Harden the dual-clean readability-auditor loop across all six in-scope files (staged under `_workflow/staged-workflow/.claude/...`, applied operative-homes-first per `## Plan of Work`): `edit-design` Step 4 — deterministic partition + expected-slice-count self-check + `slice_count`/`total_lines` params (D1/D2); `edit-design` Step 6 — canonical convergence mechanism (section-keyed settled-state, anchor-folded hash, settled = returned-clean, drop/re-audit filter, never-clean-tail exit via `iteration_budget` + S5), reconciling the live monotonic-convergence paragraph (D4/D5/D8); `readability-auditor.md` — "Range-sliced fan-out" → hard requirement, whole-doc guard, the two new params + guard condition in the `## Inputs` block, orchestrator-side-settled-state cold-read note (D2/D3); `conventions-execution.md` §2.5 — generalize the third-scope home to plan-scoped authoring-loop review scaffolding (D6); the file relocation — `edit-design` Step 4 params + `phase4-creation` comprehension `output_path` + Step 6 resume round-count glob → `_workflow/reviews/`, and only the per-spawn params-file home in `create-plan` item 9 (D6); then the cross-references — `create-plan` item 9 convergence cross-ref preserving the existing per-file slicing rule (D5), `design-document-rules.md` one-line slicing cross-ref (D2), `readability-feedback` window/cap-value cross-ref (D2) — risk: high (workflow machinery: edits the dual-clean review-iteration control-flow protocol on the design and track paths)  [x] commit: e45dff3ce6

## Episodes
<!-- Continuous-log. Empty at Phase 1; Phase B appends per-step blocks. -->

### Step 1 — commit e45dff3ce6, 2026-06-25T16:57Z [ctx=info]

**What was done:** Hardened the dual-clean readability-auditor loop across the six staged workflow files (§1.7 staging), edited operative-homes-first. `edit-design` Step 4 gained the deterministic ~200-line / `##`-`# Part` / cap-6 slice partition, the whole-doc floor invariant, the expected-slice-count self-check, and the `slice_count` / `total_lines` spawn params (D1/D2). `edit-design` Step 6 gained the canonical section-keyed settled-state convergence mechanism — anchor-folded content hash, settled = returned-clean, drop-on-unchanged / re-audit-on-changed filter — and reconciled the live monotonic-convergence paragraph so the never-clean dense-but-acceptable tail reads as a designed `iteration_budget` + S5 exit (D4/D5/D8). `readability-auditor.md` turned "Range-sliced fan-out" into a hard requirement, added the whole-doc guard and the two params + guard condition to its `## Inputs` block, and stated that the settled-state is orchestrator-side, so the agent receives none (D2/D3). `conventions-execution.md` §2.5 generalized its third-scope home to plan-scoped authoring-loop review scaffolding (D6). The authoring-loop params and `output_path` files relocated to `_workflow/reviews/`; the cross-references landed in `create-plan` item 9 (convergence mechanism, parameterized), `design-document-rules.md`, and `readability-feedback` (partition value) (D2/D5/D6). The step landed across two commits — the initial implementation `2a8167a416` plus the review-fix `e45dff3ce6`.

**What was discovered:** Generalizing the §2.5 third-scope home meant widening the section's annotation axes — roles gained `orchestrator,final-designer`, phases gained `4` — on both the TOC row and the section comment, because the authoring-loop callers (`edit-design`, `create-plan` Step 4b) now write into that home at phases 1 and 4, not only the Phase-0→1 gate at phase 1. Step-level review (the full track-pass-equivalent selection a single-step-high track runs: 5 workflow reviewers) surfaced one blocker the change introduced. The new whole-doc guard read `slice_count` / `total_lines` unconditionally, but two of the auditor's three spawn sites — the track-path per-file fan-out and the `design-sync` single pass — omit both params, and `design-sync` is a single whole-doc pass over a `design.md` routinely over 300 lines, exactly the shape the guard flags as a wiring-error blocker. The fix makes the guard self-gating: it applies only when both params are present, and both spawn sites carry an inert-guard note. Ten further findings (4 should-fix, 6 suggestions) were house-style and instruction-completeness refinements; all eleven gate-verified at iteration 2.

**What changed from the plan:** None at the design level. The WP1 guard-applicability clause refines the agent-side guard contract D2 specified — D2 left the guard's behaviour on the param-absent spawn sites unstated — so the fix closes a gap within D2's intent rather than deviating from it.
