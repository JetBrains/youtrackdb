...
- [x] Track 2: Trim `ai-tells/SKILL.md` to procedural overlay
  ...
  >
  > **Strategy refresh:** CONTINUE — Track 2's stable reference contract makes Track 3's cross-references trustworthy. The 8 H2/H3 section names Track 3 will cite (`Audience-fit`, `Glossary-introduction`, `Why-before-what`, `Navigability`, `Overview concept-first`, `Edge cases sub-section required`, `References footer shape`, `Same-shape sibling consolidation`) all resolve live in `.claude/output-styles/house-style.md`. Track 3's verification entries should adopt Track 2's F1-pattern path anchor (`.claude/output-styles/house-style.md § <Section>`) for consistency.

- [x] Track 3: Refactor `prompts/design-review.md` to verification-only
  ...
  >
  > **Track episode:**
  > Track 3 trimmed `.claude/workflow/prompts/design-review.md` from 346 → 199 lines, replacing declarative rule statements with verification-only entries that cite `.claude/output-styles/house-style.md` by section name. All four Phase 1 acceptance-preserved blocks (mutation-kind instructions, Q1-Q7 comprehension questions, `phase4-creation` plan-deviation, Output format) remain byte-identical except for one documented carve-out at the example label `(a) Audience-fit:` → `audience-fit:` so the example matches the trimmed bullet shape; the surrounding markdown scaffold and the calling `edit-design` skill's parse contract are unchanged. Phase C iteration 1 surfaced and fixed five cross-step issues that step-level review could not see (PASS at gate-check, all VERIFIED): dropped four fabricated `dsc-*` rule IDs in the Structural findings block that did not exist in `design-mechanical-checks.py` and added a trailing bullet pointing at the script + `design-document-rules.md § Mechanical checks`; rewrote the unresolved `§ Reviewer tone` cross-reference at `## Tone and depth` to point at the Reviewer-tone note under `§ Human-reader cold-read additions`; rewrote the Reviewer-tone paragraph to drop an em-dash, drop a possessive on a section name, and tighten "name the failing audience" → "name the audience the prose fails"; added a `house-style.md reads` bullet to `## Reading rules` capping the conditional instant-axis cost at one cited section per finding (via grep + offset/limit Read) rather than the whole ~366-line file. **Cross-track context:** the 8 `house-style.md § <heading>` cross-references in `design-review.md` all resolve verbatim to on-disk H3 headings, so Track 4's `dsc-ai-tell` finding descriptions inherit a stable reference contract. Track 4's planned `dsc-ai-tell` rule will be the only real `dsc-*` rule in the script — Track 3 deliberately removed the fabricated namespace to avoid future confusion. **Known follow-up items** (pre-existing or wider scope, not Track 3 ownership): H1 Title Case at `design-review.md:1`; `mutation_kind` enumerates 10 values but only 3 have per-kind blocks; the `Verify <rule>` bullets are unconditional (M5 addressed read-volume but not per-mutation-kind gating); Inputs section dropped "absolute path to" qualifiers though the caller still passes absolute paths.
  >
  > **Track file:** `plan/track-3.md` (2 steps, 0 failed; 1 review-fix iteration)
  >
  > **Strategy refresh:** CONTINUE — Track 3 cleared the `dsc-*` rule namespace and verified the 8 `house-style.md § <heading>` cross-references resolve verbatim, so Track 4's `dsc-ai-tell` rule occupies an intentionally empty slot and inherits a stable cite contract for the 9 section names its findings will reference (`Tier 1 — hard ban`, `Banned sentence patterns`, `Em-dash discipline`, `Title Case headings forbidden`, `Signposting`, `Copula avoidance`, `Persuasive authority tropes`, `Hyphenated word-pair overuse`, `Fragmented headers`).

- [x] Track 4: Add `dsc-ai-tell` rule to `design-mechanical-checks.py`
  ...
  >
  > **Track episode:**
  > Track 4 added the `dsc-ai-tell` mechanical rule (9 patterns: Tier-1 vocab × 29 base words, negative parallelism, em-dash density, Title Case H2+, signposting, copula, authority tropes, hyphenated-pair comma cluster, fragmented headers) to `.claude/scripts/design-mechanical-checks.py` alongside a new `iter_paragraphs` helper (blank-line + bullet-starter segmentation with fence / References / table-row exclusion), the seeded fixture at `.claude/scripts/tests/fixtures/dsc-ai-tell-fixture.md` (9 positive + 3 negative cases), and the Python validation runner at `.claude/scripts/tests/test_dsc_ai_tell.py` (asserts ≥1 finding per pattern on the fixture, zero on negative cases, zero on three calibration ADRs). Each finding cites `house-style.md § <Section>`; `auto_applicable: false`; signature matches the bounded-aware sibling `check_dsc_parenthetical_asides` so `--scope=bounded` is honored. Phase C fanout: 4 dimensional reviewers (WC / WH / WB / WS, workflow-only diff per case 1 override); iteration 1 applied 5 in-scope fixes in one `Review fix:` commit `bcc01759a7` — added `dsc-ai-tell` to `design-document-rules.md § Mechanical checks` table, hardened the runner with `subprocess.run(..., timeout=60)` and `sys.executable`, tightened the runner docstring to drop the Tier-2 word "harness" and one em dash; all 3 dimension gate-checks PASS. **Cross-track impact for Phase 4 (`design-final.md`):** two implementation carve-outs deviate narrower than the canonical `house-style.md` rules — (a) Title Case fires only on 3+ Title-Case words so 2-word ADR-scaffold headings pass (Surprises T4); (b) em-dash density fires on 3+ em dashes or on 2 with a sentence terminator in the middle segment, exempting balanced parenthetical asides. Phase 4 either tightens the canonical wording to match implementation or accepts the drift as documented carve-outs. **Follow-ups deferred:** WC2 (pre-existing `edit-design/SKILL.md:215-217` drift widened by one bounded-aware rule — captured in Surprises T7 as out-of-scope), runner-CI wiring (no `.github/workflows/*.yml` invokes the script today — invocation-on-demand only).
  >
  > **Track file:** `plan/track-4.md` (3 steps, 0 failed; 1 review-fix iteration)
