...
- **Acceptance gate — zero grep matches.** After Track 1 lands, `grep -rnE "concise-doc|Concise Doc" .claude/ docs/ CLAUDE.md` must return zero results.
...
- [ ] Track 2: Trim `ai-tells/SKILL.md` to procedural overlay
  ...
  > **Scope:** ~2 steps covering the content trim and the ≤80-line verification
  > **Depends on:** Track 1
