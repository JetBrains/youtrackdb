...
Four discrete artifacts change as a group: `house-style.md` (renamed and broadened, absorbing rules from three other files plus 12 humanizer-gap patterns), `ai-tells/SKILL.md` (trimmed to ≤80 lines, procedural overlay), `design-review.md` (trimmed to ≤200 lines, verification-only), and `design-mechanical-checks.py` (gains `dsc-ai-tell`). Twelve string references to the old `concise-doc` / `Concise Doc` name are updated across `CLAUDE.md`, `.claude/skills/code-review/SKILL.md`, `.claude/agents/review-workflow-consistency.md`, and `.claude/agents/review-workflow-writing-style.md`. The `dsc-ai-tell` regex is calibrated against three known-good ADRs (`persist-visible-count`, `index-gc`, `non-durable-wow`) selected as the empirical false-positive baseline because they ship merged on `develop` and were written in the target style.
...
**TL;DR.** Twelve string references to `concise-doc` / `Concise Doc` are spread across five files: `CLAUDE.md` (2 references), `.claude/skills/code-review/SKILL.md` (1), `.claude/agents/review-workflow-consistency.md` (1), `.claude/agents/review-workflow-writing-style.md` (9 — heavily threaded), and the file `.claude/output-styles/concise-doc.md` itself (1, in the frontmatter `name:`). Plus a `/output-style concise-doc` slash-command reference in `CLAUDE.md`. The find-and-replace is mechanical but must be exhaustive: the acceptance criterion is **zero grep matches** across `.claude/`, `docs/`, and `CLAUDE.md`.
