...
Two rules harden the section-level lead (per YTDB-1163):

- **Self-contained lead.** The lead must be a plain claim a reader can parse before reading the body. Gloss any domain term in the lead, or fall back to why-before-what. An opaque forward reference ("This section establishes the three-tier `mutation_kind` dispatch") is a finding.
- **Body stands without the lead.** With the lead deleted, the body must read as complete prose. An anaphor opener ("This", "It", "Such") that resolves only into the lead is a finding.

These two rules bind every section lead across the workflow-markdown surfaces and the issue and PR BLUF, including surfaces that carry no `### Summary` block. Where a section opener also names what it builds on (the connect-backward opener), the self-contained plain claim comes first and the backward link rides as a subordinate clause of it or as the following sentence, never substituting for the claim.
...
Match a technical writer whose output leaves the reader with a working mental model: direct, no celebration of decisions ("This elegantly solves…"), no apologies, no enthusiasm. The author is not impressed with themselves and does not need to be impressed with you. The technical-writer voice governs the prose of design docs and track files; issue bodies, PR descriptions, and commit-message bodies stay in the terse BLUF register, because those surfaces are read to act and a narrative arc adds length without comprehension gain. Calibrate assumed knowledge to a mid-level Java database developer: take general Java, concurrency, and database internals (B+-trees, WAL, MVCC, latch-vs-lock, page caches, isolation levels, indexing, query planning) as given and never re-teach them, but explain everything YouTrackDB-specific (class roles, invariants, RID layout, the design's own mechanism) in full. Mid-level sets what the reader already knows; it is not a verbosity dial and never licenses tutorial framing for concepts in the assumed set.
...
BLUF-lead exemplar, an opaque forward-reference lead rewritten as a self-contained plain claim (the two § BLUF lead hardening rules):

> **Before (forward-reference lead):** "This section establishes the three-tier `mutation_kind` dispatch."
>
> **After (self-contained lead):** "A schema mutation routes by its `mutation_kind` tag — add-property, drop-property, or rename — to one of three handlers. (`mutation_kind` is the enum stored on each pending change.)"

The before form names `mutation_kind` and "three-tier dispatch" without a gloss, so a reader cannot parse the lead before the body; the after form is a plain claim that glosses the term at first use, so the body can then stand on its own with the lead deleted (the second rule).
...
Excluded: shape-exempt reference sections (Core Concepts, Class Design, Workflow, Part-level Summary) — these are intentionally mechanism-first.

### Navigability

Section headers communicate purpose, not just a mechanism name. Each section's opening sentence or Summary lets a skimming reader decide whether to drill in. Cross-references to deeper detail (Mechanics links, references to sibling Parts) are present where a reader would need them.
...
If three or more sibling sections share the same internal heading sequence, consolidate them under the consolidation form: one Summary + comparison table + per-instance short bodies. Three sibling sections each with "Problem", "Solution", "Trade-offs", "References" is the canonical trigger.
...
1. **Throat-clearing, trailing hedges, prompt-restating.** Cut.
2. **Analysis patterns.** Superficial -ing, passive voice, nominalization and placeholder words, broken grammar around code identifiers, hedge stacking, filler hedges, vague attribution, generic positive conclusions, persuasive authority ("at its core"), elegant variation, false ranges. Each gets the matching rewrite from § Banned analysis patterns.
3. **Boldface.** Excessive boldface → cap at 2 per section, on the phrases the reader needs by skimming.
4. **Structure.** Section length ≤200 words is a soft cap. Five template-bound categories are exempt regardless of length: ExecPlan structured-field paragraph blocks under `## Episodes`, edit-list subsections under `design-mechanics.md`, full state-machine tables under `design.md` or `design-mechanics.md`, file:line citation blocks under `design-mechanics.md`, and multi-step derivations under `design-mechanics.md`. The unit of evaluation is the smallest labeled block. For prose outside the exempt list, a >200-word unit is a finding only when it also contains padding — a pattern from § Banned sentence patterns, or restatement per § Elegant variation. Length alone is not a finding. Also check: no faux-symmetry; no bullet-everything; no inline-header lists outside genuine definition lists; no skipped heading levels; no fragmented headers (heading + ≤1-line paragraph with ≥50% content-word overlap); no run-on mechanism sentences or mid-clause file:line citations (present a multi-step sequence as a numbered list, fenced trace, or sequence diagram per § Mechanism traces and inline citations).
5. **Orientation.** Every prose surface, not only design docs: lead with the plain claim, gloss each project-specific entity at first use, linearize causal chains one link per sentence. A passage the § Voice and tone reader cannot follow without opening the code is a finding under § Orientation. The added words must be a definition or a causal link, never a hedge or restatement.
6. **Plain language.** Every prose surface: prefer the precise or common word, keep sentences short with one idea each, avoid idioms and ambiguous phrasal verbs, expand a non-floor acronym on first use, keep the grammar explicit (active subject-verb-object). Plain language governs general English only; it never simplifies technical content or re-teaches the § Voice and tone floor.
7. **Document shape (design/ADR only).** Overview concept-first, audience-fit, glossary-introduction, why-before-what, navigability, explanatory register, Edge cases sub-section, References footer shape, same-shape sibling consolidation per § Document-shape rules.
8. **BLUF.** The first paragraph states the decision or symptom directly. Section openers don't restate the section heading. The section lead is a self-contained plain claim, parseable before the body (gloss any domain term, or fall back to why-before-what); and the body stands as complete prose with the lead deleted (an anaphor opener resolving only into the lead is a finding).
9. **Paragraphs that don't add information beyond the previous one.** Delete.
