...
| §Structural findings (always check) | reviewer-design | 1,4 | Always-on checks: edge-cases, References footer, sibling consolidation, `### Summary`, length budgets, Mechanics. |
| §Output format | reviewer-design | 1,4 | Hybrid reading narrative plus comprehension-assessment, mental-model verdict, structural findings, and suggested fixes. |
...
3. **What is the load-bearing claim of the section that just
   changed?** Read the changed section's `### Summary`; restate in your
   own words.
...
<!-- roles=reviewer-design phases=1,4 summary="Always-on checks: edge-cases, References footer, sibling consolidation, `### Summary`, length budgets, Mechanics." -->
...
- **`### Summary` present** (a section-opening summary block; legacy `**TL;DR.**` / `### TL;DR` spellings still pass, D4); **Mechanism overview ≤300 lines** (warn 200); **Length budget** ≤2,000.
...
## Output format
<!-- roles=reviewer-design phases=1,4 summary="Hybrid reading narrative plus comprehension-assessment, mental-model verdict, structural findings, and suggested fixes." -->
...
**Hybrid output — reading narrative plus structured findings (D2).** As the
time-constrained reviewer (`comprehension-review`), you read the full
document in order once and open the output with a short **reading narrative**:
a few sentences on where your mental model came together and where you
stalled and had to re-read to follow the argument. The narrative is
meaningful precisely because it is a whole-read property — the orchestrator's
gate verdict and the author's rework seed both consume it. It is not free
prose beside the findings; every stall it names should either resolve into a
comprehension answer or a structural finding, or explain a `PARTIAL` / `NO`
mental-model verdict. **The narrative and every judgment stay scoped to
comprehension and the mental model** — where the argument became hard to
*follow*, not where the prose reads dense or terse. A stumble you would
phrase as a prose-quality verdict (over-dense / too-terse / hard-to-read)
belongs to the `readability-auditor` agent, which owns the prose AI-tell axis
on every prose-judged surface (S4); it never appears in your narrative or
your findings. At `scope=whole-doc` (and `target=tracks`, reading every track
section in order) the narrative covers the whole read; at `scope=bounded` it
covers the changed section and its surroundings — the only read that
...
<2-5 sentences: where the mental model came together and where you
stalled and had to re-read to follow the argument. Comprehension and
mental-model only — no prose-quality (over-dense / too-terse /
hard-to-read) verdicts; those are the readability auditor's (S4). At
`scope=bounded`, say the narrative covers only the changed section and
its surroundings.>
...
- **blocker** — comprehension fails, mechanical violation corrupts
  cross-refs, or section lacks `### Summary` / References footer / shape.
