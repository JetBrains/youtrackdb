...
docs/adr/<dir-name>/
  ...
  _workflow/
    ...
    design.md                     <- narrative: concept-first Overview
                                     ...
                                     diagrams, per-section ### Summary + mechanism
...
`house-style.md`'s path is itself on the §1.7:orchestrator,implementer,planner,final-designer:1,3A,3B,3C,4 staging surface: a
workflow-modifying branch stages its `.claude/output-styles/**` edits
under the plan's `_workflow/staged-workflow/` mirror like the rest of
the machinery, and reads resolve staged-first per the reads-precedence
rule there, so rewritten style rules bind the tiers below only after
the Phase 4 promotion lands them on the live path.
...
live `.claude/workflow/**`, `.claude/skills/**`, `.claude/agents/**`,
`.claude/scripts/**`, and `.claude/output-styles/**`
paths — plus the repo-root `CLAUDE.md` — under a plan-scoped prefix; those files are not `_workflow/**`
artifacts in the
...
`.claude/output-styles/**` — plus every edit to the repo-root
`CLAUDE.md` — under a
plan-scoped staging
subtree during
...
docs/adr/<dir-name>/_workflow/staged-workflow/.claude/output-styles/...
docs/adr/<dir-name>/_workflow/staged-workflow/CLAUDE.md
```
...
the rest of the machinery. `.claude/output-styles/**` and the repo-root
`CLAUDE.md` are in scope (D12) because the declarative style source
(`house-style.md`, `house-conversation.md`) and the project rules file
are behavior-bearing workflow machinery in the same sense: a branch that
rewrote them live would run its own later authoring — and every
concurrent branch — under the changed rules mid-flight. The repo-root
`CLAUDE.md` is the single staged path that does not sit under the
`.claude/` mirror; it maps to
`docs/adr/<dir-name>/_workflow/staged-workflow/CLAUDE.md`, directly
under the staging root. No other prefixes participate: workflow files
outside `.claude/workflow/`, `.claude/skills/`, `.claude/agents/`,
`.claude/scripts/`, and `.claude/output-styles/` — the repo-root
`CLAUDE.md` excepted — are not stageable under this convention; the
...
```
This plan is workflow-modifying: it edits .claude/workflow/**, .claude/skills/**, .claude/agents/**, .claude/scripts/**, .claude/output-styles/**, or the repo-root CLAUDE.md.
```
...
two-prefix `.claude/workflow/** or .claude/skills/**` list, the
three-prefix list before D14 added `.claude/scripts/**`, or the
four-prefix list before D12 added `.claude/output-styles/**` and the
repo-root `CLAUDE.md`) is
recognized identically to one carrying the current enumeration,
...
`.claude/agents/**`, `.claude/scripts/**`, `.claude/output-styles/**`,
or the repo-root `CLAUDE.md` but omit the marker forfeit
...
`.claude/skills/**`, `.claude/agents/**`, `.claude/scripts/**`, or
`.claude/output-styles/**` paths, or the repo-root `CLAUDE.md`.
...
- **Staged-content presence under
  `<dir-name>/_workflow/staged-workflow/`** — the `.claude/`
  subdirectory or a staged root `CLAUDE.md` file — drives the Phase 4
  promotion guard. The Phase 4 prompt at
  `prompts/create-final-design.md` checks for the `.claude/`
  subdirectory or the `CLAUDE.md` file at that specific level (a
  partially-stripped or empty
  `staged-workflow/` shell does not trigger promotion) and skips the
  promotion commit when both are absent. This signal
...
`.claude/agents/**`, `.claude/scripts/**`, and `.claude/output-styles/**`
path — and the repo-root `CLAUDE.md` — through a staging-aware
check:
...
`.claude/skills/**`, `.claude/agents/**`, `.claude/scripts/**`, or
`.claude/output-styles/**` file — or the repo-root `CLAUDE.md` — the
...
`.claude/skills/`, `.claude/agents/`, `.claude/scripts/`, or
`.claude/output-styles/` — and every write targeting the repo-root
`CLAUDE.md` — to the
...
to a given live path on the staged surface (`.claude/workflow/**`,
`.claude/skills/**`, `.claude/agents/**`, `.claude/scripts/**`,
`.claude/output-styles/**`, or the repo-root `CLAUDE.md`) finds no
staged copy at the
mapped staged path,
...
**Promotion is additive only.** The Phase 4 promotion (a `cp -r` for
the `.claude/` mirror plus an explicit `cp` for a staged root
`CLAUDE.md`, which sits outside that mirror) lays the staged
...
`.claude/workflow/**`, `.claude/skills/**`, `.claude/agents/**`,
`.claude/scripts/**`, or `.claude/output-styles/**` file
...
`.claude/agents/**`, `.claude/scripts/**`, `.claude/output-styles/**`,
or repo-root `CLAUDE.md` matches outside the Phase 4
promotion commit is
...
`.claude/workflow`, `.claude/skills`, `.claude/agents`,
`.claude/scripts`, and `.claude/output-styles` paths — and the
repo-root `CLAUDE.md` — after a
...
Step 6, not Step 4), so the staged-content guard
(`[ -d "$STAGED_DIR/.claude" ] || [ -f "$STAGED_DIR/CLAUDE.md" ]`)
