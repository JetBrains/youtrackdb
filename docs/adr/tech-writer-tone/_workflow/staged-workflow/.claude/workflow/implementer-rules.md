...
- **Path mapping for workflow-modifying plans.** Workflow-modifying
  ...
  `.claude/agents/**`, `.claude/scripts/**`, and
  `.claude/output-styles/**` edits — plus edits to the repo-root
  `CLAUDE.md` — under a plan-scoped
  staged subtree so
  ...
  `.claude/scripts/`, or `.claude/output-styles/` — and every write
  targeting the repo-root `CLAUDE.md` — to the corresponding staged
  path under
  `docs/adr/<dir-name>/_workflow/staged-workflow/.claude/...`. Staged
  paths mirror the live relative path under the `.claude/` prefix
  byte-for-byte; the repo-root `CLAUDE.md` is the one exception,
  mapping to `docs/adr/<dir-name>/_workflow/staged-workflow/CLAUDE.md`
  directly under the staging root (D12). A write to
  `.claude/workflow/X.md` rewrites to
  `docs/adr/<dir-name>/_workflow/staged-workflow/.claude/workflow/X.md`;
  a write to the repo-root `CLAUDE.md` rewrites to
...
`.claude/agents/**`, `.claude/scripts/**`, and
`.claude/output-styles/**` write — and every write to the repo-root
`CLAUDE.md` — to the staged
...
```bash
git diff --cached --name-only -- .claude/workflow/ .claude/skills/ .claude/agents/ .claude/scripts/ .claude/output-styles/ CLAUDE.md
```
...
refused identically. `.claude/output-styles/` and the repo-root
`CLAUDE.md` are in the pathspec per D12 — the declarative style
source and the project rules file stage like the rest of the
machinery, so a live edit to either is refused the same way. The
bare `CLAUDE.md` pathspec matches only the repo-root file; a nested
`CLAUDE.md` elsewhere in the tree is outside the staging surface
