...
| §Technical-writer voice (the internals-book rules) | planner,final-designer | 1,4 | The eight internals-book voice rules for mechanism prose, and the two precedence rankings over the shape rules. |
| §Dual-register: which blocks stay registry-terse | planner,final-designer | 1,4 | The register boundary: the voice governs mechanism prose; records, summaries, footers, and diagrams stay terse. |
...
| Per-section shape compliance | Per `^## ` section (excluding shape-exempt: Overview, Core Concepts, Class Design, Workflow, Part-level Summary): section-opening summary present — the canonical `### Summary` sub-heading, with the legacy bold-prefix `\*\*TL;DR\.\*\*` and `### TL;DR` spellings still accepted (D4) in the first ~10 lines; decisions-and-invariants footer present near section end. The footer name accepts **both** spellings (`### Decisions & invariants` or `### References`, and the bold-prefix `\*\*Decisions & invariants\.\*\*` / `\*\*References\.\*\*` forms) so the rename (D11) is backward-compatible — legacy designs carrying `### References` still pass (see § Per-section mandatory shape). |
...
| `dsc-ai-tell` AI-tell detection | Detects three `house-style.md` patterns whose textual fingerprint is reliable enough to flag mechanically: persuasive authority tropes (`§ Persuasive authority tropes`), fragmented-header chains (`§ Fragmented headers`), and inflated-abstraction labels in the subject slot ("The enabling primitive is …", "The key abstraction …"; the `## Overview` section is skipped because naming "the enabling primitive(s)" is the prescribed Overview element there) (`§ Banned analysis patterns`). The negative-parallelism, Title-Case-heading, and hyphenated-pair-cluster regexes were removed with their house-style rules (D1); the disguise-only patterns are no longer flagged. Each finding cites `house-style.md § <Section>` in its description so the reader can trace the rule back to its prose. The check is `auto_applicable: false` — every finding requires a human rewrite. Honors `--scope=bounded` on the `design.md` side in parallel with the D/S parenthetical asides row above: only paragraphs whose line falls within the changed section's range are flagged. |
...
- Each section's `### Summary` + mechanism overview is re-written to
...
instruction: *"Verify that every `### Summary` and mechanism overview in
...
Overview's structure roadmap. The doc's shape (`### Summary` blocks at section
...
- The section's intro paragraph (before the first concept) is the
  `### Summary`-equivalent — it states how many concepts are below and
  the meta-pattern (one paragraph each, definition + role +
  delta + pointer). Don't add a separate `### Summary` block.
...
### Summary
<≤5 lines, plain prose, what + why — a self-contained plain claim a
reader can parse before the mechanism (§ BLUF lead in
`house-style.md`; gloss any domain term the claim uses). No file:line
...
**Summary sub-heading (D4).** The section-opening summary is a
`### Summary` sub-heading (formerly the `**TL;DR.**` bold-prefix block).
The sub-heading separates the summary from the mechanism structurally, so
the doc stays skimmable end-to-end, and "Summary" is self-explanatory to a
reader for whom "TL;DR" is jargon. The mechanical check that enforces
summary presence accepts **both** spellings — the new `### Summary` and the
legacy `**TL;DR.**` / `### TL;DR` — so designs authored before this rename,
this branch's own frozen `design.md` among them, keep passing without a
backfill (the same both-spellings pattern the footer rename D11 set). New
`design.md` seeds use `### Summary`. The block stays plain-claim and
self-contained (§ Dual-register: which blocks stay registry-terse): it is a
lead a reader parses before the mechanism, not a first sentence the
mechanism completes.
...
## Technical-writer voice (the internals-book rules)
<!-- roles=planner,final-designer phases=1,4 summary="The eight internals-book voice rules for mechanism prose, and the two precedence rankings over the shape rules." -->

Design-doc mechanism prose follows the technical-writer voice of the
YouTrackDB internals book — the teaching book whose readers praised its
narrative — so a reader builds a mental model from the prose rather than
decoding a reference dump. Eight voice rules carry over from the book's
`BOOK_BRIEF.md`: five transfer verbatim, two are adapted from chapter scope
to `##`-section scope, and one is already an established design-doc rule.

| Voice rule | How it applies to a design section |
|---|---|
| Narrative, not reference (adapted) | Each `##` section is one complete argument — problem → forces → mechanism → consequences — while the document as a whole stays dip-in navigable. |
| Concrete before abstract (verbatim) | Open with the concrete case, then generalize; do not lead with the abstraction. |
| One concept per section (verbatim) | A section teaches one idea. (Interacts with sibling consolidation — see the precedence rankings below.) |
| Earn every name (verbatim) | Introduce a class or component by its role before its name; the name lands after the reader knows what it does. |
| Connect forward and backward (adapted) | Light prose links: the section opener names what it builds on, the closer names what depends on it. Each link cites the neighbor's section heading **verbatim** so it stays greppable. |
| Source citations stay precise `file:line` (already present) | Keep the existing `file:line` precision in citations; this rule is stated for completeness, not as a new demand. |
| Diagrams must teach (verbatim) | Every diagram earns its place by teaching something the prose cannot; pair it with prose (see § Rules). |
| No bullet-point fact dumps (verbatim) | Do not replace an argument with a bullet list of disconnected facts; bullets are for genuinely list-shaped content. |

Two of these outrank kept shape rules where they would otherwise contend.
State the ranking so the author and every style-enforcing reviewer read one
rule, not two contradictory targets:

1. **A worked-example opener beats the section-length heuristics.** In
   mechanism-overview prose, a worked example that opens with the concrete
   case is on the section-length cap's exemption list — neither the ~200-line
   warn nor the bias toward less text flags it. The anti-padding clause still
   applies **inside** the example: a worked example that teaches nothing is
   still padding.
2. **One-concept-per-section beats same-shape sibling consolidation when the
   siblings teach distinct concepts.** Consolidation (§ Consolidation form for
   sibling sections) still collapses same-shape **reference** siblings
   (comparison tables, catalogues) where no concept is being taught; it yields
   when three same-shaped siblings each carry a different idea.

The forward/backward links cite a neighbor heading verbatim, and a heading is
a moving target, so the `edit-design` mutation discipline carries the guard: on
any section-move, section-remove, or section-rename, re-read the neighbor
openers and closers so a prose link does not point at a heading that no longer
exists (`edit-design/SKILL.md § Step 1` → Link-staleness re-read; no mechanical
check catches a dangling prose link).

## Dual-register: which blocks stay registry-terse
<!-- roles=planner,final-designer phases=1,4 summary="The register boundary: the voice governs mechanism prose; records, summaries, footers, and diagrams stay terse." -->

The technical-writer voice governs the mechanism **prose** only. The
machine-facing skeleton of `design.md` stays registry-terse, because the frozen
`design.md` is also the track-derivation seed and dissolving its operative
content into a story arc would make it un-extractable for the cold derivation
spawn. The register boundary runs inside the document:

- **Decision records** keep the canonical four-bullet form (alternatives /
  rationale / risks / where-implemented), not narrative.
- **`### Summary` blocks** stay plain-claim and self-contained — a lead a
  reader parses before the mechanism, extractable in isolation, never a first
  sentence the mechanism completes (§ Per-section mandatory shape → Summary
  sub-heading; the YTDB-1163 self-contained-lead rule in `house-style.md
  § BLUF lead` applies here too).
- **Decisions-and-invariants footers** stay lists of D and S codes.
- **Diagrams** stay as diagrams.

A decision record rewritten as narrative, or a `### Summary` that runs into the
details, is a finding. Applying the narrative voice to these blocks is the
extraction hazard the boundary exists to prevent, not an improvement.
...
at the `design.md` Part 5 sub-section for the `### Summary`, which footers
...
- `design.md` keeps: Overview, Core Concepts (when applicable),
  all class / workflow diagrams, every `##` section's `### Summary` +
...
**`### Summary` exception.** When the section topic IS the consolidated
set of decisions (e.g., a Part titled "Volatile Statistics"
covering D27/D28/D29/D30/D31/D33/D34/D35/D38), the `### Summary` may
...
following the per-section mandatory shape (`### Summary` + mechanism
...
8. **Per-section shape is mandatory for content sections** —
   `### Summary` + mechanism overview + edge cases + references footer.
   The shape-exempt sections are Overview, Core Concepts, Class
   Design, Workflow, and Part-level `## Summary` sections — each
...
## Class Design
<Mermaid classDiagram(s) — each in a sub-section with `### Summary` +
...
### Summary
<≤5 lines — self-contained plain claim>
