...
9. **Author the track sections through the dual-clean loop (Step-4b loop,
   ...
   **no** `output_path`: the absorption cross-check moved off this role onto the
   ...
   deliberately: its return is the **hybrid** shape (D2) — a `## Reading
   narrative` plus the bounded comprehension verdict and a summary-shaped
   `## Structural findings` list, not the long-form structural
   detail `phase4-creation` persists, so the inline return stays small even
   on a wide `full` surface of N track files plus the frozen `design.md`. Accept
   the narrative in the parse: it reads every track section in order and stays
   scoped to comprehension and the mental model (never the prose AI-tell axis,
   which the `readability-auditor` owns on this track surface too, S4), so it is
   the track-authoring rework seed, not free prose to drop. A decision-shaped comprehension-gate finding re-opens the
   ...
   the comprehension gate re-runs only once the gate clears again; on that
   re-entry the parsed narrative rides into the track author re-spawn's flagged
   passages alongside the structural findings, so the re-ground targets the
   track sections where the mental model broke. **A
