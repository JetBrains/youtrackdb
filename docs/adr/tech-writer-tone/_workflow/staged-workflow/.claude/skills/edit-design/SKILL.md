...
Design, Workflow, and `### Summary`-shaped Part sections.
...
`### Summary` + mechanism overview + edge cases + references footer in
...
**Link-staleness re-read (D9).** The design-doc voice rules connect sections
with light prose links — the opener names what the section builds on, the closer
names what depends on it — and each link cites the neighbor's section heading
verbatim so it stays greppable. A heading is a moving target, so on any
`section-move`, `section-remove`, or `section-rename`, re-read the openers and
closers of the neighboring sections after applying the edit, and fix any prose
link that now points at a heading that moved, was removed, or was renamed. No
mechanical check catches a dangling prose link (it is not a `**Full design**` /
`Mechanics:` reference the shape checks resolve), so this re-read is the guard.
It runs in addition to the `**Full design**` / `Mechanics:` reference
propagation the cold-read-scope table already requires for those kinds.
...
3. For each section in mechanics whose content moved since the last sync,
   ...
   - **`### Summary`**: re-write to reflect the current mechanism.
...
`### Summary` and mechanism overview in `design.md` accurately summarizes the current
...
The comprehension gate returns the **hybrid** shape (D2): a `## Reading
narrative` plus the comprehension assessment, mental-model verdict,
`## Structural findings`, and the **Verdict** line, all per the prompt's output
format (`prompts/design-review.md` § Output format). Parse the narrative
alongside the Verdict and findings; it is scoped to comprehension and the
mental model (never the prose AI-tell axis, which is the auditor's, S4). Which
shape the return takes is split by whether `output_path` was injected (the
path-conditional branch):
...
- **`output_path` absent** (the default — every kind except `phase4-creation`):
  ...
  (`PASS` or `NEEDS REVISION`), the inline `## Reading narrative`, and the
  inline **Structural findings** list.
- **`output_path` supplied** (`phase4-creation`): the sub-agent returns only
  ...
  writes the full Markdown — the `## Reading narrative` and the
  `## Structural findings` detail — to the file at `output_path`. Parse the
  **Verdict** and counts from the return, then partial-fetch the written file's
  `## Reading narrative` and `## Structural findings` sections for the narrative
  and finding detail that Step 5 merges.

**Route the narrative into the Step-6 author rework input.** The narrative is
the author's rework seed, not a discarded pleasantry. When the gate returns
`NEEDS REVISION`, carry the parsed narrative into Step 6 alongside the
structural findings: for the **creation kinds**, a decision-shaped or
comprehension-shaped gate finding re-opens the inner loop, and the author
re-spawn's params file carries the narrative in its `flagged_passages` context
so the re-ground targets the passages where the mental model actually broke,
not only the discrete findings. For the **interactive kinds**, the narrative is
context for the single-agent fix loop's inline edits. A `PASS` narrative that
names no open finding needs no rework routing — it rides into the review log
(Step 7) as the gate's reading record only.
...
3. Distills `design.md` — updates each affected section's `### Summary` +
