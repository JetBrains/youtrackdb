...
>   type conversion, dirty tracking, equals/hashCode, and embedded
>   document semantics.
> - DB tools handle export/import/repair — test with small, scoped
>   in-memory databases. Round-trip an exported DB through import and
>   compare for fidelity. Use `DbTestBase` for end-to-end shape;
>   prefer standalone for pure POJO/record-comparison logic where
>   feasible.
> - Carry forward Tracks 5–14 conventions (see Track 14 entry).
>
> **Constraints**:
> - In-scope: only the listed `core/record*` and `core/db/tool*`
>   packages. Do NOT touch `metadata/schema` (Track 16) or storage
>   (Tracks 19–21).
> - DB tool tests should keep dataset sizes small (10s of records)
>   to keep surefire wall-clock budgets reasonable.
>
> **Interactions**:
> - Depends on Track 1 (coverage measurement) and benefits from
>   Track 14 (db core baseline).
> - Closes the Track 12 `MemoryStream` residual gap (item h) — once
>   `RecordId*` and `RecordBytes` callers are migrated off the
>   `@Deprecated` `MemoryStream`, re-measure.
...
>   properties (`DbTestBase`).
> - Focus on property type validation, index creation, cluster
>   selection strategy dispatch, schema evolution (add/drop class,
>   add/drop property, rename), and schema proxy delegation.
> - Function/Sequence library tests cover registration, lookup,
>   removal, and persistence across session reload.
> - Carry forward Tracks 5–15 conventions.
>
> **Constraints**:
> - In-scope: only the listed `core/metadata*` packages. Security
>   metadata is owned by Track 17.
> - Avoid coupling tests to internal SchemaShared synchronization —
>   prefer public API for schema mutations.
>
> **Interactions**:
> - Depends on Track 1.
> - Schema fixtures established here may be reused by Tracks 17, 18,
>   and 22.
