...
> - Small packages: `conflict`, `dictionary`, `servlet`, `replication`,
>   `type`, `collate`, plus `api/exception` and `api/config`
>   (mechanical fix T7 from previous Phase A iter-1: enumerated
>   here, both have small surface and stand alone).
>
...
> - For each inherited DRY/cleanup item that is coverage-additive
>   (test refactors, falsifiable strengthening, additional pins,
>   `*DeadCodeTest` shape pins for clusters whose live deletion is
>   deferred to 22b/22c): land the test additions; if a production-bug
>   fix is local and test-additive, land it in lockstep with its
>   WHEN-FIXED pin flip. Production-bug pins requiring SPI changes or
>   wider review flow to 22c as YTDB tracking issues.
> - Decompose `core/exception` coverage via `@Parameterized` (mechanical
>   fix A7 from previous Phase A iter-1: ~30 exception classes share
>   the same ctor/getter shape — one parameterized test class fans the
>   permutations).
...
> - The verification step at end of 22a re-runs `coverage-analyzer.py`
>   and updates `coverage-baseline.md` with the post-22a aggregate
>   (target: ~81–82% line / ~70% branch before 22b's denominator
>   drop). **22a's number is interim, not final** — 22b's verification
>   step is the authoritative post-deletion baseline; the Goals
>   section's `~82–83% line / ~70–71% branch` headline is measured
>   after 22b lands.
>
> **Constraints**:
...
>   and `**/api/gremlin/tokens/schema/**` are NOT targeted.
> - **Gremlin I/O sub-packages are D4-accepted** (mechanical fix R6
>   from previous Phase A iter-1): `gremlin/io/binary`,
>   `gremlin/io/graphson`, `gremlin/io/gryo` aim for D4-tier coverage
>   (~65–70%) — they are TinkerPop-fork-shadowed wire formats whose
>   exhaustive testing is upstream's responsibility.
> - **TinkerPop fork-shadowing forbid in new test files** (mechanical
>   fix R7): new test files MUST NOT import `org.apache.tinkerpop.*` —
>   the project's vendored fork lives under `io.youtrackdb.*`. Existing
>   test files that import the upstream package keep their imports
>   unchanged (out-of-scope for 22a).
> - **Per-test UUID DB name** (mechanical fix T5): new tests using
>   `DbTestBase` MUST generate a per-test UUID DB name to avoid
>   parallel-suite collision (`DbTestBase.dbName = "test_" +
>   UUID.randomUUID()`). Tests reusing static DB names are flagged in
>   Phase A's track-level review.
...
>   JaCoCo branch coverage matters (CLAUDE.md §10).
>
> **Interactions**:
