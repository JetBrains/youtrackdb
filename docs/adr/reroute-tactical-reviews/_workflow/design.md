...
Today every review sub-agent returns its findings as its final message, straight into the orchestrator's resident context. The orchestrator then deduplicates, assigns severity, buckets, and hands a merged finding list to the implementer. That review loop is the orchestrator's dominant context filler and the near-exclusive cause of crossing the 40% `warning` gate, where a session teardown reloads the resident context as cache-create tokens — an estimated ~120-180K for a body-heavy review session. The steady-state carry is cheap; the restart it triggers is what costs.

This design is for the workflow engineer maintaining `.claude/`, and assumes familiarity with the Phase A/B/C review loop, the orchestrator's synthesis recipe (today's in-context dedup, severity, and bucket step), the §1.7 staging convention, the per-branch I6 invariant (live workflow stays at develop state until promotion), and the context-window `warning` gate.

This design makes every bulk-producing sub-agent write its structured output to a file at a spawn-supplied path and return only a thin manifest. The routing then splits by who consumes the finding bodies. For **tactical** code-review fan-outs (Phase B `risk:high` step review, Phase C track review, and gate-checks) the orchestrator never reads a body: it routes on the manifest metadata it already holds and hands the in-scope anchors to the per-iteration implementer, which partial-fetches the bodies, fixes at the code level, and escalates only design calls. For **strategic** reviews (Phase A panel, Phase 2 plan review, gate-verifications) the orchestrator keeps the partial-fetch read of `## Findings` from disk, because the consumer is the planner revising the plan, not an implementer. **Research and audit** sub-agents write their own file and return a summary the orchestrator partial-fetches on demand.

The enabling primitive is the manifest-plus-sections file schema: a machine-readable manifest header over anchored body sections, addressed by stable heading anchors rather than line offsets. The manifest is exactly what the sub-agent returns.

Restructured to fit: the synthesis recipe collapses to manifest-only routing; per-dimension finding IDs become the sole addressing and the synthesis `M<n>` layer is removed; agent output becomes conditional on a supplied path because the dimensional agents are shared with standalone skills; the §1.7 staging convention generalizes from two path prefixes to three so agent-definition edits stage like every other workflow file; and a verifiable refutation trail lands in each dimensional reviewer's file.
...
This design introduces eight load-bearing ideas. Each is named and used without re-definition in the Parts that follow; each entry pairs the concept with what it replaces so the delta from today is visible at a glance.
...
classDiagram
    ...
    class Implementer {
        ...
        +applyFixes() commit
        +escalate() DESIGN_DECISION_NEEDED
    }
...
sequenceDiagram
    ...
    O->>R: spawn with output-path + diff
    ...
    I-->>O: SUCCESS (Review fix: SHA) | DESIGN_DECISION_NEEDED
...
Validation is an anchor-header grep, never a body read: a reader confirms the manifest's claimed `findings` count matches `grep -cE '^### [A-Z]{2,}[0-9]+ '` over the file before trusting the index. The regex is ID-anchored on purpose. A bare `^### ` count would also catch any non-finding `### ` heading, the same false-positive trap the workflow-sha parser avoids by anchoring on its `workflow-sha:` literal instead of a bare 40-hex match. So under `## Findings` the `### <ID> ` heading shape is reserved for finding anchors: a reviewer puts no other `### ` heading there, and all reasoning prose lives in `## Evidence base` (`#### ` entries) or inside the finding body. Because the grep counts heading lines only, the orchestrator can validate at spawn time without ingesting any body, so the no-bodies invariant holds through validation. On a count mismatch the manifest carries a `CONTRACT_VIOLATION` flag and the reader falls back to a whole-section read; the fallback owner differs by routing class (the implementer for tactical, the orchestrator for strategic), so the orchestrator never falls back to a tactical body read.

### Edge cases / Gotchas

- A zero-finding reviewer writes an empty `## Findings` and a `findings: 0` manifest; the orchestrator routes on counts and never spawns an implementer for an all-zero fan-out.
- Per-dimension filenames (`iter-K-bugs-raw.md`) make every path unique across a parallel fan-out; concurrent reviewers never share a path. Each reviewer `mkdir -p`s its own `reviews/` directory (idempotent).
...
**S4.** The manifest `findings` count must equal the ID-anchored grep count, else `CONTRACT_VIOLATION` and a whole-section fallback.
...
- A `CONTRACT_VIOLATION` on a tactical file routes the whole-section fallback to the implementer, preserving the no-bodies invariant on the orchestrator.
...
Today the synthesis recipe has the orchestrator re-judge each finding's severity by reading the body: downgrade a `blocker` that is really a style preference, upgrade a `suggestion` whose stated impact reads higher. Both directions need the body. The manifest carries `sev` but not the rationale, so the body-dependent re-judgment cannot run on metadata alone.
...
The dimensional `review-*` agents run a structured protocol (premises, code-path traces, formal claims, and a Phase-4 refutation check that tries to disprove each claim before reporting) but emit only findings today. Internal-only makes the refutation check unverifiable: nothing on disk confirms it ran, so a reviewer that skipped it under load looks identical to one that did it. The trail lands in `## Evidence base` using YTDB-1069's roster rendering (survived claims compress to one line; refuted or non-passing claims appear in full). The routine reader is the implementer, which reads only `## Findings`, so the trail never enters any long-lived context. The trade is a small, compressed, net-new reviewer-output cost (~1.4K tokens per reviewer, ~3% of review cost) for a verifiable guard, with an `exempt because…` hatch for any dimension where it does not pay.
...
**TL;DR.** Every bulk-producing sub-agent class either follows the file-plus-manifest rule or carries an explicit `exempt because…` annotation. The rule's canonical home is a new subsection in `conventions-execution.md`; other docs cite it rather than restating it.

The covered classes are the Phase A panel reviewers, the Phase B and Phase C dimensional reviewers, the Phase B/C and Phase 2/3A gate-check reviewers, the Phase 2 plan reviewers, and the Phase 0/1 and Phase 4 research/audit sub-agents. Each writes its own file and returns a thin manifest or summary; the body consumer is the implementer for tactical findings and the orchestrator or planner for strategic findings and research. The dimensional reviewers additionally emit the evidence trail. The four pure-standalone agents are exempt, and `review-mode.md`'s `FIX_FINDING` path is exempt because its triples are user-sourced, tiny, and already in the orchestrator's conversation context. The rule plus the `exempt because…` hatch is itself an invariant: a new bulk-producing agent class must declare one or the other.

### Edge cases / Gotchas

- The issue's Files list misses several load-bearing sites: `implementer-rules.md` (the `findings:` field and `FIX_NOTES` references), the `structural-review.md` and `design-review.md` prompts, and `review-mode.md`; `Explore` has no agent file (its change lands in `research.md`'s delegation text); `commit-conventions.md` is out of scope, and `step-implementation-recovery.md` needs only a light consistency pass.
