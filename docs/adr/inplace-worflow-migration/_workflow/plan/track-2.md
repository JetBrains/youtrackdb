...
- **Step 1** (When `/create-plan` runs Step 4): the resulting `implementation-plan.md`, `plan/track-N.md`, `design.md`, and (when dual-seed is in scope) `design-mechanics.md` each have a line matching `<!-- workflow-sha: [0-9a-f]{40} -->` as line 1; the H1 sits on line 2. The SKILL's new top-of-file Stamp note states that stamps are written at creation only.
