...
### Step 1 — commit d396f579ca, 2026-05-24T11:39Z [ctx=safe]

**What was done:** Step 4 body in `.claude/skills/migrate-workflow/SKILL.md` now matches Track 4a's renumber-down. The seven H3 sub-step headers shifted from `### 5.1`–`### 5.6` to `### 4.1`–`### 4.7`, with a brand-new `### 4.5 Advance stamps in lockstep` inserted between today's 4.4 (apply edits) and the renumbered 4.6 (progress sentinel). Every in-prose `5.x` cross-reference inside Step 4 swept to the new numbering. The `<migration-worktree>` placeholders at lines 511, 514, 557, 580, 601 rewrote to in-branch wording. Sub-step 4.1's context-check halt message gained the 4.4→4.5 crash-window warning instructing the user to run `git diff` before re-invoking after a mid-loop `/clear`. New 4.5 prose pins the ordering invariant (edits → advance → progress sentinel), names the `Edit`-against-line-1 writer with the prior stamp as `old_string`, documents the previously-unstamped-artifact side effect, and cross-references `design.md`'s sequence diagram in §"Per-commit replay and lockstep advance".

**What was discovered:** Lines 511 and 514 in the Step 1 description carried descriptive prose ("the absolute migration-worktree path") rather than literal `<migration-worktree>` placeholders; rewriting to single-worktree wording was straightforward. Two non-Step-1 sites still carry `<migration-worktree>` references at SKILL.md lines 623/624 and 630-631 (the `## Notes` section), outside Step 1's enumerated set — Step 3's final-summary rewrite should fold these into its scope while it has the file open. The hint applies inside Track 4b only, not cross-track.

**Key files:**
- `.claude/skills/migrate-workflow/SKILL.md` (modified)

### Step 2 — commit 3d5fb65fe9, 2026-05-24T11:44Z [ctx=info]

**What was done:** Inserted `### 4.8 Final stamp-to-HEAD batch` into `.claude/skills/migrate-workflow/SKILL.md` between sub-step 4.7 (mark per-commit task completed) and the start of Step 5 (final summary). The new sub-step captures HEAD once, walks `_workflow/**` per `conventions.md` §1.6(h) byte-for-byte, and re-stamps every artifact via `Edit` against line 1. Prose documents that the writer is idempotent on equal SHA (artifacts already at HEAD pass through unchanged), that the sub-step lands invariant I2 at session end, and that optional artifacts absent from disk are silently skipped by the walk's `ls 2>/dev/null` shape. Step 0's umbrella task list grew one slot: "Final stamp-to-HEAD batch" inserted between "Per-commit migration loop" and "Final summary" so the orchestrator-side TaskCreate roster mirrors the new sub-step.

**What was discovered:** The pre-commit ephemeral-identifier gate flagged two `Step N` matches in the new prose ("Step 5", "Step 2.0") plus the invariant name `I2`. All three resolve to in-document section anchors of the migrate-workflow skill itself (its own H2 Steps 0..5 and the plan's invariant vocabulary), matching the wording already committed in Step 1. The rule's allowed-list framing for document-self references holds; no rewrite needed. Step 3's final-summary rewrite will reference "Step 2.0" and is likely to surface the same gate matches — proceed past them on inspection.

**Key files:**
- `.claude/skills/migrate-workflow/SKILL.md` (modified)

### Step 3 — commit 158c4a49e6, 2026-05-24T11:47Z [ctx=info]

**What was done:** Rewrote Step 5 final summary in `.claude/skills/migrate-workflow/SKILL.md` for the in-branch flow. Both `<migration-worktree>` placeholders in the "Total files edited" and "Review the diff" bullets are gone, replaced with in-branch wording including the verbatim quoted prompt "Review the diff in this worktree, then commit + push when satisfied. Delete `.migration-progress` after the commit." Added a "Workflow stamps now at `$HEAD_SHA`" bullet above the per-classification counts surfacing the I2 post-condition from sub-step 4.8, and a conditional bootstrap-count bullet that fires only when `$USER_BOOTSTRAP_SHA` is set by Step 2.0. Folded in the three additional stale "migration worktree" prose references from `## Notes` that Step 1's episode flagged as in-scope while the file was open. `grep -c '<migration-worktree>'` over SKILL.md returns 0.

**What was discovered:** The plan's pre-Step-1 line numbers (613, 614) had shifted to 650/651 in the current SKILL.md after Steps 1 and 2; locating Step 5 by header rather than by line number was the correct fallback. The pre-commit ephemeral-identifier gate surfaced "Step 2.0", "sub-step 4.8", and "I2" — all resolved to in-document section anchors of the migrate-workflow skill itself and the plan's named-invariant vocabulary, both permitted under the orchestrator note's allowed exceptions.
