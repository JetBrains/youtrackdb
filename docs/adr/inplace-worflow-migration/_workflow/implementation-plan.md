...
- [x] Track 3: SHA-aware drift check
  ...
  >
  > **Strategy refresh:** CONTINUE — Track 3's caller-side Phase 2 additions (merge-base failure recovery per §1.6(c), both-arrays-empty silent skip per §1.6(h), exit-code semantics, unstamped-short-circuit rendering) are caller-specific and already reflected in Track 4a's `## Plan of Work` Step 4 (citations to §1.6(c) and §1.6(h) landed during the CR1 escalation in plan review). §1.6(h) Phase 1 walk byte-copy contract holds; no downstream Track impact beyond what the plan already encodes.

- [x] Track 4a: Migration preflight + range computation
  ...
  >
  > **Track episode:** `migrate-workflow` Step 1 now runs inside the branch's worktree. The develop-worktree premise is dropped; `$ARGUMENTS` is optional with a single equality check against `git branch --show-current`; the active-plan-dir ladder runs in-preflight as `$PLAN_DIR`; the dirty-tree check narrows to the §1.6(h) artifact paths (excluding `staged-workflow/` per D14). The old "Resolve migration worktree path" Step 2 is gone and downstream steps renumber down by one. New Step 2.0 carries the unstamped-artifact bootstrap with the D8 rationale, two-subcommand validation (`rev-parse --verify $SHA^{commit}` + `merge-base --is-ancestor`), canonical 40-char `CANON_SHA` capture, and a session-bound 3-attempt counter the orchestrator owns. The renumbered Step 2 carries the stamp-walking fold producing `BASE_SHA`; a paired `STAMPED_PAIRS` array maps each stamp to its owning artifact path so the merge-base-failure recovery prompt names paths; the continue-and-collect Phase 2 fold gathers every failing pair into `MERGE_BASE_FAILED` for one user interaction; §1.6(h) both-arrays-empty halts with `no artifacts to migrate`. Step 3 renamed the progress-file header from `fork=`/`head=` to `range_start=`/`range_end=` and collapsed the three legacy stale checks into a single `range_start` mismatch. Phase C iteration 1 cleared with 13 findings VERIFIED across consistency / prompt-design / instruction-completeness / writing-style. One finding (WI3) was a real correctness gap: the original merge-base recovery appended failed files to `UNSTAMPED_FILES` but left the pruned-stamp SHAs in `STAMPED_SHAS`, so the restart fold re-failed at the same pair and exhausted the 3-attempt cap with the user blameless; the fix prunes the failing SHAs from `STAMPED_SHAS` before restart so the user's fresh bootstrap SHA actually anchors the fold. Cross-file work: `workflow-drift-check.md`'s "both files must remain text-identical" claim now carries an explicit `STAMPED_PAIRS` carve-out (WC3) so a future maintainer reading only that side cannot "fix" the migration back to byte-identity. The §1.6(h) walk byte-copy contract survives the extension because the additions are purely additive — one init line plus one assignment inside the stamped branch — and the cross-reference paragraph now documents the contract on both sides.
  >
...
- [x] Track 4b: Migration replay + final batch
  ...
  >
  > **Track episode:** `migrate-workflow/SKILL.md` now ships the new per-commit lockstep advance (sub-step 4.5) and the final stamp-to-HEAD batch (sub-step 4.8) on top of Track 4a's renumber-down. Step 1 rewrote Step 4's body (H3 renumber `5.x → 4.x`, in-branch wording sweep, the 4.4→4.5 crash-window halt warning); Step 2 inserted sub-step 4.8 and grew Step 0's umbrella task list to seven slots; Step 3 rewrote Step 5's final summary with the HEAD-SHA bullet and the conditional bootstrap-count line; Step 4 applied a content-edit mutation to `design.md` §"Per-commit replay and lockstep advance" so the crash-window prose names the migration-side resume contract directly rather than routing through the drift-check normalization path. Phase C iteration 1 surfaced 13 findings (2 blocker / 8 should-fix / 3 suggestion) across four workflow-review dimensions; all VERIFIED on gate-check. The blocker pair was structural: F1 named `PREV_SHA` for K≥2 commits at 4.5 (the original prose said "prior stamp" with no producer for commits 2..N), and F2 replaced the empty `:` body in 4.8's bash block with a prose-only writer directive matching 4.5's style. F4 moved task 5 `completed` out of Step 5 to the tail of the per-commit loop and added `in_progress`/`completed` flips for the two new umbrella tasks. F3 corrected a phantom "sequence diagram" cross-reference (the diagram lives in `design.md` §"Migration replay loop", not the section the prose cited). F10 compressed the frontmatter description from 367 to 295 characters (under the 350-char always-loaded ceiling); the change is visible in the current session's skill list. F11 added a halt clause to sub-step 4.3 for commits that modify `conventions.md` §1.6 itself — the in-place migration cannot self-bootstrap a stamp-format change. One pre-existing debt remains held: `design.md` §"Per-commit replay and lockstep advance" TL;DR at line 259 fails the design-mechanical-checks fragmented-header heuristic; the next content-edit that touches that TL;DR is the natural place to sweep it. Cross-track signal for Track 5 reflection queue: the implementer surfaced the Ephemeral-identifier pre-commit gate matching skill-internal step labels as a recurring friction point.
  >
  > **Track file:** `plan/track-4b.md` (4 steps, 0 failed)
  >
  > **Strategy refresh:** ADJUST — Track 5's `## Plan of Work` umbrella-task number "task 6" is stale post-Track-4b (umbrella has 7 slots today; reflection becomes task 8). Phase A decomposition writes the correct number; no other contradictions across remaining tracks.

- [x] Track 5: Self-improvement reflection for migration
  ...
  >
  > **Track episode:** `self-improvement-reflection.md` is now a parameterized two-caller protocol. Step 1 swept ten session-type-dependent surfaces in one atomic commit (the `## Inputs` block, line-3 intro, §"When it runs" intro and body, skip-conditions, four migration-shaped friction sub-bullets, §"Reflection procedure" Step 2 conditional, `**Phase:**` and `**Source session:**` template literals, `## Reproduction context` enumeration). Step 2 wired Step 6 of `migrate-workflow/SKILL.md` and the umbrella task 8 slot, redirecting Step 5's trailing line to Step 6. Phase C iterations (2 of 3 used) hardened the dual-caller contract: outer-caller registries (`workflow.md:579`, `execute-tracks/SKILL.md:46`) acknowledge the second caller; the §Inputs branch summary jumped from "three clauses" to "four" to match the document-actual ten surfaces; the `/migrate-workflow` skip-conditions parenthetical was rewritten to name the SKILL's real halts (Step 1 narrow-scope dirty-tree, Step 1 argument mismatch, Step 2 both-arrays-empty, Step 2.0 three-rejected-attempts, Step 3 stale `range_start`, Step 4.1 context-window halt, Step 4.3 stamp-format-change); an unrecognized-`session-type` halt clause closed the typo-tolerance gap; em-dash density at §"When it runs" lines 82-92 dropped 3 → 0; Step 4.1's context-window halt gained explicit deferred-reflection prose ("the next session's reflection at Step 6 reports this halt as friction"); `**Source session:**` collapsed to a single pipe-separated line; and the umbrella task 8 `completed` flip moved before the reflection invocation since reflection's Step 10 ends the session and never returns control. Iteration 1 closed 11 findings across four workflow-review dimensions; iteration 2 closed F12 (the iteration-1 gate-check's only new finding). Cross-track signal for Tracks 6 / 7: the six iteration-1 findings touching `self-improvement-reflection.md` converged on a single drift signature — under-specified boundary conditions when adding a second caller to a previously single-caller protocol; future tracks adding a third caller to a similar two-caller shape may want a "boundary-condition audit pass" step in Phase A.
  >
  > **Track file:** `plan/track-5.md` (2 steps, 0 failed)
  >
  > **Strategy refresh:** ADJUST — Track 6 Phase A's technical review folds in a boundary-condition audit of `workflow-drift-check.md` per Track 5's first→second caller lesson. Track 6's Plan-of-Work claim "the rest of the file stays caller-agnostic and needs no change" is reviewed as an assumption rather than accepted as fact; if the audit surfaces additional caller-specific surfaces, decomposition absorbs them. No other contradictions across remaining tracks.

- [x] Track 6: Drift gate at /create-plan startup
  ...
  >
  > **Track episode:** `/create-plan` now runs the SHA-aware drift gate at session start (Step 1.5 between the `<dir-name>` resolver block and Step 1a's handoff scan), so a re-invocation after the user rebases the branch onto a newer develop catches post-rebase drift before any research investment (D9). Defer-from-`/create-plan` has a host: Step 5 sub-step 3 reads the deferred-drift TaskCreate todo and recites the same `Deferred workflow drift: <count> commits since <short-stamp-base-SHA>` line shape `workflow.md § What to do before ending a session` reads for `/execute-tracks`. `workflow-drift-check.md` is now caller-symmetric across `/execute-tracks` and `/create-plan` — intro names both callers, body sweeps caller-specific phrasing to caller-agnostic forms or caller-qualified anchors, Migrate-now folds both branches into one paragraph, Defer names per-caller recital surfaces, § After-the-choice qualifies the Remote-authoritative re-entry paragraph as `/execute-tracks`-only. **Strategy refresh signal:** Track 5's first→second-caller drift pattern repeated here; Phase A's boundary-condition audit (triggered by Track 5's strategy-refresh recommendation) surfaced 12 caller-specific surfaces beyond the intro paragraph, shifting Track 6's scope from 2-3 steps to 5 (DL1, Full caller symmetry). **Phase C iteration 1 (1/3 used, PASS):** 18 findings across 3 dimensions (workflow-consistency, workflow-prompt-design, workflow-instruction-completeness); writing-style and context-budget reviewers returned clean. Iteration 1 applied F1-F12 in `Review fix:` commit `cd5775f6b2` and gate-check verified all 18. Phase C's scope expanded by 2 files Track 6 had declared out-of-scope: `migrate-workflow/SKILL.md:8` (stale "Auto-detection runs in /execute-tracks" claim — Tracks 4a/4b had completed without picking it up) and `mid-phase-handoff.md:94-99` (Track-6-adjacent caller-asymmetry plus pre-existing "Branch Divergence Check (workflow.md step 5)" mislabel — actual anchor is step 3). **Cross-track impact for Track 7:** Step 1.5 vs Step 1a handoff-scan interaction is now documented in three coordinated sites (SKILL.md Step 1.5, gate file § After-the-choice, track-6.md `## Validation and Acceptance`); Track 7's staging architecture will introduce a parallel convention, and the caller-symmetric pattern Track 6 established applies — name both callers explicitly where the contract differs. **Surprise carry-forwards:** Step 3 / Step 5 noted `workflow-drift-check.md` § Defer's final residue-contract paragraph still cites only `workflow.md § What to do before ending a session`; reviewed during Phase C and deliberately not changed — that workflow.md section IS the residue-contract source for both callers, and "session-end summary" reads naturally in both contexts.
  >
  > **Track file:** `plan/track-6.md` (5 steps, 0 failed)
  >
  > **Strategy refresh:** CONTINUE — Phase A technical review folds in the boundary-condition audit pattern Tracks 5/6 validated; Track 7's `## Plan of Work` claim that the 6 in-scope files cover the staging convention is reviewed as an assumption rather than accepted as fact, and decomposition absorbs any additional consumers the audit surfaces.

- [x] Track 7: Staging architecture for workflow-modifying branches
  > Introduce the staging convention for branches whose plan modifies `.claude/workflow/**` or `.claude/skills/**`. Workflow document changes accumulate under `<dir-name>/_workflow/staged-workflow/.claude/workflow/...` and `.../staged-workflow/.claude/skills/...` during execution; the live workflow stays at develop's state for the branch's own sessions. A new "Promote workflow changes" step at Phase 4 copies the staged subtree to the live paths in one commit, immediately before the final-artifacts commit.
  >
  > **Track episode:** `conventions.md` §1.7 is now the single source of truth for the staging architecture: path layout, canonical marker sentence, two-signal detection split (DL1 — Constraints declaration drives the implementer enforcement gate; staged-subtree directory presence drives the Phase 4 promotion guard), reads-precedence rule (DL3 — staged copy authoritative when present), write routing with copy-then-edit on first touch, additive-only contract for `cp -r` promotion plus documented deletion workaround, rebase-precedes-promotion rule, the precise I6 invariant (DL4 — intra-branch authoring transition only; rebase-merge from develop excluded by scope), forward-applicable carve-out, a worked on-disk-shape example, and aborted-promotion resume semantics with an empty-commit short-circuit (§1.7(j), added by Phase C iter-1 to close WI1). `implementer-rules.md` carries the path-mapping rule and the pre-commit live-workflow-path gate active only on plans declaring the canonical marker; `step-implementation.md` references both from the prompt-template static block. `workflow.md` § Final Artifacts documents the three-commit Phase 4 shape on workflow-modifying plans (promote-staged-workflow → final-artifacts → cleanup); `prompts/create-final-design.md` § Step 4 carries the promotion bash with the tightened `[ -d "$STAGED_DIR/.claude" ]` guard, the corrected divergence sanity check (compares `merge-base..origin/develop` past the fork-point rather than `merge-base..HEAD`, with a `git fetch origin develop` prelude — the formula is also resume-neutral on the branch's own promote commit), and the `git diff --cached --quiet || git commit` empty-commit short-circuit for clean Phase 4 resume. `workflow-drift-check.md` carries a defensive comment at the pathspec site recording the staged-subtree exclusion. `mid-phase-handoff.md` § Resume protocol acknowledges Phase 4 promotion as a resumable pause site, citing the durable `conventions.md` §1.7(j) anchor instead of per-branch `design.md`. The Phase 4 promotion commit-message prefix `Promote workflow changes from docs/adr/<dir-name>/_workflow/staged-workflow` is byte-identical with the pre-commit gate's allow-clause (Phase C iter-1 fix B1 collapsed prior divergence between bash output and documented prefix). **Phase C surprises:** iter-1 RESULT_MISSING after a partial S3 edit (9-finding × 6-file spawn exceeded the soft budget) recovered via halved re-spawn finalizer (`a749bbd170`); iter-2 closed the remaining S3 placeholder sweep, S5 additive-only contract, S6 guard spelling, S7+WS6 em-dash polish, and G1 terminal-period clarification (`61b36abe29`). The two Phase C consistency catches the step-level review missed — B1 (bash output ≠ documented gate prefix; would have refused real-branch Phase 4 promotion) and S2 (divergence check measured the wrong direction; would have silently passed on stale-from-develop branches) — are both resolved and gate-check VERIFIED. **Strategy refresh:** Forward-applicable. The current branch is the §1.7(h) carve-out — it does not stage its own edits because no prior version of §1.7 existed during Tracks 1–6. The first workflow-modifying branch that opens a plan after this PR merges to `develop` is the first to exercise the staging path end-to-end; Phase A reviewers on such branches should spot-check the corrected divergence formula and the empty-commit short-circuit against real execution. **Cross-track impact:** 17 findings deferred to Phase 4 self-improvement-reflection — 4 design enhancements (reads-precedence enforcement WP3/WI4, pre-commit inverse-misroute gate WI3/WP7, gate-checked-vs-final-`-m` coupling WI6, post-rebase staged-subtree audit WI5); 3 ephemeral / orchestrator-side surface edits (`design.md` §378/§361 stale guard form F6, `track-7.md` Surprises wording F7, `track-7.md` DL4 em-dash WS2); 3 context-budget refactors against DL1-DL4's explicit inline-shape choice (WB1, WB2, WB3); 4 minor documentation gaps (path scope for non-`.claude/workflow`/non-`.claude/skills` surfaces WI7, abandon-path WI9, prompt-design micro-gaps WP2 / WP5); 1 long-bullet readability observation at the soft cap by design (WS4). Reflection at Phase 4 files the design-enhancement subset as `dev-workflow` YouTrack issues (or skips with a notice if the YouTrack MCP server is unreachable); the ephemeral / orchestrator-side residue disappears with the Phase 4 cleanup commit.
  >
  > **Track file:** `plan/track-7.md` (6 steps, 0 failed)
