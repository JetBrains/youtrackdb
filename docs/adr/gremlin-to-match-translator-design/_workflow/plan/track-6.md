...
- 2026-07-27 (count, design reconciliation for Phase 4): **Two frozen-`design.md` claims are inaccurate as-built; sync in `design-final`, do not edit frozen `design.md` in Phase 3.** (1) §"Empty input" (design.md:1799-1805) is right that MATCH `count` of empty returns a `0`-row, but the surrounding text implies `dropNullRows` is what closes the count gap; count keeps `dropNullRows = false` and relies on MATCH's native `0`-row, so no count-specific empty-input handling exists or is needed. (2) §"Plan cache is unaffected" (design.md:1778-1781) holds on all three front-ends after a fix. The initial count-finalization note claimed `GremlinPlanCache` cached the count plan "safely" — the Track 6 dimensional re-review disproved that: `GremlinPlanCache.putInternal` did not consult `canBeCached()`, so it cached the `CountFromClassStep` plan and could replay a build-time security-policy decision across sessions and users (a count-cardinality disclosure past a row-hiding READ policy). Fixed by gating `putInternal` on `canBeCached()`, mirroring the `YqlExecutionPlanCache` guard, so a count MATCH plan is not cached on any front-end — exactly as the design states. One deliberate Phase-1 asymmetry remains: the indexed-equality count fast path (`CountFromIndexWithKeyStep`) fires only for YQL `SELECT`; filtered GQL/Gremlin counts run the generic aggregate (design §"Non-fast-path counts").
...
### Dimensional re-review + fixes — 2026-07-27 [ctx=info]
**What was done:** Ran the full `/code-review` dimensional pass (bugs, concurrency, security, performance, code-quality, plus test-quality/structure/concurrency) over the cumulative Track 6 diff (`ddd15063fde6..HEAD`), because step-level `review-bugs` was skipped for high-risk steps 1/3/4/6/7 and the first Phase C pass was a false PASS. Concurrency and performance came back clean (no defects; count filtered-path is not a regression). The other dimensions surfaced two blockers and a set of should-fixes, all fixed and verified:
- **Blocker (security/correctness):** `GremlinPlanCache.putInternal` ignored `plan.canBeCached()`, so the count short-circuit's `CountFromClassStep` (non-cacheable by contract, gated by a per-session security-policy check) was cached and replayed across sessions — a count-cardinality disclosure past a row-hiding READ policy. Fixed by honoring `canBeCached()`, mirroring the YQL/GQL-SQL cache.
- **Blocker (correctness):** `dedup()` after a value/map/scalar projection deduped on `(entity, value)` — the per-row unique entity defeated `RETURN DISTINCT` and returned duplicates. Now declines to native when the boundary output type is not `ELEMENT`.
- **Should-fixes:** un-aliased `RETURN count(*)` column name restored to `count(*)`; bare `group()`/`groupCount()` key by Vertex (were RID); IR-first completed (property/order/limit/skip built as AST via `ProjectionExpressionFactories`, closing a property-key injection surface and removing per-query parser instantiation — finishing the Step 6b direction); dead code removed (`setNamedDedupReturnProjection`, a dead `YTDBMatchPlanStep` ctor, unread getters); order/range recognisers read clause state via `RecognitionContext` getters instead of downcasting.

**What was discovered:** New equivalence tests over the payload-bearing aggregate shapes the review flagged as untested caught two more latent bugs: `mean()` always declined (YTDB SQL has `avg`, not `mean`) — mapped `mean → avg`; and `group()` map values were null (`list($currentMatch)` yields null in a grouped RETURN) — now `list(alias)` with the boundary wrapping the resulting RIDs as vertices.

**Key files:** `GremlinPlanCache`, `DedupGlobalStepRecogniser`, `MatchExecutionPlanner`, `HardwiredCountOptimizations`, `CountFromClassStep`, `PropertyAggregateStepRecogniser`, `GremlinAggregateAssembler`, `YTDBMatchPlanStep`, `ProjectionExpressionFactories`, `ByModulatorTranslator`, `Order`/`RangeGlobalStepRecogniser`, `RecognitionContext`/`WalkerContext`/`SubTraversalPredicateAdapter`; tests `ProjectionEquivalenceTest`, `GremlinPlanCacheTest`, `GremlinAggregateRecogniserTest`, `ProjectionExpressionFactoriesTest`. Commits `c6f98dc820`, `588a29a69f`, `efe5dbf0c8`, `e63c5aec4f`.

**Deferred (follow-up):** the shaping-flag → value-object refactor (code-quality's largest maintainability lever) — behavior-neutral but sweeps ~12 files of the shaping code, best done as its own focused pass — plus minor cosmetic suggestions (recogniser-test base extraction, singleton-doc consistency, a few `elementMap`/`skip` payload tests).

### Post-completion tail — commits `14c952d46f`, `fcefc7c37a`, 2026-07-27 [ctx=safe]
Two commits landed after the track-completion bookkeeping (16:29) and are recorded here retroactively; the plan Checklist entry is flipped to `[x]` in the same reconciliation.

**What was done:** `14c952d46f` ran a further dimensional re-review of Tracks 5 and 6 and fixed two blockers the recorded Phase C missed, plus should-fixes. `fcefc7c37a` executed the `ResultShaping` value-object refactor this episode block had deferred.
- **Blocker (correctness):** a `limit` / `skip` / `dedup` captured before a reducing or grouping terminator produced wrong results — the walker recorded the clause regardless of the following terminator, but MATCH applies SKIP / LIMIT / DISTINCT after the aggregate while Gremlin applies them to the reducer's input (`g.V().limit(5).count()` returned the total; `g.V().skip(2).count()` emitted nothing; `g.V().out().dedup().count()` counted duplicates). The aggregate and group configurers now decline such shapes to native, and `tryHardwiredMatchCount` bails when a limit is present.
- **Blocker (durability):** `GremlinPlanCache.put` cached a plan built against a schema snapshot a concurrent DDL had invalidated mid-walk, leaving a permanently stale plan in the shared per-database cache. It now stores only when no invalidation landed after the `planningStart` captured before the walk, mirroring the guard `MatchExecutionPlanner` already applies for the YQL plan cache.
- **Should-fixes:** `GremlinPlanFingerprint` length-prefixes every variable-length token, so a crafted key / label / class can no longer forge a cache-key collision; `matchedAccess` builds the `$matched` accessor as AST instead of reparsing a SELECT string per compilation; `ByModulatorTranslator` matches step supertypes with `instanceof`; doc / comment cleanups.
- **Refactor (behavior-neutral):** the seven boundary row-projection flags (`dropNullRows`, `dropOnAbsent`, `presencePropertyKeys`, `wrapMapValuesInLists`, `accumulateMap`, `unwrapSingletonMap`, `elementMapTokens`) now live in one immutable `ResultShaping` value object. `RecognitionContext` exposes a single `setResultShaping`; `TranslationResult` and `YTDBMatchPlanStep` carry one field instead of seven, collapsing the 13-parameter constructor; each terminator builds its shape from `ResultShaping.NONE` plus its overrides. The `SubTraversalPredicateAdapter` no-op stub setters and the per-terminator reset blocks are deleted. The projection, edge-traversal, and predicate equivalence suites plus the recogniser tests confirm no runtime divergence.

**Reconciliation stance:** closed under the "reconcile & advance" decision — both commits carry regression tests and the refactor is behavior-neutral with test confirmation, so no separate Phase C pass ran; the work is recorded here rather than re-reviewed.

**Key files:** `GremlinPlanFingerprint`, `GremlinPlanCache`, `GremlinAggregateAssembler`, `MatchExecutionPlanner`, `HardwiredCountOptimizations`, `ByModulatorTranslator`, `ResultShaping` (new), `YTDBMatchPlanStep`, `RecognitionContext` / `WalkerContext` / `SubTraversalPredicateAdapter`, `GremlinToMatchTranslator`, `GremlinStepWalker`, the projection recognisers; regression tests across the projection / aggregate / equivalence suites plus `GremlinPlanCacheTest` and `GremlinPlanFingerprintTest`.
