...
- 2026-07-22 (post-Step 7, plan-cache monitoring): **`GremlinPlanCache` exposes lifetime `getHits()` / `getMisses()` / `size()` (via `AbstractMetadataUpdateCache`) and feeds per-database profiler rates `GremlinPlanCacheHitRate` / `GremlinPlanCacheMissRate` (JMX `scope=Database`).** YQL/GQL plan caches share the counter skeleton (wired on get) but do not yet feed named CoreMetrics — Gremlin-first because that cache is new and operator-visible; YQL/GQL metric names deferred. R6 tests pin miss-then-hit and cold-rebuild ≡ cache-hit multisets.
