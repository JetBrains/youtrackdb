...
`hasLabel(label)` is usually folded by `YTDBGraphStepStrategy` into the start step's `hasContainers`, so the start recogniser pins `aliasClasses[a]`. The folded-`hasLabel` `@class`-narrowing AST is now produced by `MatchWhereBuilder.classEquals` — Track 4 is its first caller, since Track 3's rework deleted the interim `MatchClassFilters` helper and folded that narrowing onto `MatchWhereBuilder`. `has(key)` / `hasNot(key)` desugar (TinkerPop 3.8.1) to `TraversalFilterStep(__.values(key))` / `NotStep(__.values(key))` — neither lands on `HasStep`, so they bypass `HasStepRecogniser` and route through the new `TraversalFilterStepRecogniser` / `NotFilterStepRecogniser` Case A, emitting `IS DEFINED` / `IS NOT DEFINED` (not `IS NULL`, which over-matches null-valued properties).

The step-level logical filters (`AndStep` / `OrStep` / `NotStep` / `WhereTraversalStep` / `WherePredicateStep`) are the `ConnectiveStrategy` form — each child carries a sub-traversal of arbitrary recognized steps. Track 4's recognisers implement the post-Track-3 contract `Outcome recognize(StepCursor, RecognitionContext)` (head via `cursor.take()`, trailing shape via `takeIf` / `takeWhile`, returning `ACCEPTED` / `DECLINE`), not the retired `boolean recognize(Step, WalkerContext)` + manual `stepIndex` model. A logical child is translated by a **sub-walker** running a nested `StepCursor` over the child sub-traversal against the same registry, its recognition context inheriting the parent `boundaryAlias`; the decomposer pins the concrete sub-context type against HEAD.
...
7. **`GremlinPlanCache`** (D5): predicate literals bind as `SQLPositionalParameter` slots via a new `bindParam` on `WalkerContext` (which now implements `RecognitionContext` and no longer holds the traversal or a step index after Track 3's rework); the boundary step installs the per-walk param map through `ctx.setInputParameters(map)`. Key on the value-independent generic-statement fingerprint so one plan serves every value, and reuse the YQL plan-cache schema-change invalidation hook. Deferred out of Track 2 (no literal values there to bind).
...
- The plan cache serves one plan for the same traversal shape across distinct predicate values; a schema change invalidates it (D5).
...
**In scope (new):** full `GremlinPredicateAdapter`; `HasStepRecogniser`, `HasLabelStepRecogniser`, `HasIdStepRecogniser`, `TraversalFilterStepRecogniser`, `NotFilterStepRecogniser`, `AndStepRecogniser`, `OrStepRecogniser`, `NotStepRecogniser`, `WhereTraversalStepRecogniser`, `WherePredicateStepRecogniser`; `SubTraversalPredicateAdapter` + `SubWalkerContext`; `SQLEndsWithCondition` AST node + find-mode flag on `SQLMatchesCondition` (D-TEXT-OPS); `GremlinPlanCache` + `WalkerContext.bindParam` positional-parameter binding (D5); predicate-equivalence / NULL / collection / logical-combinator / plan-cache tests.
...
**Signatures:** `P.getBiPredicate()`; `HasContainer{key, predicate}`; `QueryOperatorEquals.equals` (lines 63-69 unbox, 71-73 null short-circuit); `MatchExecutionPlanner.manageNotPatterns` (first-NOT-alias precondition); `SQLPositionalParameter.getValue(params)`; `CommandContext.setInputParameters(map)` (per-walk param install for D5).
