...
7. **`GremlinPlanCache`** (D5): predicate literals bind as `SQLPositionalParameter` slots via `WalkerContext.bindParam`; the boundary step installs the per-walk param map through `ctx.setInputParameters(map)`. Key on the value-independent generic-statement fingerprint so one plan serves every value, and reuse the YQL plan-cache schema-change invalidation hook. Deferred out of Track 2 (no literal values there to bind).
