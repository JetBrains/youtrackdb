...
## Purpose / Big Picture
After this track the `core` TinkerPop feature suite completes in a single fork with the translator on, its failure set is a committed artifact rather than a promise, and the dropped per-alias filter that silently mistranslates every multi-hop traversal is fixed. Today neither holds: the suite does not terminate with the strategy enabled, and a filter on a non-root alias is discarded without declining, so the wrong answer ships silently.
...
- The per-directory and single-fork Cucumber failure sets are recorded as a committed artifact before any fix lands, and every criterion below reads against it rather than against an unenumerated run.
...
- The post-fix baseline is recorded and named as the number Track 11 reads.
