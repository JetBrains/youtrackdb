...
**Reference-accuracy note.** The class-shape facts above (the `final` modifier, the private lifecycle/projection members, the single `ResultShaping` field) are direct source reads, cross-confirmed by all three pre-split Phase A reviews and re-read at escalation time. PSI was unavailable in those review sessions (cold kotlinc index tripped the MCP timeout), so the *enumeration of sites to rewire onto the base* — the sole construction site `GremlinToMatchStrategy` plus the upstream `TranslationResult` producers `GremlinToMatchTranslator` / `GremlinStepWalker.buildResult`, whose rewire depends on the chosen base shape — rests on grep/read and must be re-verified via PSI find-usages at this track's decomposition (`## Concrete Steps` pre-write rule).
...
2. **Rewire the single-plan boundary construction** — `GremlinToMatchStrategy` (`replaceAllStepsWithBoundary`) is the sole site that constructs `YTDBMatchPlanStep` — onto the extracted base with no behavior change, and rewire the upstream `TranslationResult` producers (`GremlinToMatchTranslator`, `GremlinStepWalker.buildResult`) only if the chosen base shape requires it. PSI-verify each site during decomposition before it lands in `## Concrete Steps`.
...
**In scope (modified):** `YTDBMatchPlanStep` (becomes the single-plan concrete form over the base); `GremlinToMatchStrategy` (the sole boundary-step construction, rewired onto the base); `GremlinToMatchTranslator`, `GremlinStepWalker` (upstream `TranslationResult` producers — rewired only if the chosen base shape requires it); `ResultShaping` or an adjacent immutable type (carries / composes with the ordered post-process).
