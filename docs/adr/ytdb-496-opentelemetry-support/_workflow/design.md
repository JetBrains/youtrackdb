...
- Cross-thread `close()`: `InstrumentedSqlResultSet.close()` MUST be called from the same thread that constructed the wrapper. The wrapper enforces this with a `private final Thread constructionThread = Thread.currentThread()` field captured in the constructor and a Java `assert` at the top of `close()`: `assert Thread.currentThread() == constructionThread : "InstrumentedSqlResultSet.close() called on a different thread than construction; parent-child span linkage is best-effort under this path"`. With assertions off in production (default), runtime cost is zero. When a host genuinely consumes a result-set across threads the wrapper still calls `capturedContext.makeCurrent()` and the listener still fires, but the inner `Scope.close()` on the closing thread is no-op-warned by OTel's `ThreadLocalContextStorage` per the OTel `Scope` contract ("Trying to close scope which does not represent current context. Ignoring close."), and the SQL span's parent linkage to the construction-thread context is best-effort rather than guaranteed. Hosts intentionally fanning out result-set consumption to other threads MUST propagate context via OTel's standard `Context.wrap` / `Context.taskWrapping` pattern instead of relying on the wrapper's captured context. Track 6c's `OTelResultSetThreadAffinityTest` exercises both shapes: the same-thread close shape (assert passes; parent linkage holds), and the cross-thread close shape (assert fires with `-ea`; without `-ea` the wrapper still emits the span but the test asserts the documented degradation — `Span.fromContext(capturedContext)` matches the construction-time parent, but `Scope.close()` no-op-warns on the closing thread).
