...
**Explicit transaction tracking ID.** A host-provided string identifier passed to a transaction via `YTDBTransaction.withTrackingId(String)` (pre-existing on `YTDBTransaction` today). When set, `FrontendTransaction.getTrackingId(): String` returns the explicit value; when unset, the accessor falls back to `String.valueOf(getId())` so the existing internal-ID source remains the default. YTDB-496 lifts `getTrackingId()` from `YTDBTransaction` only onto the `FrontendTransaction` interface so both fire sites can read the same value through the interface without downcasting; the storage and setter on `YTDBTransaction` stay as-is. Both `QueryDetails.getTransactionTrackingId()` and `TransactionDetails.getTransactionTrackingId()` read through the lifted accessor, so explicit IDs surface in OTel span attributes (`youtrackdb.transaction.tracking_id`) and in custom listener implementations alike. → §"Class Design" and §"Listener registration and ordering".
...
The contrast with the query-side default (`OPENTELEMETRY_QUERY_SLOW_THRESHOLD_MILLIS=100`) is intentional. Queries are emitted at high frequency on read-heavy workloads, so dropping fast successful queries is the default; commits stay operationally interesting at any duration (fast commits show throughput; slow commits show lock or fsync contention), so the default is "emit every one" and operators opt into throttling.
