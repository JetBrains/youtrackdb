...
- `FrontendTransactionImpl.assertOnOwningThread()` is a `private` method called by every TX operation entry point (`begin`, `commit`, `rollback`, `query`, `command`, record-CRUD paths), so listener fires happen on the same thread as the surrounding `assertOnOwningThread` check.
