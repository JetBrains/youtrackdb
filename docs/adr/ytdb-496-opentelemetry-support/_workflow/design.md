...
3. try (Scope ignored = capturedContext.makeCurrent()):       // restore construction-time context so each
...
Placement of the wrapper after `queryStartedLifecycle(original)` is what lets YTDB-820 land without re-wiring SQL telemetry. The result-set store from [PR #1077](https://github.com/JetBrains/youtrackdb/pull/1077) hooks into the same three entry points listed above and, depending on its state, returns either the existing `LocalResultSet` or a new `CachedResultSetView`; the wrapper sits below the cache decision in the entry-point control flow (and one layer above whichever inner type the decision yields), so the fire site is uniform across the full inner-type matrix:
...
- Span count per query depends on Gremlin path. Path A (translated, post-PR-1038) emits one Gremlin span with no children because `YTDBMatchPlanStep` calls `SelectExecutionPlan.start()` directly without going through the SQL entry points. Path B (fallback) emits one parent Gremlin span and one child SQL span; the child carries the sanitized SQL translated from Gremlin and signals to operators that the traversal fell back to per-step SQL rather than benefiting from MATCH planning. Direct SQL emits one SQL span. DDL emits one SQL span. Multi-statement scripts emit one SQL span per script call. The Path B redundancy is intentional diagnostic information rather than noise.
- Multi-statement scripts via `DatabaseSessionEmbedded.computeScript(language, script, args)` (script-engine entry point, around line 753) emit exactly one SQL span per script call when the script executor routes through one of the three SQL entry points; inner statements run through `ScriptLineStep.start()` which calls each inner plan's `start()` directly without going through an entry point, so they do not double-fire. The outer span's `db.query.text` carries the full script text; inner statement timing accrues to the outer span. Per-statement breakdown of a script is a future-ticket concern.
