...
| `db.youtrackdb.span.thread_mismatch` | `InstrumentedSqlResultSet.close()` sets `true` when `Thread.currentThread() != constructionThread`; absent otherwise | boolean (rarely set; fires only when host fans out result-set consumption across threads) | makes the documented best-effort-linkage degradation directly filterable in trace viewers without log search; emitted by the SQL fire site only |
...
- Cross-thread `close()`: `InstrumentedSqlResultSet.close()` MUST be called from the same thread that constructed the wrapper. The wrapper enforces this with a `private final Thread constructionThread = Thread.currentThread()` field captured in the constructor, a Java `assert` at the top of `close()` for development builds, AND a `db.youtrackdb.span.thread_mismatch = true` boolean attribute set on the emitted SQL span whenever `Thread.currentThread() != constructionThread`. The assert message reads "InstrumentedSqlResultSet.close() called on a different thread than construction; parent-child span linkage is best-effort under this path". With assertions off in production (default), the assert costs nothing and the attribute is the only runtime signal — operators filter affected spans with `WHERE db.youtrackdb.span.thread_mismatch = true` instead of relying on a log search for OTel's `ThreadLocalContextStorage` no-op-warn line. When a host genuinely consumes a result-set across threads the wrapper still calls `capturedContext.makeCurrent()` and the listener still fires, but the inner `Scope.close()` on the closing thread is no-op-warned by OTel's `ThreadLocalContextStorage` per the OTel `Scope` contract ("Trying to close scope which does not represent current context. Ignoring close."), and the SQL span's parent linkage to the construction-thread context is best-effort rather than guaranteed. The attribute makes the degradation visible in trace viewers without operators having to know the assert-message text. Hosts intentionally fanning out result-set consumption to other threads MUST propagate context via OTel's standard `Context.wrap` / `Context.taskWrapping` pattern instead of relying on the wrapper's captured context. Track 6c's `OTelResultSetThreadAffinityTest` exercises both shapes: the same-thread close shape (assert passes; parent linkage holds; `thread_mismatch` attribute absent), and the cross-thread close shape (assert fires with `-ea`; without `-ea` the wrapper still emits the span carrying `thread_mismatch = true` and the test asserts both the attribute presence and the documented degradation — `Span.fromContext(capturedContext)` matches the construction-time parent, but `Scope.close()` no-op-warns on the closing thread).
