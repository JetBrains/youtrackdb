...
classDiagram
    ...
    class YTDBMatchPlanStep["YTDBMatchPlanStep (PR #1038)"] {
        +getMatchPlanInputs() MatchPlanInputs
    }
    class MatchPlanInputs["MatchPlanInputs (PR #1038)"] {
        <<record>>
        +pattern Pattern
        +aliasClasses Map
    }
    ...
    GremlinBytecodeClassifier ..> YTDBMatchPlanStep : reads MatchPlanInputs for Path A
    YTDBMatchPlanStep --> MatchPlanInputs : holds
...
The producer copies each `Classification(operationName, collectionName)` value into the `QueryDetails` accessors before the listener fires. No thread-local suppression mechanism is needed for nested SQL spans inside Gremlin traversals: the OTel module manages the Gremlin span lifecycle directly (opens at first `hasNext()`, closes at `YTDBQueryMetricsStep.close()`), and inner SQL spans emitted by `LocalResultSet.close()` read `Context.current()` at construction so they become children of the active Gremlin span automatically. See §"Context propagation in embedded" for the lifecycle and parent-child relationship.
...
The SQL path is symmetric. `LocalResultSet`, the wrapper that every `db.query()` / `db.command()` / `db.execute()` call returns, plays the role of `YTDBQueryMetricsStep` as the listener fire site: the constructor captures the start clock at `executionPlan.start()`, and `close()` fires the listener. `SqlSyntaxClassifier` replaces `GremlinBytecodeClassifier` for the accessor population. Span name construction, attribute mapping, and parent-context resolution are identical from the listener's point of view, because the listener reads `QueryDetails` accessors that have already been populated by whichever classifier ran. See §"SQL execution layer hook" for the SQL-side anatomy.
...
| `db.namespace` | Conditionally Required | `QueryDetails.getDatabaseName()` (Gremlin: `session.getDatabaseName()` at the `YTDBQueryMetricsStep` fire site; SQL: same at the `LocalResultSet` fire site, populated when the result-set is constructed) | Track 1 adds the default accessor on both `QueryDetails` and `TransactionDetails`; omitted when the session has no name |
...
**TL;DR.** The host application's active span automatically becomes the parent of every YTDB query span. `Context.current()` resolves to the host's span because the listener fires synchronously on the caller's thread per existing YTDB semantics, and transaction operations are pinned to the owner thread via `assertOnOwningThread`. For Gremlin traversals, the OTel module manages the Gremlin span lifecycle directly (opens on first `hasNext()`, closes at `YTDBQueryMetricsStep.close()`) so the Gremlin span is `Context.current()` during inner SQL execution; SQL spans emitted by `LocalResultSet.close()` then attach as children of the Gremlin span automatically. No thread-local suppression machinery is needed, the parent-child relationship is the natural shape produced by OTel's `Context` API. A regression test guards against future threading changes.
...
The Gremlin span lifecycle is OTel-module-managed. An OTel-specific hook on `YTDBQueryMetricsStep` (added by the OTel module's strategy) opens the Gremlin span via `tracer.spanBuilder(...).startSpan()` plus `scope = span.makeCurrent()` on the first `hasNext()` invocation, and closes the scope plus ends the span at `YTDBQueryMetricsStep.close()`. The hook is independent of the YTDB-native `QueryMetricsListener` API (which fires only `queryFinished` at close); the OTel `OTelQueryMetricsListener` does not emit the Gremlin span itself, the lifecycle hook does. During the iteration window between the first `hasNext()` and `close()`, any nested `LocalResultSet.close()` reads `Context.current()` and emits its SQL span as a child of the active Gremlin span. End result: Path B fallback shows one Gremlin span with one child SQL span; Path A (translated) shows one Gremlin span with no children; direct SQL shows one SQL span parented to whatever `Context.current()` resolves to outside any Gremlin step (host span if any, else trace root).
...
**TL;DR.** Two extraction paths feed one `Classification(operationName, collectionName)` value record. **Path A (translated):** when the step list contains a `YTDBMatchPlanStep` (which the [Gremlin-to-MATCH translator from PR #1038](https://github.com/JetBrains/youtrackdb/pull/1038) injects in place of the original step chain), the dispatcher reads `YTDBMatchPlanStep.getMatchPlanInputs()` and extracts operation and collection from the `MatchPlanInputs` IR (operation from the statement shape, typically `SELECT` for read patterns; collection from the first alias class in the pattern). **Path B (fallback):** when the translator declined and the original step chain survives, the dispatcher walks the TinkerPop `Bytecode` instruction list to identify the start step (`V`/`E`/`addV`/`addE`/`drop`) and the first label-bearing operator (`hasLabel`, `addV(X)` argument, `addE(X)` argument). The dispatcher returns `Optional.empty()` for both fields when the shape doesn't yield clean values; never throws.

**Path A (MatchPlanInputs) classification rules** — applied when `YTDBMatchPlanStep` is present in the step list:

| MatchPlanInputs shape | Operation name | Collection name source |
|---|---|---|
| Pattern with RETURN projection | `SELECT` | first node's class from `pattern.firstNode().aliasClasses`, else `Optional.empty()` |
| Pattern with aggregation (count / sum / etc., output type SCALAR) | `SELECT` | as above |
| Pattern with `group()` (output type MAP) | `SELECT` | as above |
| Union (`MultiPlanMatchStep` per PR #1038's Track 10) | `SELECT` | first child plan's first-node class, else `Optional.empty()` |
...
Implementation lives in `core/.../profiler/monitoring/GremlinBytecodeClassifier.java` as a static utility with one public entry point (`Classification classify(Traversal)`) and two private dispatch targets (`classifyFromMatchPlan(MatchPlanInputs)` for Path A; `classifyFromBytecode(Bytecode)` for Path B). The dispatcher checks `traversal.getSteps().stream().anyMatch(s -> s instanceof YTDBMatchPlanStep)`; when true, Path A; otherwise Path B. `YTDBQueryMetricsStep.close()` calls `classify(traversal)` directly when building the inline `QueryDetails` and stashes the returned `Classification` in two `Optional<String>` fields read back by `QueryDetails.getOperationName()` / `getCollectionName()`. The same `QueryDetails` instance also carries the `errorType` slot populated from any exception caught around the traversal-close path (the `step.next()` / `step.hasNext()` flow), so the slow-query threshold gate in `OTelQueryMetricsListener` can bypass on error. When no listener consults those accessors the work is paid (the call is unconditional inside the fire site), but the cost is one Path-A field read (translated) or one bytecode walk (fallback) reusing the same instruction-list traversal pattern as the existing `produceScript()` sanitization, measured in microseconds and dominated by the listener call itself.

### Strategy ordering: metrics step after translator

TinkerPop runs each `ProviderOptimizationStrategy` in topological order determined by `applyPrior()` and `applyPost()` declarations. Track 1's installer adds `applyPost(GremlinToMatchStrategy.class)` to `YTDBQueryMetricsStrategy`, so the metrics-step injection runs after PR #1038's translator. The injected step lands as the terminal element of whichever step list the traversal ends up with: a single `YTDBMatchPlanStep` (Path A) or the original step chain (Path B, when the translator declined). PR #1038's translator declares `applyPrior() = {}` and `applyPost() = {}` for itself, so the ordering is established by the metrics-strategy declaration alone. If PR #1038 lands first, no further coordination is required on its side. If YTDB-496 lands before PR #1038, the `applyPost` reference resolves to "no such strategy" and TinkerPop ignores it silently, keeping the order stable across the landing sequence.

If the order were reversed (metrics first, translator second), the translator's step-list replacement would wipe the injected metrics step, leaving the traversal with only `YTDBMatchPlanStep` and no fire site at close. The `applyPost` guard prevents this.

### `YTDBMatchPlanStep.getMatchPlanInputs()` accessor contract

To dispatch Path A correctly, the design needs the IR record the translator built. PR #1038's boundary step holds this record in a final field for plan reuse on `clone()` (per PR #1038's §"Boundary-step lifecycle"). YTDB-496 imposes one coordination requirement on PR #1038: a public getter on that field, `getMatchPlanInputs(): MatchPlanInputs`, returning the same value the translator constructed and passed to `MatchExecutionPlanner` per PR #1038's §"Class Design". Track 4 wires this as a coordination point; the getter lands in the PR #1038 changeset (not in YTDB-496 directly) so YTDB-496's classifier can compile against it without forking the boundary step.

For `MultiPlanMatchStep` (PR #1038's union variant holding `List<SelectExecutionPlan>` plus a list of per-child `MatchPlanInputs`), the classifier reads the first child's `MatchPlanInputs` because all union children agree on output type at translation time (per PR #1038's §"Boundary step output types").

Timing capture in `YTDBQueryMetricsStep.close()` follows the per-query mode resolution mechanism described in §"Query tagging and per-tag rule resolution": the step reads the query tag from `traversal.getConfig(YTDBQueryConfigParam.querySummary)` and calls `currentTx.resolveQueryMonitoringMode(tag)` to pick the clock source for this traversal. Two Gremlin traversals in the same transaction with different tags can therefore record at different precisions. Under Path B fallback the inner SQL queries spawned by the traversal each emit their own span via `LocalResultSet.close()`; these spans become children of the outer Gremlin span via OTel `Context.current()` propagation (see §"Context propagation in embedded"). Each level resolves its mode independently: the outer Gremlin step reads its mode from the traversal's tag; inner SQL spans read theirs from `Optional.empty()` and resolve to the per-TX default. The two modes may differ, which is acceptable because they measure different things (outer Gremlin step duration vs. inner SQL execution time).
...
- Mechanics: none
- External: [PR #1038 — Gremlin-to-MATCH translator design](https://github.com/JetBrains/youtrackdb/pull/1038) (Path A source; provides `YTDBMatchPlanStep`, `MatchPlanInputs`, `GremlinToMatchStrategy`)
...
SQL path (LocalResultSet constructor):
...
**TL;DR.** `LocalResultSet` is the natural outer-boundary fire site for SQL. Every `db.query()` / `db.command()` / `db.execute()` call returns a `LocalResultSet` that wraps the underlying `ExecutionPlan.start()` stream (line 45 of `LocalResultSet.java`). The constructor captures the start clock via mode-resolved timing (mode resolves from `currentTx.resolveQueryMonitoringMode(Optional.empty())` since SQL has no tag source); `close()` computes elapsed time, captures any exception thrown during iteration, and fires `queryFinished` on every listener in the snapshot of `currentTx.iterateAllQueryListeners()` taken at construction. Nested sub-plans (MATCH steps, sub-query steps, IF / WHILE flow control, script line steps) call `ExecutionPlan.start()` directly without wrapping in `LocalResultSet`, so they do not double-fire; sub-plan execution time accrues to the outer span. Multi-statement scripts (`ScriptExecutionPlan` wrapped in one outer `LocalResultSet`) emit one span per script as a whole, with inner statements not separately instrumented at this layer. SQL spans emitted by `LocalResultSet.close()` read `Context.current()` at construction, so under Gremlin Path B fallback (`YTDBGraphQuery.execute(...)` → `transaction.query(...)` → `LocalResultSet`) the inner SQL span becomes a child of the active Gremlin span automatically. No thread-local suppression mechanism is needed: the parent-child hierarchy is the natural shape produced by OTel's `Context` API.

Hook anatomy in `LocalResultSet` (constructor captures the start clock and stores state; `close()` fires the listener):

```text
Constructor(executionPlan, statement, rawSql, args, currentTx):
1. listeners = currentTx.iterateAllQueryListeners()  ← snapshot of (global registry + per-TX list)
2. If listeners empty:
     stream = executionPlan.start()                 // uninstrumented fast path
     instrumented = false
     return
   // listeners empty → zero overhead when OTel is off and no per-TX listener.
   // Path B Gremlin-driven SQL: instrumented path still runs; the emitted SQL span
   // becomes a child of the active Gremlin span via Context.current() at construction.
   // Path A translated Gremlin never reaches this constructor (YTDBMatchPlanStep
   // calls SelectExecutionPlan.start() directly without wrapping in LocalResultSet).
3. tag  = Optional.empty()                          ← SQL has no tag source
   mode = currentTx.resolveQueryMonitoringMode(tag) ← resolver short-circuits to per-TX default
   if LIGHTWEIGHT:
...
4. stream = executionPlan.start()
   instrumented = true
   Store: statement, rawSql, args, listeners, mode, startMillis, startEpochNanos, startNanoTime, caughtError = null

Iteration (hasNext / next):
   Delegate to stream.hasNext(ctx) / stream.next(ctx). If the stream throws,
   set caughtError = e and rethrow. No timing overhead in the hot iteration path.

close():
1. If not instrumented: stream.close(); return.
2. elapsedNanos = (mode == LIGHTWEIGHT)
                    ? ticker.approximateNanoTime() - startNanoTime
                    : System.nanoTime() - startNanoTime
3. Build QueryDetails (lazy sanitization + classification from statement;
...
4. For each listener in the snapshot:
     try { listener.queryFinished(details, startMillis, elapsedNanos); }
     catch (Exception | LinkageError | AssertionError t) { log WARN with listener class name }
5. stream.close()
```

All three SQL execution entry points produce a parsed `SQLStatement` plus the underlying `ExecutionPlan`, then construct a `LocalResultSet` that wraps the plan's `ExecutionStream`. `query(String, Object...)` (line 617 of `DatabaseSessionEmbedded`) parses, asserts `isIdempotent()`, builds the plan, constructs the result-set. `query(String, boolean, Map)` (line 652) follows the same shape; line-617 and line-652 share the parse-and-wrap pattern. `executeInternal()` (line 702) uses the pre-parsed statement when supplied (via the `execute(SQLStatement, Map<?,?>)` overload at line 697) and otherwise calls `SQLEngine.parse(...)`. The `LocalResultSet` constructor receives the `SQLStatement` (for lazy classification + sanitization), the raw SQL text (from `stringStatement` when non-null, else from `statement.getOriginalStatement()`), the args, the listener snapshot from `currentTx.iterateAllQueryListeners()`, and the chosen clock readings. The result-set itself never parses or executes; it owns the lifecycle from `executionPlan.start()` to `close()`.
...
- `stringStatement` can be null when an internal recursive call passes a pre-parsed `SQLStatement`. The `LocalResultSet` constructor falls back to `statement.getOriginalStatement()` for the raw SQL. If both are null (unusual), the result-set emits `db.query.text=""` and the span still carries operation / collection.
...
- The transaction tracking ID comes from `String.valueOf(currentTx.getId())` — `FrontendTransaction.getId(): long` already exists at line 215 and returns a stable internal ID. No new accessor is added for the SQL hook.
- An exception thrown during stream iteration (inside `hasNext` / `next`) propagates to the caller. The `LocalResultSet` captures the exception class on its way out and surfaces it through `QueryDetails.getErrorType()` when `close()` fires the listener; the span carries status ERROR and `error.type`. If the consumer abandons iteration without calling `close()` (rare; `try-with-resources` is the documented usage), no listener fire happens — the same drop-rate the existing per-traversal Gremlin path already has.
...
- Tag rules are immutable after process startup; mid-TX changes to the rule table are not supported in YTDB-496 (see §"Query tagging and per-tag rule resolution" Edge cases). Mid-TX changes to the per-TX default via `YTDBTransaction.withQueryMonitoringMode(...)` take effect immediately for the next query in the same TX because the `LocalResultSet` constructor re-reads `getDefaultQueryMonitoringMode()` per query: no snapshot, no `next begin()` cycle latency.
- Span count per query depends on Gremlin path. Path A (translated, post-PR-1038) emits one Gremlin span with no children because `YTDBMatchPlanStep` calls `SelectExecutionPlan.start()` directly without wrapping in `LocalResultSet`. Path B (fallback) emits one parent Gremlin span and one child SQL span; the child carries the sanitized SQL translated from Gremlin and signals to operators that the traversal fell back to per-step SQL rather than benefiting from MATCH planning. Direct SQL emits one SQL span. DDL emits one SQL span. Multi-statement scripts emit one SQL span per script call. The Path B redundancy is intentional diagnostic information rather than noise.
- Multi-statement scripts via `DatabaseSessionEmbedded.computeScript(language, script, args)` (script-engine entry point, around line 753) emit exactly one SQL span per script call. The script execution plan (`ScriptExecutionPlan`) is wrapped in one outer `LocalResultSet` at the call site; inner statements run through `ScriptLineStep.start()` which calls each inner plan's `start()` directly without wrapping in `LocalResultSet`, so they do not double-fire. The outer span's `db.query.text` carries the full script text; inner statement timing accrues to the outer span. Per-statement breakdown of a script is a future-ticket concern (would require either instrumenting `ScriptLineStep` directly or extending `QueryDetails` to carry a list of inner statements).
...
**TL;DR.** A wall-clock heartbeat gate inside `OTelQueryMetricsListener` emits one span every `N` ms regardless of query duration, so operators see a representative sample of the workload even when the slow-query gate filters out everything fast. Toggleable via `OPENTELEMETRY_QUERY_HEARTBEAT_SAMPLE_MILLIS` (default `0` = disabled); per-tag overrides through `OPENTELEMETRY_QUERY_HEARTBEAT_SAMPLE_TAG_RULES` resolve against a process-global `SampleHeartbeatResolver` consuming `TagRule<Long>` on the same sealed interface that powers mode and slow-query resolution. As with the slow-query gate, per-tag overrides apply only to Gremlin traversals; SQL statements always resolve to the global heartbeat interval. Composes disjunctively with the slow-query gate: a query emits if either gate passes, so heartbeat picks fast queries while slow-query still catches latency outliers.
...
Mechanism overview: the heartbeat gate runs at the top of `OTelQueryMetricsListener.queryFinished`, BEFORE the slow-query gate, with disjunctive composition — a query emits if either gate passes. The resolver reads the same Gremlin-only tag source as the slow-query gate (`g.with(YTDBQueryConfigParam.querySummary, ...)` for traversals; `Optional.empty()` for SQL). An `AtomicLong.compareAndSet` on `lastHeartbeatNanos` resolves the multi-thread race so exactly one query per heartbeat window claims the slot; losers fall through to the slow-query gate.

Pseudo-code, the CAS rationale, and edge cases (per-tag shared clock, error-bypass clock semantics, lookup-cost lower bound, custom-listener interaction) live in [`design-mechanics.md §"Time-based sampling"`](design-mechanics.md).
...
Mechanism overview: `OTelLogAppender` implements `LogAppenderHook` and is registered against `SLF4JLogManager` via `installAppenderHook(LogAppenderHook)`. Each call to `SLF4JLogManager.log(...)` invokes every installed hook's `onLog(requesterName, dbName, slf4jLevel, formattedMessage, thrown)` after the per-logger `isEnabledForLevel(level)` filter SLF4J already applies and before `logEventBuilder.log()` fires the SLF4J emit, so the hook sees the same already-formatted message and resolved database name SLF4J would emit. The hook maps the slf4j `Level` to an OTel `severityNumber`, applies the severity floor, captures `Context.current()` for trace correlation, and emits via `LogRecordBuilder.setSeverity(...).setBody(...).emit()`. A thread-local re-entrance guard breaks the cycle a log call inside the OTel exporter would otherwise create.
...
Mechanism overview: `OTelMetricsBridge.start()` calls `Profiler.getMetricsRegistry().forEachMetric(...)` to enumerate every metric registered at SDK init time, builds a `Map<String, ObservableInstrument>` keyed by OTel metric name, registers each callback against the `SdkMeterProvider`'s `Meter`, and subscribes a `Consumer<MetricRegistrationEvent>` via `addRegistrationListener(...)` so per-database metrics created after `start()` (every time a new database opens) auto-register their OTel callback on the registration event. The bridge also schedules a `ScheduledExecutorService` task to advance YTDB-side `TimeRate`/`Ratio` windows independently of the OTel `PeriodicMetricReader` cadence. A `MetricGroup` annotation on `MetricDefinition` (added by Track 8 — an enum with values `QUERIES`, `CACHE`, `STORAGE`, `WAL`, `LOCKS`, `TRANSACTIONS`) drives the `OPENTELEMETRY_METRICS_INCLUDED_GROUPS` filter.
...
- `LocalResultSet.close()` wraps the `listener.queryFinished(...)` call for native SQL fires in the same multi-catch union, iterating over the listener snapshot captured at constructor time.
