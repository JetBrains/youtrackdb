...
The canonical form is the entry-point-return wrap in the table above: each public SQL entry point returns `InstrumentedSqlResultSet.wrapIfListening(...)` below its cache decision, so the fire site covers every inner type the cache can yield, including the cache-hit and cacheable-miss `CachedResultSetView` returns that bypass `queryStartedLifecycle(...)` (see §"Coordination with YTDB-820 cache contract"). D44 records an optimization on top of this: collapse the five idempotent return-sites into a single wrap inside `queryStartedLifecycle(...)` (the private near-chokepoint they share; PSI find-usages lists only those five SQL entry-point sites today), plus the inline wrap-before-drain at the non-idempotent prefetch branch, reducing five edited return-sites to one method-body change. The collapse preserves fire semantics only for paths that route through `queryStartedLifecycle(...)`; because PR #1077's cache returns `CachedResultSetView` without calling it, the chokepoint form must also route the cache-return path through `queryStartedLifecycle(...)` (or wrap it separately) to keep cache-hit coverage. Until that is reconciled with PR #1077's control flow, the entry-point-return form the wrap-sites table documents is the safe default.
