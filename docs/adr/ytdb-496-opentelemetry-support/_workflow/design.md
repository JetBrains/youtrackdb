...
**`FrontendTransaction` accessors.** Three new methods on the `FrontendTransaction` interface so the SQL fire site and the commit fire site read consistent state without downcasting to `YTDBTransaction`:
...
Cardinality policy. Every attribute in `db.youtrackdb.*` MUST take values in one of four bounded shapes: boolean, small integer (under ~20 distinct values), enum (closed value set), or bounded-by-host-discipline (the host's explicit choice or query content carries the cardinality risk consciously). The bound applies to the *attribute value*, not to the *attribute key*; the key set is closed across the four tables in this section (vendor MVP, advisor, Gremlin DSL outer, Gremlin DSL inner), with one multi-instance key shape `db.youtrackdb.gremlin.embedded_sql.N` capped at N = 0..2 (at most 3 entries per outer span). Trace backends typically index per attribute key per distinct value, so an unbounded value range translates directly to backend storage and query cost. Three attributes fall in the host-discipline bucket: `db.youtrackdb.transaction.tracking_id` (hosts who set a low-cardinality ID like `"req-batch-2026-06-02"` get safe correlation; hosts who template a UUID per request blow cardinality and own that choice), `db.youtrackdb.gremlin.statements_summary` (semicolon-joined per-statement summary capped at 5 statements by YTDB), and `db.youtrackdb.gremlin.embedded_sql.N` (sanitized SQL of the first three embedded `CallStep` entries, unique per host query structure even after literal substitution).
