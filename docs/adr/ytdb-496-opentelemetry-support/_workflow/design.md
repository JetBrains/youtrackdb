...
All twenty-one knobs are `GlobalConfiguration` entries (env-var capable, created by Track 5); every default keeps YTDB silent on the OTel side, so the operator opts in. `plan/track-5.md` carries the canonical per-entry types; the table below is the operator-facing summary grouped by signal.
