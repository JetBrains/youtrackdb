...
Today YTDB has an internal `QueryMetricsListener` SPI that fires only on Gremlin traversal close, plus a `TransactionMetricsListener` that fires on write-transaction commit, but the listeners are per-transaction and the project ships no OTel binding. Native SQL queries (the path used for `db.command(...)`, MATCH, and DDL) currently fire neither listener. The design closes that gap with two load-bearing additions: a global listener registry in `core` so an OTel listener registered once at startup auto-applies to every subsequent transaction; and a new listener fire site at the SQL session-boundary, applied as a thin wrapper on whatever `ResultSet` each `db.query()` / `db.command()` / `db.execute()` call returns. The wrapper class `InstrumentedSqlResultSet` (new, in `internal.core.db`) is installed at the three SQL entry points on `DatabaseSessionEmbedded` (`query(String, Object...)` line 617, `query(String, boolean, Map)` line 652, `executeInternal()` line 702) as the outermost wrapper of whatever the entry point would otherwise return, after any cache-view construction ([PR #1077](https://github.com/JetBrains/youtrackdb/pull/1077)) and after any `queryStartedLifecycle(...)` decoration that runs on the bypass paths. The wrapper captures the start clock at construction; iteration delegates to the inner result-set; `close()` fires the listener with elapsed time. Every SQL statement type (SELECT, INSERT, UPDATE, DELETE, MATCH, DDL) and multi-statement scripts flow through this single outer-boundary fire site. Sub-plans called from MATCH steps, sub-query steps, and IF / WHILE control flow construct their own un-instrumented inner `LocalResultSet` instances via the existing 2-arg constructor and never reach the entry-point wrapper, so they do not double-fire.
...
**Explicit transaction tracking ID.** A host-provided string identifier passed to a transaction via `YTDBTransaction.withTrackingId(String)` (pre-existing on `YTDBTransaction` today). When set, `FrontendTransaction.getTrackingId(): String` returns the explicit value; when unset, the accessor falls back to `String.valueOf(getId())` so the existing internal-ID source remains the default. YTDB-496 lifts `getTrackingId()` from `YTDBTransaction` onto the `FrontendTransaction` interface so both fire sites read the same value through the interface without downcasting; Track 1 also moves the storage from `YTDBTransaction.trackingId` to `FrontendTransactionImpl` (one atomic commit) and rewires `withTrackingId(...)` to call the new package-private setter. Both `QueryDetails.getTransactionTrackingId()` and `TransactionDetails.getTransactionTrackingId()` read through the lifted accessor; the OTel listener surfaces the value as the `youtrackdb.transaction.tracking_id` span attribute ONLY when `OPENTELEMETRY_INCLUDE_TRACKING_ID=true` (default `false`). The opt-in gate caps cardinality: the default fallback `String.valueOf(getId())` is a per-process monotonic counter that would otherwise emit a unique attribute value per span, blowing per-attribute-value indexing on OTel backends. Hosts wanting cross-system correlation flip the flag and accept the cardinality cost; everyone else correlates via OTel-native `traceId`. → §"Class Design" and §"Listener registration and ordering".
...
| `youtrackdb.transaction.tracking_id` | `FrontendTransaction.getTrackingId()` (explicit when host set via `withTrackingId(...)`, else `String.valueOf(getId())` fallback) | UNBOUNDED — gated by `OPENTELEMETRY_INCLUDE_TRACKING_ID` (default `false`); opt-in only | violates the cardinality policy below — the default fallback is a per-process monotonic counter, so the attribute is omitted by default. Hosts wanting cross-system correlation opt in via the flag and accept the indexing cost; everyone else correlates via OTel-native `traceId`. |

Cardinality policy. Every attribute in `db.youtrackdb.*` MUST be bounded: boolean, small integer (under ~20 distinct values), or enum. The bound applies to the *attribute value*, not to the *attribute key* (the key set is closed, defined in this table). Trace backends typically index per attribute key per distinct value, so an unbounded value range translates directly to backend storage and query cost. `youtrackdb.transaction.tracking_id` is the one explicit exception — listed in the table for completeness, opt-in only via `OPENTELEMETRY_INCLUDE_TRACKING_ID=true` (default `false`), with operators accepting the cardinality cost as a deliberate trade-off for cross-system correlation.
...
1. **First `hasNext()` of `YTDBQueryMetricsStep`** — the step invokes the installed `GremlinSpanLifecycleHook.onFirstHasNext(details, startEpochNanos, parentContext)`. The OTel hook impl calls `tracer.spanBuilder(name).setParent(parentContext).setStartTimestamp(startEpochNanos, NS).startSpan()`, then `scope = span.makeCurrent()`. The span carries no sem-conv attributes yet. `Span.fromContext(Context.current())` now resolves to this Gremlin span; the step stores the `(span, scope)` pair internally.
2. **During iteration** — any inner `InstrumentedSqlResultSet` constructor reads `Context.current()` into its `capturedContext` field and binds the Gremlin span as the parent for the SQL span the wrapper will emit at `close()`.
3. **`YTDBQueryMetricsStep.close()`** — the step iterates the listener snapshot and calls `queryFinished(details, startedAtMillis, executionTimeNanos)` on each. `OTelQueryMetricsListener.queryFinished` reads `details.getQuerySource()`: on `GREMLIN`, it reads `Span.fromContext(Context.current())`, sets the sem-conv attributes (`db.system.name`, `db.query.text`, `db.operation.name`, `db.collection.name`, `db.youtrackdb.*`), and sets `error.type` + status `ERROR` when `details.getErrorType().isPresent()`. It does NOT call `spanBuilder()` or `span.end()`. On `SQL`, it creates a new span as in the workflow diagram.
...
**TL;DR.** A new wrapper class `InstrumentedSqlResultSet` in `internal.core.db` is the SQL fire site. The three `DatabaseSessionEmbedded` SQL entry points (`query(String, Object...)` line 617, `query(String, boolean, Map)` line 652, `executeInternal()` line 702) each end with `return InstrumentedSqlResultSet.wrapIfListening(inner, statement, rawSql, args, currentTx)` where `inner` is whatever the entry point would otherwise return: a `LocalResultSet` (registered in `activeQueries` via `queryStartedLifecycle(...)` on bypass / splice-failure paths) or a `CachedResultSetView` (returned directly by PR #1077's cache on hit and cacheable-miss paths without going through `queryStartedLifecycle`). The wrapper is the outermost layer in either case. The wrapper holds the inner `ResultSet`, captures the start clock at construction via mode-resolved timing (mode resolves from `currentTx.resolveQueryMonitoringMode(Optional.empty())` since SQL has no tag source), counts emitted rows during iteration, captures any exception thrown during iteration, and fires `queryFinished` on every listener in the snapshot of `currentTx.iterateAllQueryListeners()` taken at construction. Nested sub-plans (MATCH steps, sub-query steps, IF / WHILE flow control, script line steps) construct their own un-instrumented `LocalResultSet` instances via the existing 2-arg constructor and never reach this wrapper, so they do not double-fire; sub-plan execution time accrues to the outer span. Multi-statement scripts emit one span per script call, with inner statements not separately instrumented at this layer. SQL spans emitted by `InstrumentedSqlResultSet.close()` read `Context.current()` at construction, so under Gremlin Path B fallback (`YTDBGraphQuery.execute(...)` → `transaction.query(...)` → entry-point wrapper) the inner SQL span becomes a child of the active Gremlin span automatically. No thread-local suppression mechanism is needed: the parent-child hierarchy is the natural shape produced by OTel's `Context` API. `LocalResultSet.java` itself stays unchanged in Track 4 scope, keeping the YTDB-820 cache's stream-slot substitution untouched.
...
3. try {
     try (Scope ignored = capturedContext.makeCurrent()) {     // restore construction-time context so each
                                                              // listener's Context.current() read returns
                                                              // the parent span the wrapper bound at ctor.
       for each listener in the snapshot:
   ...
   } catch (Exception | LinkageError | AssertionError t) {    // outer guard covers Scope.makeCurrent() throws
                                                              // (e.g., concurrent SDK shutdown during active
                                                              // iteration): keep close() side-effects safe
     log WARN "Scope acquisition failed during listener fire"
   }
4. inner.close()
```
...
| Cache MISS, cacheable RECORD / MATCH shape | `CachedResultSetView` (cache substitutes `IdempotentExecutionStream` into the constructed `LocalResultSet`'s stream slot and returns the view) | yes |
...
| `computeScript()` script-engine entry (lines 753 + 813) | `ResultSet` from `executor.execute(...)` decorated by `queryStartedLifecycle(original)` | yes (one span per script call; inner statements accrue to it) |
...
Deferred coordination point: when YTDB-820's implementation lands a `getCacheOutcome()` accessor on `CachedResultSetView` (returning `hit | miss | bypass`), the wrapper will stamp `youtrackdb.query.cache.outcome` on the SQL span by type-checking the inner result-set. Until then, the attribute is omitted; the wrapper's existence is independent of the accessor's presence and ships in Track 4 without a hard dependency on YTDB-820's API surface. Documented in Track 4's References footer as a follow-up.
...
- Span count per query depends on Gremlin path. Path A (translated, post-PR-1038) emits one Gremlin span; child SQL spans appear only when the MATCH plan re-enters one of the three SQL entry points (a SQL-side UDF lookup, a sub-query expression evaluator, a context-variable resolution that issues `db.query(...)`). For the common case — a pure MATCH plan with no UDF / sub-query callbacks — Path A emits one Gremlin span with no SQL children. Path B (fallback) emits one parent Gremlin span and one child SQL span; the child carries the sanitized SQL translated from Gremlin and signals to operators that the traversal fell back to per-step SQL rather than benefiting from MATCH planning. Direct SQL emits one SQL span. DDL emits one SQL span. Multi-statement scripts emit one SQL span per script call. The Path B redundancy is intentional diagnostic information rather than noise; Path A re-entry through SQL entry points (UDF / sub-query) emits child spans that surface the inner SQL work to operators.
- Multi-statement scripts via `DatabaseSessionEmbedded.computeScript(language, script, args)` (script-engine entries at lines 753 and 813) emit exactly one SQL span per script call because Track 4 instruments `computeScript` itself with its own `InstrumentedSqlResultSet.wrapIfListening(...)` call (the script executor builds a `ScriptExecutionPlan` and drives `ScriptLineStep` for each inner statement without re-entering the three SQL entry points, so per-script instrumentation is the correct site). Inner statements accrue to the outer span's duration; the wrapper carries `statement=null` and the classifier returns `Optional.empty()`. The outer span's `db.query.text` carries the full script text. Per-statement breakdown of a script is a future-ticket concern.
...
- Cross-thread `close()`: a consumer that creates the result-set on thread A and closes it on thread B (e.g., from a pooled cleanup executor) still propagates the captured context to the emitted span — `capturedContext` is the host-bound `Context` from thread A. OTel's `Scope` API is documented as thread-affine on `makeCurrent()` / `close()`; cross-thread close may log a WARN from the SDK on some versions. Hosts intentionally fanning out result-set consumption to other threads should propagate context via OTel's standard `Context.wrap` / `Context.taskWrapping` pattern instead of relying on the wrapper's captured context.
