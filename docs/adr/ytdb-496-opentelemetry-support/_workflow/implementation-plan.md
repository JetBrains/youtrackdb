...
- **`FrontendTransactionImpl` (core)**: `beginInternal()` captures the registry snapshot into a new `txListenerSnapshot` field plus the active `QueryMonitoringMode` (default `LIGHTWEIGHT`, override propagated by `YTDBTransaction.withQueryMonitoringMode(...)` via a new package-private setter) into a sibling field. The method iterates the listener snapshot firing `transactionStarted`; `rollbackInternal()` iterates the same snapshot firing `transactionRolledBack` (gated by `txStartCounter == 0` so nested rollbacks don't double-fire). The existing private `notifyMetricsListener` wrapper covers commit success/failure unchanged; the same try/catch shape applies to the two new fires. A new `getQueryMonitoringMode()` accessor on the `FrontendTransaction` interface exposes the snapshotted mode to Track 4's SQL hook. `YTDBTransaction.doOpen()` / `doRollback()` are not touched — they delegate to the underlying impl, so Gremlin and SQL paths both go through the same fire sites.
...
- **Timing-mode uniformity**: every listener fire site in one transaction (`YTDBQueryMetricsStep.close()` for Gremlin, `DatabaseSessionEmbedded.executeInternal()` for SQL, `FrontendTransactionImpl.notifyMetricsListener()` for commit) reads timestamps from the same source per the `QueryMonitoringMode` snapshot captured at `beginInternal()`. `LIGHTWEIGHT` routes through `GranularTicker`; `EXACT` routes through `System.nanoTime()` / `System.currentTimeMillis()`. Checked by `OTelTimingModeTest` in Track 6.
...
- `DatabaseSessionEmbedded.executeInternal()`, new listener fire site wrapping `statement.execute(...)` for the SQL path. Uses `currentTx.getId()` (existing) as the tracking ID via `String.valueOf(...)`; no new `FrontendTransaction.getTrackingId()` accessor is added. Reads `currentTx.getQueryMonitoringMode()` (new accessor in Track 1) to route timing capture between `GranularTicker` (LIGHTWEIGHT) and `System.nanoTime()` (EXACT).
- `FrontendTransaction.getQueryMonitoringMode(): QueryMonitoringMode`, new accessor on the existing interface, returning the snapshot captured at `beginInternal()` time. Default `LIGHTWEIGHT`; `YTDBTransaction.withQueryMonitoringMode(EXACT)` propagates the override into the snapshot.
...
- [ ] Track 6: Test suite using `opentelemetry-sdk-testing`
  > Cover the listener-to-span mapping for both Gremlin and SQL paths (sem-conv attributes, span kinds, hierarchy parent/child links), context propagation in embedded (host span becomes parent), server-mode lifecycle (init/close), the exception-isolation invariant, and the timing-mode uniformity invariant. Uses `InMemorySpanExporter` so assertions run against real SDK behavior. Detailed description in `plan/track-6.md`.
  > **Scope:** ~8 steps covering Gremlin attribute mapping, SQL attribute mapping, hierarchy, context propagation, lifecycle, exception isolation, timing-mode uniformity, coverage gate.
  > **Depends on:** Track 3, Track 4, Track 5

## Plan Review
- [ ] Plan review (consistency + structural) — re-validation needed after Mutation 9 aligned the SQL hook timing capture with the per-TX `QueryMonitoringMode` (see `design-mutations.md` Mutation 9). Track 1 grew by one step (mode snapshot field + accessor), Track 4 edit 2 is rewritten (mode-aware capture), Track 6 grows by one test (`OTelTimingModeTest`), the plan adds a "Timing-mode uniformity" invariant and a new Integration Point, and D8 Rationale + Component Map bullets for `FrontendTransactionImpl` and `DatabaseSessionEmbedded` now describe the mode-routing. The next `/execute-tracks` invocation will re-run Phase 2 against the revised plan.
