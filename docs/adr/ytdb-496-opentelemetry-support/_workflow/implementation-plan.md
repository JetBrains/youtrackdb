...
- `DatabaseSessionEmbedded.executeInternal()`, new listener fire site wrapping `statement.execute(...)` for the SQL path. See design.md §"SQL execution layer hook" for the `QueryMonitoringMode` routing, the tracking-ID source (`currentTx.getId()` via `String.valueOf(...)`), and the timer pattern.
