...
- **`YTDBGraphQuery` (core)**: Gremlin-to-SQL bridge whose `execute(session)` runs the translated SQL via `session.getActiveTransaction().query(...)`. Track 4 wraps that call in a try-with-resources `GremlinSqlSuppression.activate()` token so the SQL helper short-circuits for the duration of Gremlin-driven SQL. Counter is re-entrant so nested Gremlin steps inside one another do not interfere.
