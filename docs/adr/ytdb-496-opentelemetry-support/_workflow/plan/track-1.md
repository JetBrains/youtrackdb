...
- Provides for Track 4: the `SqlSyntaxClassifier` static helper called by `DatabaseSessionEmbedded.executeStatementWithMetrics` (invoked from both `query()` and `executeInternal()`) to populate operation/collection on the inline SQL `QueryDetails`; the global query snapshot accessible from `currentTx`; the `currentTx.resolveQueryMonitoringMode(Optional<String>)` accessor returning the per-query `LIGHTWEIGHT` / `EXACT` value the SQL hook uses to route timing capture (LIGHTWEIGHT → `GranularTicker`, EXACT → `System.nanoTime()`); plus `currentTx.getDefaultQueryMonitoringMode()` as fallback for queries with no tag rule match. Track 4 reads the query tag from `statement.getQueryTag()` (its own deliverable from the SQL parser hint `/*+ TAG=X */`) and passes it to the resolver.
