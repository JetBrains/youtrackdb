...
The track touches none of the actual emission code (that lives in Track 3). It adds API surfaces, consolidates the TX-lifecycle fires inside `FrontendTransactionImpl`, refactors the existing single-listener call sites to iterate the per-TX snapshot, and widens the existing exception-isolation wrappers from `Exception` to `Exception | LinkageError | AssertionError`. Concrete deliverables:
...
9. Exception-isolation widening: the existing `} catch (Exception e) {` blocks in `FrontendTransactionImpl.notifyMetricsListener:730` and `YTDBQueryMetricsStep:148` widen to `} catch (Exception | LinkageError | AssertionError t) {`. The two new TX-lifecycle fires reuse the same wrapper shape in `FrontendTransactionImpl`. The union is deliberately narrower than `Throwable`: it isolates OTel-typical failure modes (misconfigured SDK throwing `IllegalStateException`, missing exporter classes throwing `NoClassDefFoundError`, listener assertions) while letting `VirtualMachineError` and `ThreadDeath` propagate per JLS guidance.
10. `YTDBQueryMetricsStep` reads the query's `QueryMonitoringMode` via `txStateAccessor.resolveQueryMonitoringMode(traversal.getConfig(YTDBQueryConfigParam.querySummary))` — per-query resolution from the traversal's tag using the resolver from deliverable 7. The step caches the resolved `isLightweight` at construction time (it stays once-per-step rather than once-per-method-call to keep `hasNext()` / `next()` / `close()` consistent within one step iteration). Required for the per-query Timing-mode uniformity invariant: both the Gremlin step and the SQL hook resolve from the same query tag and therefore reach the same mode value, while a different traversal with a different tag in the same TX can pick a different mode.
