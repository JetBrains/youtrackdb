...
6. Snapshot wiring at TX begin: `FrontendTransactionImpl.beginInternal()` captures the global registry snapshots (`List<QueryMetricsListener>` and `List<TransactionMetricsListener>`) and the active `QueryMonitoringMode` into new fields before incrementing `txStartCounter` so nested begins reuse the outermost snapshot. Default mode is `LIGHTWEIGHT`. A package-private setter `setQueryMonitoringModeSnapshot(QueryMonitoringMode)` exists on `FrontendTransactionImpl`, called exactly once per TX by `YTDBTransaction.doOpen()` right after `beginInternal()` returns (and by `DatabaseSessionEmbedded.begin()` for the native-SQL path) to propagate the YTDBTransaction's current `queryMonitoringMode` field into the snapshot. The snapshot is then **immutable for the transaction's lifetime**: mid-TX `YTDBTransaction.withQueryMonitoringMode(...)` calls mutate the YTDBTransaction field but do not call the setter, so they take effect on the next `begin()` cycle, not the active TX. A new `getQueryMonitoringMode()` accessor on the `FrontendTransaction` interface lets the SQL hook in Track 4 read the snapshotted mode from inside `DatabaseSessionEmbedded.executeInternal()`. The fields clear on outermost commit / rollback.
...
10. `YTDBQueryMetricsStep` reads `QueryMonitoringMode` from the immutable `FrontendTransactionImpl` snapshot via `txStateAccessor.getQueryMonitoringMode()` (the new accessor from deliverable 6) instead of from the mutable `YTDBTransaction.queryMonitoringMode` field. The step continues to cache `isLightweight` at construction time; cache consistency across `hasNext()` / `next()` / `close()` is guaranteed by the snapshot's immutability post-`beginInternal()`. Required for the Timing-mode uniformity invariant: without this change, a `YTDBTransaction.withQueryMonitoringMode(EXACT)` call between step injection and the step's first iteration would cause the Gremlin span to disagree with the SQL hook and commit timer (which already read from the immutable snapshot).
...
The fifth edit wires `FrontendTransactionImpl.beginInternal()` to capture the global query and transaction snapshots into two new fields before `txStartCounter` increments, plus a third field holding the active `QueryMonitoringMode` (default `LIGHTWEIGHT`). A package-private setter `setQueryMonitoringModeSnapshot(QueryMonitoringMode)` exists on `FrontendTransactionImpl` and is called exactly once per TX by `YTDBTransaction.doOpen()` right after `beginInternal()` returns (and by `DatabaseSessionEmbedded.begin()` for the native-SQL path) to propagate the YTDBTransaction's current mode into the snapshot. The snapshot is then immutable for the transaction's lifetime: a mid-TX `YTDBTransaction.withQueryMonitoringMode(...)` mutates the YTDBTransaction field but does not call the setter, so it takes effect on the next `begin()` cycle. A public `getQueryMonitoringMode()` accessor on `FrontendTransaction` returns the snapshotted mode for downstream consumers (Track 4's SQL hook reads it from `currentTx`; the Gremlin step reads it via `txStateAccessor` for the per-step cache). The same edit then iterates the transaction snapshot calling `transactionStarted(details)` against each listener, inside the existing exception-isolation wrapper shape. `rollbackInternal()` iterates the snapshot calling `transactionRolledBack(details)` when `txStartCounter == 0` (so nested rollbacks don't double-fire). On outermost commit / rollback the snapshots clear.
...
The seventh edit refactors the single-listener call sites: `YTDBQueryMetricsStep.close()` now iterates every query listener in the per-TX snapshot (which includes any per-TX `withQueryListener` plus the global snapshot taken at begin), and `FrontendTransactionImpl.notifyMetricsListener()` iterates the transaction snapshot for commit success/failure. Each per-iteration call sits inside the existing try/catch wrapper, widened to `Exception | LinkageError | AssertionError`. The same edit switches `YTDBQueryMetricsStep`'s timing-mode read source from `ytdbTx.getQueryMonitoringMode()` (mutable YTDBTransaction field) to `txStateAccessor.getQueryMonitoringMode()` (immutable post-begin `FrontendTransactionImpl` snapshot). The construction-time `isLightweight` cache (line 68) stays, but now caches from the snapshot. Cache consistency across `hasNext()` / `next()` / `close()` follows from the snapshot's immutability for the TX lifetime, so all three sites dispatch on the same value, and the Gremlin / SQL / commit fire sites all read the mode from one source. Zero hot-path overhead change: the read still happens once per step at construction.
...
- `core/src/main/java/com/jetbrains/youtrackdb/internal/core/tx/FrontendTransactionImpl.java` (snapshot capture in `beginInternal` for listeners + `QueryMonitoringMode`, lifecycle fires in `beginInternal` + `rollbackInternal`, iterate transaction snapshot in `notifyMetricsListener`, widen wrappers to `Exception | LinkageError | AssertionError`)
...
- `core/src/main/java/com/jetbrains/youtrackdb/internal/core/gremlin/YTDBTransaction.java` (`doOpen()` calls `frontendTx.setQueryMonitoringModeSnapshot(queryMonitoringMode)` once right after `beginInternal()` returns to propagate the current mode into the snapshot; `withQueryMonitoringMode(...)` continues to mutate the YTDBTransaction field but does NOT call the setter — mid-TX mode flips take effect on the next `begin()`; existing `getQueryMonitoringMode()` accessor unchanged)
