...
The TX-lifetime span hierarchy is handled by carrying the TX-span Context through the listener registry. When `transactionStarted` fires, `OTelTransactionMetricsListener` creates the TX span and stores it in a `ThreadLocal<Context>` (because the callbacks fire on the owner thread per D4). The query listener reads `Context.current()` which by then contains the TX span as parent.
