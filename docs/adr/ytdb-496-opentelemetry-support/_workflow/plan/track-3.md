...
The TX-lifetime span hierarchy is handled by carrying the TX-span `Context` through the TX listener's **instance** ThreadLocal. When `transactionStarted` fires, `OTelTransactionMetricsListener` creates the TX span, derives `contextWithSpan = Context.current().with(txSpan)`, and stores that `Context` in its own instance `ThreadLocal<Context>` field `activeTxContext` (callbacks fire on the owner thread per D4 because `FrontendTransactionImpl.assertOnOwningThread` pins TX ops). The query listener holds an explicit reference to its paired TX listener (injected by `YouTrackDBOpenTelemetry` at construction time) and calls `txListener.activeTxContext()` to read the context for parent-span propagation. No static state, no `@AfterEach` cleanup needed for cross-test isolation — each test instantiates its own facade and discards it.
...
1. `OTelQueryMetricsListener implements QueryMetricsListener`: translates one `queryFinished` callback into one span (kind = `clientKind` constructor argument — INTERNAL embedded / CLIENT server per D5) with sem-conv attributes. Constructor signature `(Tracer tracer, SpanKind clientKind, OTelTransactionMetricsListener txListener)`; the injected `txListener` reference is what lets the query listener read the active TX context without static state.
2. `OTelTransactionMetricsListener implements TransactionMetricsListener`: translates `transactionStarted` into an INTERNAL TX span (logical container, always INTERNAL in both modes); `writeTransactionCommitted` into a child commit span with kind = `clientKind`; `writeTransactionFailed` and `transactionRolledBack` into proper end states. Holds an **instance** `ThreadLocal<Context>` field `activeTxContext` exposed via a package-private accessor. Constructor signature `(Tracer tracer, SpanKind clientKind, long defaultCommitThresholdNanos)`; the new `defaultCommitThresholdNanos` argument carries the commit-side slow-query threshold per D38 (default `0` = emit every successful commit; positive value gates `executionTimeNanos < threshold` for successful commits only; failed commits always bypass). The gate sits at the top of `writeTransactionCommitted` and returns early before any span allocation when the comparison drops the commit; `writeTransactionFailed` is unchanged.
3. `YouTrackDBOpenTelemetry`: facade with static entry points (`setOpenTelemetry`, `shutdown`) and instance-managed state. Holds the `Tracer` instance built via `getTracer("com.jetbrains.youtrackdb", YouTrackDBConstants.getRawVersion())` and the `serverMode` flag selected at SDK wiring time. Builds the `OTelTransactionMetricsListener` / `OTelQueryMetricsListener` pair together so the query listener can be constructed with an explicit reference to its paired TX listener (eliminates the need for a static accessor on the TX listener). Reads `OPENTELEMETRY_COMMIT_SLOW_THRESHOLD_MILLIS` from `GlobalConfiguration` and multiplies to nanoseconds to populate the `defaultCommitThresholdNanos` argument on `OTelTransactionMetricsListener` (D38), alongside the existing `OPENTELEMETRY_QUERY_SLOW_THRESHOLD_MILLIS` → `defaultThresholdNanos` and `OPENTELEMETRY_QUERY_HEARTBEAT_SAMPLE_MILLIS` → `defaultHeartbeatNanos` reads for `OTelQueryMetricsListener`.
...
The third edit implements `OTelQueryMetricsListener`. On `queryFinished`, it reads the active TX context from the injected `txListener.activeTxContext()` (when a TX is in progress) or falls back to `Context.current()` when no TX-scoped context is set, builds a span name using the fallback chain (`querySummary` → `operationName + collectionName` → `collectionName` → `db.system.name`), sets sem-conv attributes (`db.system.name=youtrackdb`, `db.query.text`, `db.query.summary`, `db.operation.name`, `db.collection.name` when present), starts the span with the `startedAtMillis` timestamp, ends it with `startedAtMillis + executionTimeNanos`. Span kind from the `clientKind` constructor argument (INTERNAL embedded / CLIENT server per D5).

The fourth edit implements `OTelTransactionMetricsListener` with an **instance** `ThreadLocal<Context>` field `activeTxContext`. On `transactionStarted`, opens an INTERNAL TX span as child of `Context.current()` and stores `Context.current().with(txSpan)` (a single `Context` value) in the instance ThreadLocal. On `writeTransactionCommitted`, fetches the ThreadLocal context, opens a child commit span with kind = `clientKind` using `commitAtMillis`+`commitTimeNanos`, ends the TX span with OK status. On `writeTransactionFailed`, same but TX span gets ERROR status with `error.type` from the cause. On `transactionRolledBack`, ends the TX span without a commit child (no failure attribute). The ThreadLocal is cleared in a `finally` block. Because the ThreadLocal is per-instance, multiple `YouTrackDBOpenTelemetry` facades coexisting in the same JVM (e.g., a test that builds a fresh SDK per test method) never share state.

The fifth edit wires the `Context.makeCurrent()` semantics inside the query listener. Because TX listeners only set their instance `ThreadLocal<Context>` but do not call `makeCurrent()` (that would leak the context outside YTDB), the query listener calls its injected `txListener.activeTxContext()` to obtain the TX-scoped context for parent-span propagation. The lookup is a package-private **instance** method to keep coupling explicit and to avoid process-global state — a deliberate replacement of the static accessor pattern.
...
- A Gremlin query `g.V().hasLabel("User").toList()` in embedded mode emits one INTERNAL span named `SELECT User` with attributes `db.system.name=youtrackdb`, `db.query.text=g.V().hasLabel(?).toList()` (sanitized), `db.query.summary=SELECT User`, `db.operation.name=SELECT`, `db.collection.name=User`. The same query in server mode emits the same span but with kind CLIENT (per D5).
- A Gremlin query labeled via `g.with(YTDBQueryConfigParam.querySummary, "findActiveUsers")...` emits a span named `findActiveUsers`.
- A write transaction containing two queries and a commit produces four spans: one INTERNAL TX span (parent, always INTERNAL in both modes), two query spans (children of TX) with kind = `clientKind`, one commit span (child of TX) with kind = `clientKind`. In embedded mode all three children are INTERNAL; in server mode all three are CLIENT. The commit span overlaps the tail of the TX span.
