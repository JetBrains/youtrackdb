...
Add `InstrumentedSqlResultSet` as a new `ResultSet` implementation in `core/src/main/java/com/jetbrains/youtrackdb/internal/core/db/InstrumentedSqlResultSet.java`. Wire all five wrap sites on `DatabaseSessionEmbedded` to call `InstrumentedSqlResultSet.wrapIfListening(...)` after their existing `queryStartedLifecycle(original)` (or after `LocalResultSetLifecycleDecorator` construction on the non-idempotent prefetched branch): `query(String, Object...)` line 617, `query(String, boolean, Map)` line 652, `executeInternal()` line 702 (BOTH return branches — idempotent line 742 and non-idempotent prefetched line 738), and `computeScript(...)` entries at lines 753 and 813. Implement `SqlSanitizer` as an AST-walk static utility in `core` (alongside Track 1's `SqlSyntaxClassifier`). The SQL classifier and sanitizer are both called by the wrapper's lazy `QueryDetails` accessors. Tag source for SQL is `Optional.empty()` (Gremlin-only tag source per D15); the resolver short-circuits to the per-TX default. `LocalResultSet.java` is NOT modified by this track.
...
4. Wire all five wrap sites on `core/src/main/java/com/jetbrains/youtrackdb/internal/core/db/DatabaseSessionEmbedded.java` to call the wrapper factory. The five sites and their edits:
   ...
   - `executeInternal()` line 702, idempotent branch (line 742): same replacement.
   - `executeInternal()` line 702, non-idempotent prefetched branch (line 738): replace `var result = new LocalResultSetLifecycleDecorator(prefetched); return result;` with `var lifecycled = new LocalResultSetLifecycleDecorator(prefetched); return InstrumentedSqlResultSet.wrapIfListening(lifecycled, statement, rawSql, args, currentTx);`. Without this edit every `db.execute(...)` write (INSERT, UPDATE, DELETE, CREATE CLASS, DROP CLASS, ALTER CLASS) emits zero spans, AND every `g.yql("INSERT ...")` / `g.command("CREATE CLASS ...")` Gremlin DSL write (which routes through `session.execute(SQLStatement, Map)` line 697 → `executeInternal()` non-idempotent branch) emits zero inner SQL spans (D39).
   - `computeScript(...)` at lines 753 and 813: wrap the script's outer ResultSet at each return. Without this every multi-statement script via `db.executeSqlScript(...)` emits zero spans.
...
- A `g.yql("BEGIN")` call short-circuits in `executeCommand` before reaching `session.execute(...)` (returns `SqlCommandExecutionResult.unit()`); the wrapper is NOT constructed; no inner SQL span. Outer Gremlin span carries `db.operation.name = "BEGIN"` and `youtrackdb.gremlin.has_transaction_control = true` from the D40 classifier extension.
- A direct `db.query("SELECT FROM User")` call without a wrapping Gremlin span sees `getInvokedVia() = Optional.of("native")` because `Span.fromContext(capturedContext)` carries no `youtrackdb.gremlin.dsl_method` attribute.
...
- `core/src/main/java/com/jetbrains/youtrackdb/internal/core/db/DatabaseSessionEmbedded.java` (five wrap sites at lines 638, 679, 738, 742, 753, 813 — note `executeInternal()` line 702 contributes two return statements, the idempotent path at line 742 and the non-idempotent prefetched branch at line 738; each call site invokes `InstrumentedSqlResultSet.wrapIfListening(lifecycled, statement, rawSql, args, currentTx)` after the existing `queryStartedLifecycle(original)` call or `LocalResultSetLifecycleDecorator` construction).
...
- `youtrackdb.query.cache.outcome` span attribute — wired only when `CachedResultSetView.getCacheOutcome()` is present on `develop` at Track 4 entry (PR #1038 + PR #1077 / YTDB-820 baseline). When absent, the attribute lands in YTDB-707 follow-up and Track 4 ships without the type-check.
