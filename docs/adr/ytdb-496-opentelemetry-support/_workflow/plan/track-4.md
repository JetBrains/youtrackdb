...
5. Wire `GremlinSqlSuppression.activate()` into BOTH `YTDBGraphQuery.execute(session)` (lines 23-26) AND `YTDBGraphQuery.explain(session)` (line 31) via try-with-resources around the existing `transaction.query(...)` calls. The explain wire prevents parasitic SQL spans when Gremlin uses explain-introspection (e.g., `YTDBGraphStep.usedIndexes`).
...
The fourth edit wires `GremlinSqlSuppression.activate()` into BOTH `YTDBGraphQuery.execute(session)` at `core/.../gremlin/YTDBGraphQuery.java` lines 23-26 AND `YTDBGraphQuery.explain(session)` at line 31. The existing `return transaction.query(this.query, this.params);` (in `execute`) and `try (var resultSet = transaction.query(String.format("EXPLAIN %s", query), params))` (in `explain`) are each wrapped in a `try (var ignored = GremlinSqlSuppression.activate()) { ... }`. Cleanup runs even if the SQL throws. The explain wire prevents parasitic SQL spans when Gremlin uses explain-introspection via `YTDBGraphStep.usedIndexes`.
