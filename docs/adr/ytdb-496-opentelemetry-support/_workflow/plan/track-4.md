...
Add `InstrumentedSqlResultSet` as a new `ResultSet` implementation in `core/src/main/java/com/jetbrains/youtrackdb/internal/core/db/InstrumentedSqlResultSet.java`. **Before wiring, evaluate the single-chokepoint consolidation.** Five of the six sites end in `return queryStartedLifecycle(original)` (the private near-chokepoint at `DatabaseSessionEmbedded` ~L800). Wrapping inside `queryStartedLifecycle` cuts those five edited return sites to one with less line-number drift and identical fire semantics, at the cost of a 3-parameter signature change on `queryStartedLifecycle` to thread `statement` / `rawSql` / `args`. The sixth site (non-idempotent prefetched branch at lines 733-739) cannot be consolidated into the chokepoint because it requires structural restructuring: the wrapper sits BEFORE the eager `forEachRemaining(...)` drain so `executionTimeNanos` covers the executor's drain work rather than the consumer's no-op iteration over the pre-fetched `InternalResultSet`. Adopt the chokepoint form for the five idempotent sites unless decomposition surfaces a concrete blocker (for example a site that reaches `queryStartedLifecycle` without statement context); the sixth site always restructures inline. Otherwise wire all six wrap sites on `DatabaseSessionEmbedded` to call `InstrumentedSqlResultSet.wrapIfListening(...)`: at the five idempotent sites (`query(String, Object...)` line 617, `query(String, boolean, Map)` line 652, `executeInternal()` idempotent branch at line 742, and the two `computeScript(...)` returns at lines 785 and 847 with method declarations at 753 and 813) the wrap goes AFTER `queryStartedLifecycle(original)`. At the sixth site (`executeInternal()` non-idempotent prefetched branch, lines 733-739) the wrap sits BEFORE the drain: `var instrumented = wrapIfListening(original, ...); instrumented.forEachRemaining(prefetched::add); instrumented.close(); return new LocalResultSetLifecycleDecorator(prefetched);`. Implement `SqlSanitizer` as an AST-walk static utility in `core` (alongside Track 1's `SqlSyntaxClassifier`). The SQL classifier and sanitizer are both called by the wrapper's lazy `QueryDetails` accessors. Tag source for SQL is `Optional.empty()` (Gremlin-only tag source per D15); the resolver short-circuits to the per-TX default. `LocalResultSet.java` is NOT modified by this track.
...
`DatabaseSessionEmbedded` has six wrap sites Track 4 instruments. The three SQL execution entry points: `query()` at lines 617-686 (backs `db.query(...)` and the Gremlin bridge), the Map-args overload at line 652, and `executeInternal()` at lines 702-751 (backs `db.command(...)` / `db.execute(...)` AND every `g.yql(...)` / `g.command(...)` Gremlin DSL service-call per D39 — `session.execute(SQLStatement, Map)` line 697 and `schemaSession.command(SQLStatement, Map)` line 4517 both funnel through this method). All three follow the same shape: parse via `SQLEngine.parse(...)`, build the execution plan via `statement.execute(this, args, true)`, route the result through `queryStartedLifecycle(original)` for `activeQueries` tracking, return. Inside `executeInternal()` BOTH return branches need wrapping. The idempotent path that ends with `queryStartedLifecycle(original)` at line 742 (stream-style ResultSets for read queries) gets the standard wrap-after-lifecycle treatment. The non-idempotent prefetched branch at lines 733-739 (writes and DDL) requires a different shape: the wrapper sits BEFORE the eager `forEachRemaining(...)` drain so `executionTimeNanos` covers the executor's drain work, with the caller still receiving a wrapper-free `LocalResultSetLifecycleDecorator(prefetched)`. Without wrapping the non-idempotent branch every `db.execute(...)` write would emit zero spans; without the wrapper-before-drain restructure that wrap would emit a span with near-zero duration because timing would only cover the consumer's no-op iteration over the pre-fetched `InternalResultSet`. The remaining two wrap sites are the `computeScript(...)` returns at lines 785 and 847 (method declarations at 753 and 813) backing multi-statement scripts via `db.executeSqlScript(...)`. Track 4 adds one trailing call per wrap site to wrap the lifecycled result:
...
4. Wire all six wrap sites on `core/src/main/java/com/jetbrains/youtrackdb/internal/core/db/DatabaseSessionEmbedded.java` to call the wrapper factory. The six sites and their edits:
   ...
   - `executeInternal()` line 702, non-idempotent prefetched branch (lines 733-739): restructure so the wrapper sits **before** the eager drain rather than around the post-drain decorator. Replace the existing block

     ```java
     var prefetched = new InternalResultSet(this);
     original.forEachRemaining(prefetched::add);   // executor work happens here
     original.close();
     ...
     var prefetched = new InternalResultSet(this);
     instrumented.forEachRemaining(prefetched::add);   // drain through the wrapper so timing covers executor work
     instrumented.close();                              // fires queryFinished with real executor elapsed time
     return new LocalResultSetLifecycleDecorator(prefetched);  // caller gets a wrapper-free decorator; no double-fire on consumer close
     ```

     Critical for measurement: the wrapper must capture its start clock BEFORE `forEachRemaining(...)` runs so `executionTimeNanos` covers the executor's drain work. If the wrapper sat AFTER drain (the original Track 4 plan), every `db.execute(...)` write recorded ~0 ns elapsed because all DB work would already have completed before timing began. With the wrapper before drain, under `LIGHTWEIGHT` the ticker delta covers the drain; under `EXACT` the per-call `System.nanoTime()` accumulator captures every `original.next()` the drain pulls. Consumer-facing semantics are preserved: caller still receives a `LocalResultSetLifecycleDecorator` (no wrapper visible at the API boundary), iteration over the pre-fetched in-memory `InternalResultSet` does not refire the listener (the wrapper's `close()` already ran before the return). Without this restructure every `db.execute(...)` write (INSERT, UPDATE, DELETE, CREATE CLASS, DROP CLASS, ALTER CLASS) plus every `g.yql("INSERT ...")` / `g.command("CREATE CLASS ...")` Gremlin DSL write (which routes through `session.execute(SQLStatement, Map)` line 697 → `executeInternal()` non-idempotent branch) emits a span with near-zero duration — exactly the operations operators most want to trace.
...
The fourth edit wires all six wrap sites on `DatabaseSessionEmbedded` to call the wrapper factory. Five of the six sites place the wrap after `queryStartedLifecycle(original)`: `query(String, Object...)` line 617 (return at line 638), `query(String, boolean, Map)` line 652 (return at line 679), `executeInternal()` line 702 idempotent branch at line 742, and the two `computeScript(...)` returns at lines 785 and 847 (method declarations at 753 and 813). Each wrap site already has the parsed `SQLStatement` in scope after the `SQLEngine.parse(...)` call. The mechanical change at these five sites: replace `return queryStartedLifecycle(original);` with `var lifecycled = queryStartedLifecycle(original); return InstrumentedSqlResultSet.wrapIfListening(lifecycled, statement, rawSql, args, currentTx);`. The sixth wrap site (`executeInternal()` non-idempotent prefetched branch, lines 733-739) requires a different shape: the wrapper sits **before** the eager `forEachRemaining(...)` drain so timing covers the executor's drain work, not after the post-drain decorator construction. Restructure that block to `var instrumented = InstrumentedSqlResultSet.wrapIfListening(original, statement, rawSql, args, currentTx); var prefetched = new InternalResultSet(this); instrumented.forEachRemaining(prefetched::add); instrumented.close(); return new LocalResultSetLifecycleDecorator(prefetched);`. The wrapper's `close()` fires `queryFinished` after the drain finishes (the real executor work elapsed), the caller receives a wrapper-free `LocalResultSetLifecycleDecorator`, and consumer iteration over the pre-fetched in-memory `InternalResultSet` does not refire the listener. Without this restructure every `db.execute(...)` write plus every `g.yql("INSERT ...")` / `g.command("CREATE CLASS ...")` Gremlin DSL write (D39) emits a span with near-zero duration because the wrapper would otherwise time only the consumer's no-op iteration over an already-drained `InternalResultSet`.
...
- A `g.command("INSERT INTO Order SET amount = ?", 100)` call routes through `executeCommand` → `session.execute(SQLStatement, Map)` → `executeInternal()` non-idempotent prefetched branch (restructured at lines 733-739 so the wrapper sits before the eager `forEachRemaining(...)` drain); fires the listener with `getOperationName() = "INSERT"`, `getCollectionName() = "Order"`, `getInvokedVia() = Optional.of("gremlin_dsl")`, and `executionTimeNanos` reflecting the real executor drain time (not ~0 ns; a regression test asserts the duration is greater than zero for any non-trivial INSERT/UPDATE/DELETE). Without the wrapper-before-drain restructure, this call would emit a span with near-zero duration because the wrapper would time only the consumer's no-op iteration over the pre-fetched `InternalResultSet`. Without the non-idempotent-branch wrap at all, this call emits zero inner SQL spans.
...
- `core/src/main/java/com/jetbrains/youtrackdb/internal/core/db/DatabaseSessionEmbedded.java` (six wrap sites; five wrap AFTER `queryStartedLifecycle(original)` at lines 638, 679, 742, 785, 847 with `executeInternal()` line 702 idempotent branch contributing the 742 site, and the two `computeScript(...)` returns sitting at lines 785 and 847 with their method declarations at 753 and 813; the sixth site is `executeInternal()` non-idempotent prefetched branch at lines 733-739, restructured so the wrapper sits BEFORE the eager `forEachRemaining(...)` drain (new body: `var instrumented = wrapIfListening(original, statement, rawSql, args, currentTx); instrumented.forEachRemaining(prefetched::add); instrumented.close(); return new LocalResultSetLifecycleDecorator(prefetched);`), giving `executionTimeNanos` coverage of the executor's drain work rather than the consumer's no-op iteration over the pre-fetched `InternalResultSet`).
