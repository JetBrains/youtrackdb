...
- [x] Phase C track-level code review: PASS. Workflow-only diff, so the baseline group was skipped; four workflow-review agents ran (consistency, context-budget, writing-style, instruction-completeness), delta-scoped to the staged-vs-live edit per §1.7(h). One fix iteration (commit `39fbacc84d`), all should-fix, gate-checks PASS: tightened the prose-only hinge's "TOC" to "TOC-format" with an explicit single-section-rename→MEDIUM clause (HIGH-vs-MEDIUM tier boundary), scoped the `~14+` overblown flag to `low`/`medium` (reconciled with high-risk isolation's no-file-cap), and BLUF-led the `### Workflow machinery` section. Consistency clean (Seven-HIGH count, both new TOC rows, and all cross-refs verified). Suggestions not fixed: the glossary's "not a minimal file count" (load-bearing D2 disambiguation) and the soft `(12,14)` band (tildes + coherence cover it).
- **Deferred / out-of-track blocker (branch-wide).** This is the first branch to exercise §1.7 staging end-to-end, and it surfaced an inconsistency in develop's workflow tooling: `workflow-reindex.py` rule_1 demands a line-1 `workflow-sha` stamp on staged `.claude/workflow/**` copies, but §1.6(f) excludes staged copies from the stamped set, §1.7(e) mandates a verbatim copy of the unstamped live file, and the Phase 4 promotion `cp -r` does not strip stamps (a stamp would land on the live file). CI `workflow-toc-check.yml` runs `--check` (full sweep) on non-draft PRs — reproduced exit=1 on exactly the three staged copies — so this branch and Tracks 2/3 (which also stage `.claude/workflow/` copies) fail the TOC-check gate at ready-time. The staged copies are correct as-is; the agent's "prepend a stamp" suggestion was rejected because it would corrupt the live files at promotion. Fix: exempt staged `.claude/workflow`/`.claude/skills` copies from rule_1 in `workflow-reindex.py` (a live edit, outside staging scope). Filed as a dev-workflow YouTrack issue; out of this track's sizing/risk-taxonomy scope. PR stays in draft until resolved.
