...
**TL;DR.** The checklist sizing line is rewritten from `~N steps covering X, Y, Z` to `~N files covering X, Y, Z`. A step count pre-judges Phase A decomposition, since steps do not exist until execution; it anchors the reader on a number the workflow itself calls non-binding. A planned file count is a knowable plan-time fact; the review checks compare it against a track-level ceiling of ~20-25 files, distinct from the per-step ~12 / ~5 thresholds. The sizing check in structural and consistency review rekeys from a claimed-versus-described mismatch to a size-versus-norm comparison, staying plan-file-only.
...
The sizing check survives in rekeyed form. Structural and consistency review today flag "a track claiming ~2 steps but describing 8 distinct changes." After the change they compare the file footprint and the coverage-list cardinality against a track-level norm: a footprint at or above ~20-25 in-scope files signals a track that should split into dependent tracks. The unit is files, the same unit the per-step sizing rules use, but the threshold is track-level. A 5-7-step track aggregates many steps, so its footprint sits well above the per-step ~12 split cap and ~5 MEDIUM trigger; reusing those per-step numbers as the track ceiling would mis-flag normal-sized tracks. Structural review stays plan-file-only: the `~N files` figure lives in the plan-checklist `**Scope:**` line, so the check needs no track-file read.
...
- The file footprint is a track-level soft heuristic, not a per-step cap; it does not reintroduce the ~3-file step cap this design removes. The per-step file-count signals stay at ~12 / ~5; the track-level scope-indicator footprint ceiling is ~20-25 files. The branch rejects file count only as the *definition* of a step.
