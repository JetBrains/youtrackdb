...
- [ ] Track 2: Define the new shape + atomic shape switch (spec + directory rename + terminology rename + writer rewire + self-migration)
  ...
  > **Plan correction (from Track 1 Phase C, deferred finding WC#5):** when the `conventions-execution.md` §2.1 step is implemented, also update the agent-count summary line ("4 baseline + up to 6 conditional") to include the workflow-review tier introduced by Track 1 (e.g., "4 baseline + up to 6 conditional + up to 6 workflow-review"). The §2.1 step otherwise focuses on the lifecycle table; this is the only other §2.1 line that Track 1's change made stale.
  > **Depends on:** Track 1
