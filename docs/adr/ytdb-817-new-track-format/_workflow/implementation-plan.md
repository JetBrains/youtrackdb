...
- [ ] Track 2: Define the new shape + atomic shape switch (spec + directory rename + terminology rename + writer rewire + self-migration)
  ...
  > **Plan correction (from Track 1 Phase C, deferred finding WC#5; re-pinned at Phase A iter-1):** when the `conventions-execution.md` §2.1 step (Track 2 step 4) is implemented, also update the agent-count summary line ("4 baseline + up to 6 conditional, max 3 iterations") at `conventions-execution.md:304` — which lives under **§2.4 "Commit messages, code review, complexity, decomposition"**, NOT §2.1 as the original note implied (Phase A iter-1 finding T5 verified). The §2.1 step's scope expands to include this single §2.4 line so the §2.4 agent-count summary reflects the workflow-review tier Track 1 added ("4 baseline + up to 6 conditional + up to 6 workflow-review, max 3 iterations"). This is the only stale agent-count summary in `conventions-execution.md`; track-2.md step 4 captures the per-step detail.
  > **Depends on:** Track 1
