...
- [x] Track 4: Update readers (workflow docs, sub-agent prompts, remaining references)
  ...
  >
  > **Track episode:**
  > Closed the reader-half of the 14-section ExecPlan migration: every `.claude/workflow/` doc, sub-agent prompt under `.claude/workflow/prompts/`, and reader skill that grepped a per-track file by section heading or per-step blockquote now performs the new section-join (`## Concrete Steps` roster line for the `risk:` tag + `## Episodes` block by step-number-primary / commit-SHA-disambiguator). Six steps (1 high, 2 medium, 3 medium, 4 low, 5 high, 6 medium), all `[x]`, 0 failed; two Phase B `risk: high` step-level review loops (Step 1, Step 5) plus one Phase C track-level review iteration on the cumulative diff, all PASS terminal. The atomic shape-switch boundary from Track 2 step 6 closes here: the transient reader-side staleness window is now zero.
  >
  > **Phase C** ran 5 workflow-review dimensions (WC / WP / WI / WB / WS); WH skipped (no hooks / scripts / settings in the diff); baseline 4 Java-focused agents skipped per the workflow-only diff override. 8 in-scope findings applied in commit `ba37514f86`: M1 [blocker] aligned the track-level Mermaid diagram location across the three byte-identical Phase A sub-agent prompts to `## Context and Orientation` (matching the writer); M2 [blocker] swept a stale `## Reviews completed` read target in `track-review.md` L399–401 to `## Outcomes & Retrospective`; M3 [blocker] dropped a misleading `conventions-execution.md` §2.1 citation in `track-review.md` L355–360 for the Pre-Flight Clarifications writer prose; five should-fix findings covered retired-vocabulary residue in `create-final-design.md`, em-dash overload in `risk-tagging.md` and `workflow.md` user-interaction table, an explicit `## Episodes` enumeration in `create-final-design.md`'s Read-list, and a paragraph break in `track-code-review.md` sub-agent context block. Gate-check PASS across all 4 re-checked dimensions (WC, WP, WI, WS) with zero new in-scope findings — iteration 1 terminal.
  >
  > **Cross-track impact for Phase 4:** five spec docs still carry the legacy phrasing the reader/prompt side has migrated away from. Fold into the existing WC3 sync backlog: `conventions-execution.md` L229 (Clarifications → `## Decision Log` should become `## Context and Orientation`); `design.md` migration-table rows L294 (Mermaid location) + L295 (Clarifications target); `planning.md` L403–405 (Mermaid in Interfaces and Dependencies); `design-document-rules.md` L39 (Mermaid in Plan of Work). Phase 4 `create-final-design.md` distillation should sync these alongside the existing `Context: <level>` → `[ctx=<level>]` drift Track 2 flagged.
  >
  > **Discoveries propagated through episodes (for Phase 4 distillation):** (a) C&O-as-current-state idiom (Step 1) — the carve-out rule for classification axes that split sections by intent vs. current-state; (b) forward-looking-rewrite precedent (Step 4) — drop legacy-shape historical framing in favour of clean prose against the new vocabulary; (c) smoke-chain grep + human-eye as a non-redundant verification idiom (Step 6) — catches load-bearing prose mentions outside template blocks that per-step Phase B sweeps cannot reach.
  >
