...
- **`readability-feedback/SKILL.md`** — its `## Rule sync map` (line 41) is the codified add-a-rule procedure; the new cold-read block needs a row in its design-review entry (which today lists only the Human-reader bullets). The same file's governance grep (line 54) is flipped by Track 1; see the overlap note below. **Plan correction (WC1, from Track 1 Phase C review):** this file's audit sub-agent prompt also omits the `## Orientation` rule Track 1 adds — its STEP 1 house-style section list (line 70) and its GAP-routing target guidance (line 36) name the other house-style sections but not `## Orientation`. A too-terse passage, the failure `## Orientation` now governs, therefore misroutes as a GAP ("no rule addresses it") when one now does, defeating the readability-feedback convergence premise. The fix location sits in neither track's original roster; the Track 1 reviewer endorsed folding it here, since this track already edits the file.
...
4. **Propagate `## Orientation` into the readability audit prompt (WC1 plan correction).** Add `§ Orientation` to `readability-feedback/SKILL.md`'s audit sub-agent prompt — its STEP 1 "note especially" house-style section list (`:70`) and its GAP-routing target-section guidance (`:36`, general too-terse / one-line-assertion tells route to `§ Orientation`) — so the audit recognises the too-terse rule rather than misclassifying its hits as a GAP. A one-line addition on each of the two lines; it does not touch the governance grep (Track 1's, kept four-string) or the step-1 Rule-sync-map row above. Folded here per the Track 1 reviewer's endorsement; keep all readability-feedback `## Orientation` propagation in this track's single edit of the file.
...
- `readability-feedback/SKILL.md`'s audit sub-agent prompt names `§ Orientation` in both its STEP 1 "note especially" section list (`:70`) and its GAP-routing target guidance (`:36`), so a too-terse passage routes to `§ Orientation` rather than GAP (WC1 plan correction).
...
- `.claude/skills/readability-feedback/SKILL.md` — the Rule sync map design-review row, **plus** the audit sub-agent prompt's `## Orientation` references (STEP 1 section list `:70` + GAP-routing guidance `:36`, WC1 plan correction from Track 1 review). The file's governance grep is Track 1's.
