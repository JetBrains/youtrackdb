...
- **Risks/Caveats**: ~54 hand edits, no generator (`workflow-reindex.py` only rebuilds TOC and stamps; only the hook holds blurb text). The atomic-sync constraint (A4): the flip lands as one commit, or at minimum inside this track with the four-vs-five window closed before Phase C — under live-edit (D5), any split that commits the canonical sites at five while blurbs still read four leaves a window `review-workflow-consistency` flags. Three sites escape the narrow `banned-section heading slugs` grep and need hand-edits: `review-workflow-pr/SKILL.md:44-45` hard-wraps the find string across a line break, and `commit-conventions.md:191-194` and `implementer-rules.md:1102-1105` are closed-set enumerations in differing surrounding sentences (the first line-wraps the literal, the second uses the variant phrasing "the four section slugs that make up the Tier-B AI-tell subset") — these two take the reworded blurb adapted to their context, not the byte-identical paste. `test_house_style_hook.py:694-697` pins the section strings and gates the sync's correctness for the hook.
...
4. **The atomic subset flip (D1).** Flip four→five across all ~54 sites in one commit (or with the window closed before Phase C): the 30 "banned-section heading slugs" blurbs take the canonical reworded sentence byte-identically; the 11 chat blurbs take the find/replace pair (and `house-conversation.md`'s "four → five" count bumps); the two governance greps gain `Orientation`; the `ai-tells` catalogue gains an Orientation row; `test_house_style_hook.py` updates its pinned section list. Hand-edit the three sites the narrow grep misses: the hard-wrapped chat-blurb site (`review-workflow-pr/SKILL.md:44-45`), and the two closed-set enumerations (`commit-conventions.md:191-194`, `implementer-rules.md:1102-1105`), which take the canonical reworded sentence adapted to their differing surrounding context.
...
- After this track, the governance grep `grep -rln 'Banned vocabulary\|Banned sentence patterns\|Banned analysis patterns\|Em-dash discipline' .claude/ CLAUDE.md` returns 54 files, and **every closed-set enumeration among them names `## Orientation`** — the 30 "banned-section heading slugs" blurbs, the 11 chat "AI-tell subset of" blurbs, the two governance greps (`readability-feedback/SKILL.md`, `conventions.md:570`), the `ai-tells` catalogue, and the two sites the narrow grep misses (`commit-conventions.md`, `implementer-rules.md`); the three files that match the grep but cite individual `§` sections rather than the closed set (`review-workflow-writing-style.md`, `design-mechanical-checks.py`, `design-document-rules.md`) are unaffected. No site enumerates the subset as four-of-five. (The old narrow `grep -rln 'banned-section heading slugs'` check is insufficient — it returns 30 and silently misses the two line-wrapped / variant-phrased sites.)
...
- `.claude/workflow/commit-conventions.md` (`:191-194`) and `.claude/workflow/implementer-rules.md` (`:1102-1105`) — two four-name closed-set enumerations the narrow `banned-section heading slugs` grep misses (the first line-wraps the literal, the second uses variant phrasing); flip four→five, taking the canonical reworded sentence adapted to their differing surrounding context, not the byte-identical paste.
